...
import { FlipDirection, KeyModifiers, Keys } from '../../../src/diagram/enum/enum';
...
describe('Diagram Control', () => {
    describe('Testing the Commands - Command Manager', () => {
        ...
        beforeAll((): void => {
            ...
            ele = createElement('div', { id: 'diagram_command1' });
            ...
            diagram.appendTo('#diagram_command1');
        ...
        it('Checking commands - nudge commands', (done: Function) => {
            ...
            const offsetX: number = diagram.selectedItems.nodes[0].offsetX;
            const offsetY: number = diagram.selectedItems.nodes[0].offsetY;
            mouseEvents.keyDownEvent(diagramCanvas, 'Up');
            expect(diagram.selectedItems.nodes[0].offsetY == (offsetY-1)).toBe(true);
            mouseEvents.keyDownEvent(diagramCanvas, 'Down');
            expect(diagram.selectedItems.nodes[0].offsetY == offsetY).toBe(true);
            mouseEvents.keyDownEvent(diagramCanvas, 'Left');
            expect(diagram.selectedItems.nodes[0].offsetX == (offsetX-1)).toBe(true);
            mouseEvents.keyDownEvent(diagramCanvas, 'Right');
            expect(diagram.selectedItems.nodes[0].offsetX == offsetX).toBe(true);
            done();
        });
...
describe('SendToBack ordercommands', () => {
    ...
    let ele: HTMLElement;
    beforeAll((): void => {
        ele = createElement('div', { id: 'diagram_div1' });
        document.body.appendChild(ele);
        var nodes:NodeModel[] = [
            {
              id: 'node1',
...
describe('SendToBack exception', () => {
    ...
    beforeAll((): void => {
        ele = createElement('div', { id: 'diagram_div2' });
        document.body.appendChild(ele);
        let nodes: NodeModel[] = [
            {
                id: "rectangle1",
...
describe('Diagram keyboard commands', () => {
    describe('Testing the Commands', () => {
        ...
        it('Checking commands - move elements 5 pixel commands', (done: Function) => {
            ...
            expect(diagram.selectedItems.nodes[0].offsetY == (offsetY-5)).toBe(true);
            mouseEvents.keyDownEvent(diagramCanvas, 'Down',false, true);
            expect(diagram.selectedItems.nodes[0].offsetY == offsetY).toBe(true);
            mouseEvents.keyDownEvent(diagramCanvas, 'Left',false, true);
            expect(diagram.selectedItems.nodes[0].offsetX == (offsetX-5)).toBe(true);
...
describe('Diagram Testing Tab keyboard commands', () => {
    let diagram: Diagram;
    let ele: HTMLElement;
    let mouseEvents: MouseEvents = new MouseEvents();
    let diagramCanvas: HTMLElement;
    beforeAll((): void => {
        ...
        ele = createElement('div', { id: 'diagram_tabcommand3' });
        document.body.appendChild(ele);
        let node: NodeModel = {
            id: 'node1', width: 100, height: 100, offsetX: 100,
            offsetY: 100, annotations: [{ content: "node1" }]
        ...
        diagram.appendTo('#diagram_tabcommand3');
    });
...
describe(' keyboard commands', () => {
    describe('Testing the order Commands', () => {
        ...
        beforeAll((): void => {
            ...
            ele = createElement('div', { id: 'diagram_command4' });
            document.body.appendChild(ele);
            let node: NodeModel = {
            ...
            diagram.appendTo('#diagram_command4');
        });
        ...
        it('Checking sendbackward commands', (done: Function) => {
            ...
            expect(zIndexBeforeCall === 1 && zIndexAfterCall === -1).toBe(true);
            done();
        });
...
describe('Diagram Control', () => {
    ...
    let ele: HTMLElement;
    beforeAll((): void => {
        ele = createElement('div', { id: 'diagram_command5' });
        document.body.appendChild(ele);
        let selArray: (NodeModel)[] = [];
        let nodes: NodeModel[] = [
            {
        ...
        diagram.appendTo('#diagram_command5');
        selArray.push(diagram.nodes[0]);
        diagram.select(selArray);
    });
    afterAll((): void => {
    ...
    it('selection change in BPMNSubProcess1', (done: Function) => {
        diagram.select([diagram.nodes[3]]);
        diagram.select([diagram.connectors[0]]);
        expect(diagram.nodes.length>0).toBe(true);
        done();
    });
    it('selection change in BPMNSubProcess2', (done: Function) => {
        diagram.select([diagram.nodes[3]]);
...
describe('connection Event Change', () => {
    let diagram: Diagram;
    let ele: HTMLElement;
    let mouseEvents = new MouseEvents();
    beforeAll((): void => {
        ele = createElement('div', { id: 'connectionEventChange' });
        document.body.appendChild(ele);
        ...
        diagram.appendTo('#connectionEventChange');
    });
    afterAll((): void => {
        diagram.destroy();
        ele.remove();
    ...
    it('connector with bpmn sub processes', (done: Function) => {
        diagram.select([diagram.connectors[0]]);
        let decorator = document.getElementById('connectorSourceThumb');
        let diagramCanvas = document.getElementById(diagram.element.id + 'content');
        let bounds: any = decorator.getBoundingClientRect();
        mouseEvents.mouseDownEvent(diagramCanvas, bounds.x, bounds.y, false, false);
        mouseEvents.mouseMoveEvent(diagramCanvas, bounds.x, bounds.y, false, false);
        mouseEvents.mouseMoveEvent(diagramCanvas, bounds.x - 50, bounds.y, false, false);
        mouseEvents.mouseMoveEvent(diagramCanvas, bounds.x - 100, bounds.y, false, false);
        mouseEvents.mouseUpEvent(diagramCanvas, bounds.x - 100, bounds.y, false, false);
        expect(diagram.nodes.length > 0).toBe(true);
        done();
    });
...
describe('copy paste node and connector', () => {
    let diagram: Diagram;
    let ele: HTMLElement;
    let mouseEvents = new MouseEvents();
    beforeAll((): void => {
        ele = createElement('div', { id: 'copyPaste' });
        document.body.appendChild(ele);
        ...
        diagram = new Diagram({
            width: 1000, height: 600, nodes: nodes, connectors: connectors
        });

        diagram.appendTo('#copyPaste');
    });
    afterAll((): void => {
        diagram.destroy();
        ele.remove();
    ...
    it('copy-paste connector connected to node at source end', (done: Function) => {
        let node: NodeModel = {
            id: 'Node3',
            offsetX: 300,
            offsetY: 220,
        ...
        diagram.add(node);
        const connector1 = diagram.nameTable['connector1'];
        diagram.select([connector1]);

        let diagramCanvas = document.getElementById(diagram.element.id + 'content');
        let diagramLeft = diagram.element.offsetLeft;
        let diagramTop = diagram.element.offsetTop;
        ...
        let node3: any = diagram.getObject('Node3');
        let node3CenterX = diagramLeft + node3.offsetX;
        let node3CenterY = diagramTop + node3.offsetY;

        mouseEvents.mouseDownEvent(diagramCanvas, sourceCx, sourceCy);
        mouseEvents.mouseMoveEvent(diagramCanvas, sourceCx + (node3CenterX - sourceCx) * 0.5, sourceCy + (node3CenterY - sourceCy) * 0.5);
        mouseEvents.mouseMoveEvent(diagramCanvas, node3CenterX, node3CenterY);
        mouseEvents.mouseUpEvent(diagramCanvas, node3CenterX, node3CenterY);

        diagram.select([connector1, node3]);
        diagram.copy();
        diagram.paste();
    ...
    it('copy-paste connector connected to node at target end', (done: Function) => {
        let node: NodeModel = {
            id: 'Node5',
            offsetX: 600,
            offsetY: 220,
        ...
        diagram.add(node);
        const connector2 = diagram.nameTable['connector2'];
        ...
        let node5: any = diagram.getObject('Node5');
        let node5CenterX = diagramLeft + node5.offsetX;
        let node5CenterY = diagramTop + node5.offsetY;

        mouseEvents.mouseDownEvent(diagramCanvas, sourceCx, sourceCy);
        mouseEvents.mouseMoveEvent(diagramCanvas, sourceCx + (node5CenterX - sourceCx) * 0.5, sourceCy + (node5CenterY - sourceCy) * 0.5);
        mouseEvents.mouseMoveEvent(diagramCanvas, node5CenterX, node5CenterY);
        mouseEvents.mouseUpEvent(diagramCanvas, node5CenterX, node5CenterY);
        
        diagram.select([connector2, node5]);
        diagram.copy();
        diagram.paste();
