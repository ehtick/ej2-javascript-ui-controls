...
describe('Diagram Control', () => {
    ...
    describe('Bug 909155- Issue in connecting nodes inside swimlane when undo redo', () => {
        ...
        let ele: HTMLElement;
        let mouseEvents: MouseEvents = new MouseEvents();
        ele = createElement('div', { id: 'diagramSwim909155' });
        document.body.appendChild(ele);
        beforeAll((): void => {
            diagram = new Diagram({
                width: 1500, height: 1000,
            });
            diagram.appendTo('#diagramSwim909155');
        });
        afterAll((): void => {
            diagram.destroy();
            diagram = null;
        ...
        it('Add swimlanes and child nodes-connectors and perform undo-redo ', (done: Function) => {
            let swimlane1:NodeModel = {id:'swimlane1',shape:{type:'SwimLane',lanes:[{id:'lane1',header:{width:50,height:50,style:{strokeColor:'black',fontSize:11}}}],orientation:'Horizontal'},height:250,width:400,offsetX:300,offsetY:300};
            diagram.add(swimlane1);
            let child1:NodeModel = {id:'child1',annotations:[{content:'Child1'}],height:50,width:100,margin:{left:50,top:50}};
            diagram.addNodeToLane(child1,swimlane1.id,'lane1');
            let child2:NodeModel = {id:'child2',annotations:[{content:'Child2'}],height:50,width:100,margin:{left:200,top:50}};
            diagram.addNodeToLane(child2,swimlane1.id,'lane1');
            let con1:ConnectorModel = {id:'con1',sourceID:'child1',targetID:'child2',type:'Orthogonal'};
            diagram.add(con1);
            diagram.undo();
            diagram.undo();
            diagram.undo();
            ...
            diagram.redo();
            diagram.clear();
            let swimlane2:NodeModel = {id:'swimlane2',shape:{type:'SwimLane',lanes:[{id:'lane1',header:{width:50,height:50,style:{strokeColor:'black',fontSize:11}}}],orientation:'Vertical'},height:250,width:400,offsetX:300,offsetY:300};
            diagram.add(swimlane2);
            let child3:NodeModel = {id:'child3',annotations:[{content:'Child3'}],height:50,width:100,margin:{left:50,top:50}};
            diagram.addNodeToLane(child3,swimlane2.id,'lane1');
            let child4:NodeModel = {id:'child4',annotations:[{content:'Child4'}],height:50,width:100,offsetX:250,offsetY:470};
            diagram.add(child4);
            let con2:ConnectorModel = {id:'con2',sourceID:'child4',targetID:'child3',type:'Orthogonal'};
            diagram.add(con2);
            diagram.undo();
            diagram.undo();
            diagram.undo();
            ...
            diagram.redo();
            diagram.redo();
            expect(diagram.nodes.length === 5 && diagram.connectors.length === 1 && (diagram.connectors[0].zIndex > diagram.nodes[0].zIndex)).toBe(true);
            done();
        });
    });
