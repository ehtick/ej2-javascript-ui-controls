...
function checkPortdirection(
    ...
    //1014469: Connector path not routed based on port connection direction
    const valdiatePortDirection: string = (sourcePort && (sourcePort as any).connectionDirection) ?
        (sourcePort as any).connectionDirection : 'Auto';
    const compareDirection: string = valdiatePortDirection !== 'Auto' ? valdiatePortDirection : direction;
    if (seg.direction !== compareDirection) {
        pointsFromNodeToPoint(seg, compareDirection, bounds, seg.points[0], seg.points[seg.points.length - 1], false);
