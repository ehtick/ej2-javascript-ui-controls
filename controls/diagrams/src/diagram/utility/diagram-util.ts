...
export function serialize(model: Diagram): string {
    ...
    model.nodes.forEach((node: Node) => {
        if (node.shape.type === 'SwimLane') {
            const processes: any = model.commandHandler.findProcesses(node);
            ...
            if (processes.length > 0) {
                removeNodes = removeNodes.filter((removeNodeId: string) => !processes.includes(removeNodeId));
            }
        }
    });
    ...
    */
    ...
    if (dataObj.connectors) {
        for (let i: number = 0; i < dataObj.connectors.length; i++) {
            ...
            if (dataObj.connectors[i].type === 'Bezier' && dataObj.connectors[i].segments && dataObj.connectors[i].segments.length > 0 && dataObj.connectors[i].bezierSettings && dataObj.connectors[i].bezierSettings.allowSegmentsReset) {
    ...
    diagram.nodes.forEach((node: Node) => {
        if (node.shape.type === 'SwimLane') {
            const processes = diagram.commandHandler.findProcesses(node);
            processes.forEach((processId: string) => {
 ...
 * @param {boolean} isGroupNode - provide the details about it is group or not.
 * @private
 */
export function updateStyle(changedObject: TextStyleModel, target: DiagramElement, isTransactionChild?: boolean, isEvent?: boolean,
                            isGroupNode?: boolean): void {
