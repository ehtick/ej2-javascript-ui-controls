...
export class DiagramEventHandler {
    ...
    private getMouseEventArgs(position: PointModel, args: MouseEventArgs, source?: IElement, padding?: number): MouseEventArgs {
        ...
        //1015628 - Touch Zoom Does Not Preserve Focused Element Position in Diagram.
        //For pinch zoom, compute midpoint in diagram coords and set args.position so zoom centers on the gesture point.
        if (args.moveTouches && (args.moveTouches as any).length > 1) {
            const touches: any[] = [...(args.moveTouches as any)];
            // Use the first two touches
            const t0: any = touches[0];
            const t1: any = touches[1];
            const toDiagram: (pageX: number, pageY: number) => PointModel = (pageX: number, pageY: number) => {
                let offsetX: number = this.diagram.modifyClientOffset(pageX);
                let offsetY: number = this.diagram.modifyClientOffset(pageY);
                const ruler: Size = getRulerSize(this.diagram);
                const rect: ClientRect = this.diagram.element.getBoundingClientRect();
                offsetX = offsetX + this.diagram.diagramCanvas.scrollLeft - rect.left - ruler.width;
                offsetY = offsetY + this.diagram.diagramCanvas.scrollTop - rect.top - ruler.height;
                offsetX = (offsetX / this.diagram.scroller.transform.scale) - this.diagram.scroller.transform.tx;
                offsetY = (offsetY / this.diagram.scroller.transform.scale) - this.diagram.scroller.transform.ty;
                return { x: offsetX, y: offsetY } as PointModel;
            };
            const p0: PointModel = toDiagram(t0.pageX || t0.clientX || t0.x, t0.pageY || t0.clientY || t0.y);
            const p1: PointModel = toDiagram(t1.pageX || t1.clientX || t1.x, t1.pageY || t1.clientY || t1.y);
            args.position = { x: (p0.x + p1.x) / 2, y: (p0.y + p1.y) / 2 };
        }
        return args;
    }
