        ...
        if (this.objectType !== 'Port') {
            ...
            if (((object as Node).id === 'helper')|| ((object as Node).id !== 'helper')) {
                ...
                if (!isSubGroupSelection &&
                    ...
                    //Bug(1013865) - Connector Segment Issue in undo/redo
                    if (obj && this.undoElement) {
                        if (this.undoElement.connectors && this.undoElement.connectors.length > 0) {
                            for (const connector of this.undoElement.connectors) {
                                const isConnectorPresent = obj.connectors.find((conn: Connector) => conn.id === connector.id);
                                if (!isConnectorPresent) {
                                    const currentConnector: Connector = this.commandHandler.diagram.nameTable[`${connector.id}`];
                                    if (currentConnector) {
                                        obj.connectors.push(cloneObject(currentConnector));
                                    }
                                }
                            }
                        }
                        if (this.undoElement.nodes && this.undoElement.nodes.length > 0) {
                            for (const node of this.undoElement.nodes) {
                                const isNodePresent = obj.nodes.find((node: Node) => node.id === node.id);
                                if (!isNodePresent) {
                                    const currentNode: Node = this.commandHandler.diagram.nameTable[`${node.id}`];
                                    if (currentNode) {
                                        obj.nodes.push(cloneObject(currentNode));
                                    }
                                }
                            }
                        }
