    ...
    private renderPopup(): void {
        ...
        this.popupObj = new Popup(this.popupWrapper as HTMLElement, {
            ...
            position: (this.isMobile ?
                (this.isValidPreset() && !this.isCustomWindow ? { X: 'left', Y: 'bottom' } : { X: 'center', Y: 'center' }) :
                this.enableRtl ? { X: 'left', Y: 'bottom' } : { X: 'right', Y: 'bottom' }),
            offsetX: this.isMobile || this.enableRtl ? 0 : -popupWidth,
            offsetY: OFFSETVALUE,
            collision: this.isMobile ?
                (this.isValidPreset() && !this.isCustomWindow ? { X: 'fit' } : { X: 'fit', Y: 'fit' }) : { X: 'fit', Y: 'flip' },
                ...
                this.trigger('open', eventArgs, (eventArgs: RangePopupEventArgs) => {
                    ...
                    if (!this.openEventArgs.cancel) {
                        ...
                        if (!isNullOrUndefined(this.mobileRangePopupWrap) && !isNullOrUndefined(this.popupObj)
                            ...
                            this.isValidPreset() && isPreset && (this.isMobile || Browser.isDevice) && !this.isCustomWindow) {
