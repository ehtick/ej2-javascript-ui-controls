...
import { crossReferenceTable, passwordInput, jsonData } from "./inputs.spec";
...
describe('Viewer Reported Issues', () => {
    ...
    it('1021081 - Coverage', () => {
       let jsonDoc: _JsonDocument = new _JsonDocument();
       jsonDoc._isColorSpace = true;
       let table: Map<string, string> = new Map<string, string>();
    ...
    it('1020747 - Annotation Import Issue', () => {
        ...
        document = new PdfDocument(updatedData);
        page = document.getPage(0);
        let annot = page.annotations.at(0) as PdfRubberStampAnnotation;
        ...
        expect(result[3]._operands).toEqual(['/62a5f61c-b4dc-4de1-a6b9-5566cbb86a5e']);
	...
	it('1025945 - Free Text Font issue', () => {
        ...
        annot.borderColor = { r: 255, g: 0, b: 0 };
        annot.border.width = 7;
        annot.font = new PdfStandardFont(PdfFontFamily.timesRoman, 20);
        ...
        annot.name = 'Syncfusion';
        annot.opacity = 1.0;
        annot.subject = 'Annotation';
        annot.text = 'Syncfusion';
        annot.setAppearance(true);
        page.annotations.add(annot);
        let updatedData = document.save();
        page = document.getPage(0) as PdfPage;
        let freeTextAnnot = page.annotations.at(0) as PdfFreeTextAnnotation;
        expect(freeTextAnnot._dictionary.get('RC')).toEqual('<?xml version="1.0"?><body xmlns="http://www.w3.org/1999/xhtml" style="font:Times 20pt;font-weight:normal;color:#000000"><p dir="ltr"><span style = "text-align:center;">Syncfusion</span></p></body>');
        document.destroy();
    });
	it('1026448 - _PdfMainObjectCollection constructor with Info as _PdfReference returning valid _PdfDictionary', () => {
        let document: PdfDocument = new PdfDocument(crossReferenceTable);
        document.fileStructure.isIncrementalUpdate = false;
        let crossRef: any = document._crossReference;
        let infoDict: _PdfDictionary = new _PdfDictionary(crossRef);
        infoDict.set('Title', 'Test Document');
        infoDict.set('Author', 'Test Author');
        let infoRef: _PdfReference = crossRef._getNextReference();
        crossRef._cacheMap.set(infoRef, infoDict);
        crossRef._trailer.update('Info', infoRef);
        let updated = document.save();
        expect(crossRef._offsets.indexOf(17) >= 0).toBeTruthy();
        document.destroy();
    });
    it('1026448 - _PdfMainObjectCollection constructor with Info as _PdfDictionary', () => {
        let document: PdfDocument = new PdfDocument(crossReferenceTable);
        document.fileStructure.isIncrementalUpdate = false;
        let crossRef: any = document._crossReference;
        let infoDict: _PdfDictionary = new _PdfDictionary(crossRef);
...
describe('1026363 - allowImportCustomData full coverage', () => {
    it('1026363 - allowImportCustomData XFDF Coverage - string with colon splits to more than 2 parts', () => {
        ...
        document._allowImportCustomData = true;
        let annotation: PdfRubberStampAnnotation = new PdfRubberStampAnnotation({ x: 10, y: 10, width: 100, height: 100 });
        annotation._dictionary.set('CustomData', 'a:b:c');
        page.annotations.add(annotation);
        let settings: PdfAnnotationExportSettings = new PdfAnnotationExportSettings();
        settings.dataFormat = DataFormat.xfdf;
        let exportedData: Uint8Array = document.exportAnnotations(settings);
        document.destroy();
        document = new PdfDocument();
        document.addPage();
        document._allowImportCustomData = true;
        ...
        let loadedPage: PdfPage = document.getPage(0);
        let loadedAnnotation: PdfRubberStampAnnotation = loadedPage.annotations.at(0) as PdfRubberStampAnnotation;
        expect(loadedAnnotation._dictionary.get('CustomData')).toEqual('a:b:c');
        document.destroy();
    });
    it('1026363 - allowImportCustomData XFDF Coverage - string without colon', () => {
        let document: PdfDocument = new PdfDocument();
        document.addPage();
        let page: PdfPage = document.getPage(0);
        document._allowImportCustomData = true;
        let annotation: PdfRubberStampAnnotation = new PdfRubberStampAnnotation({ x: 10, y: 10, width: 100, height: 100 });
        annotation._dictionary.set('CustomData', 'simpleString');
        page.annotations.add(annotation);
        let settings: PdfAnnotationExportSettings = new PdfAnnotationExportSettings();
        settings.dataFormat = DataFormat.xfdf;
        let exportedData: Uint8Array = document.exportAnnotations(settings);
        ...
        let loadedPage: PdfPage = document.getPage(0);
        let loadedAnnotation: PdfRubberStampAnnotation = loadedPage.annotations.at(0) as PdfRubberStampAnnotation;
        expect(loadedAnnotation._dictionary.get('CustomData')).toEqual('simpleString');
        document.destroy();
    });
    it('1026363 - allowImportCustomData XFDF Coverage - non-string value number', () => {
        let document: PdfDocument = new PdfDocument();
        document.addPage();
        let page: PdfPage = document.getPage(0);
        document._allowImportCustomData = true;
        let annotation: PdfRubberStampAnnotation = new PdfRubberStampAnnotation({ x: 10, y: 10, width: 100, height: 100 });
        annotation._dictionary.set('CustomData', 12345);
        page.annotations.add(annotation);
        let settings: PdfAnnotationExportSettings = new PdfAnnotationExportSettings();
        settings.dataFormat = DataFormat.xfdf;
        let exportedData: Uint8Array = document.exportAnnotations(settings);
        ...
        let loadedPage: PdfPage = document.getPage(0);
        let loadedAnnotation: PdfRubberStampAnnotation = loadedPage.annotations.at(0) as PdfRubberStampAnnotation;
        expect(loadedAnnotation._dictionary.get('CustomData')).toEqual('12345');
        document.destroy();
    });
    it('1026363 - allowImportCustomData JSON Coverage - string with colon splits to 2 parts', () => {
        let document: PdfDocument = new PdfDocument();
        document.addPage();
        let page: PdfPage = document.getPage(0);
        document._allowImportCustomData = true;
        let annotation: PdfRubberStampAnnotation = new PdfRubberStampAnnotation({ x: 10, y: 10, width: 100, height: 100 });
        annotation._dictionary.set('CustomData', 'testKey:testValue');
        page.annotations.add(annotation);
        let settings: PdfAnnotationExportSettings = new PdfAnnotationExportSettings();
        settings.dataFormat = DataFormat.json;
        let exportedData: Uint8Array = document.exportAnnotations(settings);
        ...
        let loadedPage: PdfPage = document.getPage(0);
        let loadedAnnotation: PdfRubberStampAnnotation = loadedPage.annotations.at(0) as PdfRubberStampAnnotation;
        expect(loadedAnnotation._dictionary.get('CustomData')).toEqual('{"testKey":"testValue"}');
        document.destroy();
    });
    it('1026363 - allowImportCustomData JSON Coverage - string with colon splits to more than 2 parts', () => {
        let document: PdfDocument = new PdfDocument();
        document.addPage();
        let page: PdfPage = document.getPage(0);
        document._allowImportCustomData = true;
    ...
    it('1026363 - allowImportCustomData JSON Coverage - string without colon', () => {
        let document: PdfDocument = new PdfDocument();
        document.addPage();
        let page: PdfPage = document.getPage(0);
        document._allowImportCustomData = true;
    ...
    it('1026363 - allowImportCustomData JSON Coverage - non-string value number', () => {
        let document: PdfDocument = new PdfDocument();
        document.addPage();
        let page: PdfPage = document.getPage(0);
        document._allowImportCustomData = true;
    ...
    it('1026363 - allowImportCustomData JSON Coverage - key is type', () => {
        let document: PdfDocument = new PdfDocument();
        document.addPage();
        let page: PdfPage = document.getPage(0);
        document._allowImportCustomData = true;
        let annotation: PdfRubberStampAnnotation = new PdfRubberStampAnnotation({ x: 10, y: 10, width: 100, height: 100 });
        annotation._dictionary.set('type', 'Stamp');
        page.annotations.add(annotation);
        let settings: PdfAnnotationExportSettings = new PdfAnnotationExportSettings();
        settings.dataFormat = DataFormat.json;
        let exportedData: Uint8Array = document.exportAnnotations(settings);
        ...
        let loadedPage: PdfPage = document.getPage(0);
        let loadedAnnotation: PdfRubberStampAnnotation = loadedPage.annotations.at(0) as PdfRubberStampAnnotation;
        expect(loadedAnnotation._dictionary.has('type')).toBeFalsy();
        document.destroy();
    });
    it('1026363 - allowImportCustomData JSON Coverage - key is page', () => {
        let document: PdfDocument = new PdfDocument();
        document.addPage();
        let page: PdfPage = document.getPage(0);
        document._allowImportCustomData = true;
        let annotation: PdfRubberStampAnnotation = new PdfRubberStampAnnotation({ x: 10, y: 10, width: 100, height: 100 });
        annotation._dictionary.set('page', '0');
        page.annotations.add(annotation);
        let settings: PdfAnnotationExportSettings = new PdfAnnotationExportSettings();
        settings.dataFormat = DataFormat.json;
        let exportedData: Uint8Array = document.exportAnnotations(settings);
