 ...
 */
export function _decodeName(name: string): string {
    if (!name || name.length === 0) {
        return name;
    }
    const pattern: RegExp = /_[Xx]([0-9A-Fa-f]{4}|[0-9A-Fa-f]{8})_/g;
    return name.replace(pattern, function (_match: string, hex: string): string {
        ...
        const code: number = codePoint - 0x10000;
        const high: number = 0xd800 + (code >> 10);
        const low: number = 0xdc00 + (code & 0x3ff);
