...
import { _stringToAnnotationFlags, _convertToColor, _encode, _byteArrayToHexString, _stringToBytes, _annotationFlagsToString, _bytesToString, _hexStringToByteArray, _decode, _isNullOrUndefined, _compressStream, _hasUnicodeCharacters, _decodeName } from './../utils';
...
export class _JsonDocument extends _ExportHelper {
    ...
    _writeDictionary(dictionary: _PdfDictionary, pageIndex: number, hasAppearance: boolean): void {
        ...
        dictionary.forEach((key: string, value: any) => { // eslint-disable-line
            if (!((!hasAppearance && key === 'AP') || key === 'P'
                || key === 'Parent' || key === 'Measure')) {
                ...
                if (entry && entry instanceof _PdfDictionary) {
                    ...
                    this._writeAttribute(key, entry, dictionary);
                }
            }
        });
    ...
    _writeAttribute(key: string, primitive: any, dictionary: _PdfDictionary): void { // eslint-disable-line
        ...
        case 'RC':
            ...
            if (typeof rcValue === 'string' && rcValue.includes('<body')) {
                const index: number = rcValue.indexOf('<body');
                if (index > 0) {
    ...
    _writeObject(table?: Map<string, string>,
        ...
        if (value instanceof _PdfName) {
            ...
            if (key !== 'AllowedInteractions' && !this._isColorSpace) {
        ...
        annotationKeys.forEach((key: string) => {
            ...
            case 'contents-richtext':
                if (typeof value === 'string') {
                    this._addString(dictionary, 'RC', value);
                }
                break;
            case 'flags':
