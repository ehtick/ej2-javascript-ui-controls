...
describe('Splitter Control', () => {
    ...
    describe('Check paneSettigns in expand and collapse', () => {
        let splitterObj: any;
        beforeAll((): void => {
        let element: HTMLElement = createElement('div', { id: 'default'});
        let child1: HTMLElement = createElement('div');
        element.appendChild(child1);
        document.body.appendChild(element);
        splitterObj = new Splitter({ height: '400px', width: '400px', paneSettings: [{ size: '30%', content:'splitter content', collapsible: true },{ size: '70%', content:'splitter content', collapsible : true }] });
        splitterObj.appendTo(document.getElementById('default'));
        });
        afterAll((): void => {
        document.body.innerHTML = '';
        ...
        it('check paneSettings', function() {
         expect(splitterObj.paneSettings[0].collapsed).toBe(false);
         splitterObj.allBars[0].firstElementChild.click();
         expect(splitterObj.paneSettings[0].collapsed).toBe(true);
         splitterObj.allBars[0].lastElementChild.click();
         expect(splitterObj.paneSettings[0].collapsed).toBe(false);
         splitterObj.collapse(0);
         expect(splitterObj.paneSettings[0].collapsed).toBe(true);
    ...
    describe('splitter other pane sizes not updated when update size dynamically', () => {
        ...
        it('Check flexible pane size -Pixel -Two Flexible pane', function () {
            splitterObj = new Splitter({ width: '500px', paneSettings: [{ size: '100px', content: 'First Pane', collapsible: true }, { content: 'Middle Pane' }, { content: 'Last Pane' }] });
            splitterObj.appendTo(document.getElementById('default'));
        ...
        it('Check flexible pane size -Pixel -one flexible pane', function () {
            splitterObj = new Splitter({ width: '500px', paneSettings: [{ size: '100px', content: 'First Pane', collapsible: true }, { size: '100px', content: 'Middle Pane' }, { content: 'Last Pane' }] });
            splitterObj.appendTo(document.getElementById('default'));
            expect(splitterObj.allPanes[0].style.flexBasis).toBe('100px');
        ...
        it('Check flexible pane size -Percentage - Two flexible pane', function () {
            splitterObj = new Splitter({ width: '500px', paneSettings: [{ size: '30%', content: 'First Pane', collapsible: true }, { content: 'Middle Pane' }, { content: 'Last Pane' }] });
            splitterObj.appendTo(document.getElementById('default'));
        ...
        it('Check flexible pane size -Percentage - one flexible pane', function () {
            splitterObj = new Splitter({ width: '500px', paneSettings: [{ size: '30%', content: 'First Pane', collapsible: true }, { size: '20%', content: 'Middle Pane' }, { content: 'Last Pane' }] });
            splitterObj.appendTo(document.getElementById('default'));
            expect(splitterObj.allPanes[0].style.flexBasis).toBe('30%');
    ...
    describe('Null or undefined value testing', () => {
        ...
        it('cssClass', () => {
            splitterObj = new Splitter({ cssClass: null },'#splitter');
            expect(splitterObj.cssClass).toBe(null);
            splitterObj.destroy();
            splitterObj = new Splitter({ cssClass: undefined },'#splitter');
            expect(splitterObj.cssClass).toBe('');
            splitterObj.destroy();
        });
        it('enableHtmlSanitizer', () => {
            splitterObj = new Splitter({ enableHtmlSanitizer: null },'#splitter');
            expect(splitterObj.enableHtmlSanitizer).toBe(null);
            splitterObj.destroy();
            splitterObj = new Splitter({ enableHtmlSanitizer: undefined },'#splitter');
            expect(splitterObj.enableHtmlSanitizer).toBe(true);
            splitterObj.destroy();
        });
        it('enablePersistence', () => {
            splitterObj = new Splitter({ enablePersistence: null },'#splitter');
            expect(splitterObj.enablePersistence).toBe(null);
            splitterObj.destroy();
            splitterObj = new Splitter({ enablePersistence: undefined },'#splitter');
            expect(splitterObj.enablePersistence).toBe(false);
            splitterObj.destroy();
        });
        it('enableReversePanes', () => {
            splitterObj = new Splitter({ enableReversePanes: null },'#splitter');
            expect(splitterObj.enableReversePanes).toBe(null);
            splitterObj.destroy();
            splitterObj = new Splitter({ enableReversePanes: undefined },'#splitter');
            expect(splitterObj.enableReversePanes).toBe(false);
            splitterObj.destroy();
        });
        it('enableRtl', () => {
            splitterObj = new Splitter({ enableRtl: null },'#splitter');
            expect(splitterObj.enableRtl).toBe(false);
            splitterObj.destroy();
            splitterObj = new Splitter({ enableRtl: undefined },'#splitter');
            expect(splitterObj.enableRtl).toBe(false);
            splitterObj.destroy();
        });
        it('enabled', () => {
            splitterObj = new Splitter({ enabled: null },'#splitter');
            expect(splitterObj.enabled).toBe(null);
            splitterObj.destroy();
            splitterObj = new Splitter({ enabled: undefined },'#splitter');
            expect(splitterObj.enabled).toBe(true);
            splitterObj.destroy();
        });
        it('height', () => {
            splitterObj = new Splitter({ height: null },'#splitter');
            expect(splitterObj.height).toBe(null);
            splitterObj.destroy();
            splitterObj = new Splitter({ height: undefined },'#splitter');
            expect(splitterObj.height).toBe('100%');
            splitterObj.destroy();
        });
        it('locale', () => {
            splitterObj = new Splitter({ locale: null },'#splitter');
            expect(splitterObj.locale).toBe('en-US');
            splitterObj.destroy();
            splitterObj = new Splitter({ locale: undefined },'#splitter');
            expect(splitterObj.locale).toBe('en-US');
            splitterObj.destroy();
        });
        it('orientation', () => {
            splitterObj = new Splitter({ orientation: null },'#splitter');
            expect(splitterObj.orientation).toBe(null);
            splitterObj.destroy();
            splitterObj = new Splitter({ orientation: undefined },'#splitter');
            expect(splitterObj.orientation).toBe('Horizontal');
            splitterObj.destroy();
        });
        it('separatorSize', () => {
            splitterObj = new Splitter({ separatorSize: null },'#splitter');
            expect(splitterObj.separatorSize).toBe(null);
            splitterObj.destroy();
            splitterObj = new Splitter({ separatorSize: undefined },'#splitter');
            expect(splitterObj.separatorSize).toBe(null);
            splitterObj.destroy();
        });
        it('width', () => {
            splitterObj = new Splitter({ width: null },'#splitter');
            expect(splitterObj.width).toBe(null);
            splitterObj.destroy();
            splitterObj = new Splitter({ width: undefined },'#splitter');
            expect(splitterObj.width).toBe('100%');
            splitterObj.destroy();
        });
    });
