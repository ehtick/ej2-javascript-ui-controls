...
export class Splitter extends Component<HTMLElement> {
    ...
    private createSeparator(i: number): HTMLElement {
        ...
        (separator as any).delegateFocusHandler = (): void => {
            separator.classList.add(SPLIT_BAR_ACTIVE);
            this.currentSeparator = separator;
            this.getPaneDetails();
        };
        (separator as any).delegateBlurHandler = (): void => {
            separator.classList.remove(SPLIT_BAR_ACTIVE);
        };
        separator.addEventListener('focus', (separator as any).delegateFocusHandler);
        separator.addEventListener('blur', (separator as any).delegateBlurHandler);
        return separator;
    }
    ...
    private addMouseActions(separator: HTMLElement): void {
        ...
        (separator as any).delegateMouseEnterHandler = (): void => {
            /* istanbul ignore next */
            sTout = setTimeout((): void => {
                addClass([separator], [SPLIT_BAR_HOVER]);
            }, this.iconsDelay);
        };
        (separator as any).delegateMouseLeaveHandler = (): void => {
            clearTimeout(sTout);
            removeClass([separator], [SPLIT_BAR_HOVER]);
        };
        (separator as any).delegateMouseOutHandler = (): void => {
            clearTimeout(hoverTimeOut);
        };
        (separator as any).delegateMouseOverHandler = (): void => {
            /* istanbul ignore next */
            hoverTimeOut = setTimeout((): void => {
                addClass([separator], [SPLIT_BAR_HOVER]);
            });
        };
        separator.addEventListener('mouseenter', (separator as any).delegateMouseEnterHandler);
        separator.addEventListener('mouseleave', (separator as any).delegateMouseLeaveHandler);
        separator.addEventListener('mouseout', (separator as any).delegateMouseOutHandler);
        separator.addEventListener('mouseover', (separator as any).delegateMouseOverHandler);
    }
        ...
        if (!this.isDestroyed) {
            ...
            if (separators.length > 0) {
                for (let i: number = 0; i < separators.length; i++) {
                    ...
                    if ((separators[i as number] as any).delegateFocusHandler) {
                        separators[i as number].removeEventListener('focus', (separators[i as number] as any).delegateFocusHandler);
                        (separators[i as number] as any).delegateFocusHandler = null;
                    }
                    if ((separators[i as number] as any).delegateBlurHandler) {
                        separators[i as number].removeEventListener('blur', (separators[i as number] as any).delegateBlurHandler);
                        (separators[i as number] as any).delegateBlurHandler = null;
                    }
                    if ((separators[i as number] as any).delegateMouseEnterHandler) {
                        separators[i as number].removeEventListener('mouseenter', (separators[i as number] as any).delegateMouseEnterHandler);
                        (separators[i as number] as any).delegateMouseEnterHandler = null;
                    }
                    if ((separators[i as number] as any).delegateMouseLeaveHandler) {
                        separators[i as number].removeEventListener('mouseleave', (separators[i as number] as any).delegateMouseLeaveHandler);
                        (separators[i as number] as any).delegateMouseLeaveHandler = null;
                    }
                    if ((separators[i as number] as any).delegateMouseOutHandler) {
                        separators[i as number].removeEventListener('mouseout', (separators[i as number] as any).delegateMouseOutHandler);
                        (separators[i as number] as any).delegateMouseOutHandler = null;
                    }
                    if ((separators[i as number] as any).delegateMouseOverHandler) {
                        separators[i as number].removeEventListener('mouseover', (separators[i as number] as any).delegateMouseOverHandler);
                        (separators[i as number] as any).delegateMouseOverHandler = null;
                    }
