...
export class PivotFieldList extends Component<HTMLElement> implements INotifyPropertyChanged {
    ...
    public frameCustomProperties(fieldListData?: IOlapField[], fieldList?: IOlapFieldListOptions)
        ...
        const isTabular: boolean = this.pivotGridModule ? this.pivotGridModule.isTabular : false;
        ...
        for (const prop of Object.keys(newProp)) {
            ...
            case 'dataSourceSettings':
                ...
                if (newProp.dataSourceSettings && Object.keys(newProp.dataSourceSettings).length === 1
                    ...
                    const control: PivotView = !this.isPopupView ? this.staticPivotGridModule : this.pivotGridModule;
                    if (control) {
        ...
        control.trigger(events.enginePopulating, args, (observedArgs: EnginePopulatingEventArgs) => {
            ...
            if (this.dataType === 'pivot') {
                if (this.dataSourceSettings.groupSettings && this.dataSourceSettings.groupSettings.length > 0) {
                    const pivotDataSet: IDataSet[] | string[][] = this.engineModule.data as IDataSet[] | string[][];
    ...
    private generateData(): void {
        ...
        if (this.dataSourceSettings && (this.dataSourceSettings.dataSource || this.dataSourceSettings.url)) {
            if ((this.dataSourceSettings.url !== '' && this.dataType === 'olap') ||
                (!isNullOrUndefined(this.dataSourceSettings.dataSource) && (this.dataSourceSettings.dataSource as IDataSet[]).length > 0)) {
                if (this.dataType === 'pivot') {
