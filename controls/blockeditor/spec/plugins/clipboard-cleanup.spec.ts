...
import { ClipboardCleanupModule } from '../../src/block-manager/plugins/common/clipboard-cleanup';
import { createEditor } from '../common/util.spec';
import { BlockType, ContentType, CommandName } from '../../src/models/enums';
import { BlockEditor, BlockModel } from '../../src/index';
...
describe('Clipboard Cleanup Module', () => {
    ...
    beforeEach(() => {
        ...
        cleanupModule = new ClipboardCleanupModule(editor.blockManager);
    });
    ...
    describe('cleanupPaste method', () => {
        it('should handle plain text formatting when isPlainText is true', () => {
            ...
            editor.pasteCleanupSettings.plainText = true;
        ...
        it('should strip all formatting when keepFormat is false', () => {
            ...
            editor.pasteCleanupSettings.keepFormat = false;
            const result = cleanupModule.cleanupPaste({
                html: html
            });
    ...
    describe('HTML cleanup functionality', () => {
        ...
        it('should handle denied tags', () => {
            ...
            editor.pasteCleanupSettings.deniedTags = ['font', 'marquee'];
            const result = cleanupModule.cleanupPaste({
                html: html
            });
        ...
        it('should handle denied attributes', () => {
            ...
            editor.pasteCleanupSettings.deniedTags = ['data-custom', 'target'];
        ...
        it('should handle allowed styles', () => {
            ...
            editor.pasteCleanupSettings.allowedStyles = ['color', 'background-color'];
    ...
    // describe('Clipboard Performance - Copy 100 mixed blocks, create new para with Enter, then Paste (sync timing)', () => {
    //     let editor: BlockEditor;
    //     let editorElement: HTMLElement;

    //     function createMixedTopLevelBlocks(count: number): BlockModel[] {
    //         const blocks: BlockModel[] = [];
    //         for (let i = 0; i < count; i++) {
    //             const t = i % 8; // cycle through 8 mixed types
    //             switch (t) {
    //                 case 0:
    //                     blocks.push({ id: `p-${i}`, blockType: BlockType.Paragraph, content: [{ id: `p-c-${i}`, contentType: ContentType.Text, content: `Paragraph ${i}` }] });
    //                     break;
    //                 case 1:
    //                     blocks.push({ id: `h-${i}`, blockType: BlockType.Heading, properties: { level: (i % 3) + 1 }, content: [{ id: `h-c-${i}`, contentType: ContentType.Text, content: `Heading ${i}` }] });
    //                     break;
    //                 case 2:
    //                     blocks.push({ id: `bu-${i}`, blockType: BlockType.BulletList, content: [{ id: `bu-c-${i}`, contentType: ContentType.Text, content: `Bullet ${i}` }] });
    //                     break;
    //                 case 3:
    //                     blocks.push({ id: `nl-${i}`, blockType: BlockType.NumberedList, content: [{ id: `nl-c-${i}`, contentType: ContentType.Text, content: `Numbered ${i}` }] });
    //                     break;
    //                 case 4:
    //                     blocks.push({ id: `code-${i}`, blockType: BlockType.Code, properties: { language: (i % 2 === 0 ? 'javascript' : 'typescript') }, content: [{ id: `code-c-${i}`, contentType: ContentType.Text, content: `const v${i} = ${i};` }] });
    //                     break;
    //                 case 5:
    //                     blocks.push({ id: `q-${i}`, blockType: BlockType.Quote, content: [{ id: `q-c-${i}`, contentType: ContentType.Text, content: `Quote ${i}` }] });
    //                     break;
    //                 case 6:
    //                     blocks.push({
    //                         id: `callout-${i}`,
    ...
    //                                 id: `callout-child-${i}`,
    //                                 blockType: BlockType.Paragraph,
    //                                 content: [{ id: `callout-child-c-${i}`, contentType: ContentType.Text, content: `Callout child ${i}` }]
    //                             }]
    //                         }
    //                     });
    //                     break;
    ...
    //                         content: [{ id: `cp-c-${i}`, contentType: ContentType.Text, content: `Collapsible ${i}` }],
    //                         properties: {
    //                             isExpanded: (i % 2 === 0),
    //                             children: [{
    //                                 id: `cp-child-${i}`,
    //                                 blockType: BlockType.Paragraph,
    //                                 content: [{ id: `cp-child-c-${i}`, contentType: ContentType.Text, content: `CP child ${i}` }]
    //                             }]
    //                         }
    //                     });
    //                     break;
    ...
    //     beforeEach(() => {
    //         editorElement = createElement('div', { id: 'editor-copy-mixed-enter-then-paste-sync' });
    //         document.body.appendChild(editorElement);

    //         const blocks = createMixedTopLevelBlocks(100);
    //         editor = createEditor({ blocks });
    //         editor.appendTo('#editor-copy-mixed-enter-then-paste-sync');
    ...
    //     it('should copy 100 mixed blocks, create a new paragraph via Enter after last block, set cursor, and paste once (sync timing)', (done) => {
    //         const initialTopLevelCount = editor.blocks.length;
    //         expect(initialTopLevelCount).toBe(100);

    //         const firstBlockEl = editor.element.querySelector(`#${editor.blocks[0].id}`) as HTMLElement;
    //         editor.blockManager.setFocusToBlock(firstBlockEl);
    ...
    //             getData: (format: string) => (format === 'text/blockeditor' ? copiedData : '')
    //         };
    ...
    //         const lastBlockEl = editor.element.querySelector(`#${lastBlockId}`) as HTMLElement;
    //         const lastContentEl = getBlockContentElement(lastBlockEl);
    //         editor.blockManager.setFocusToBlock(lastBlockEl);
    //         setCursorPosition(lastContentEl, lastContentEl.textContent.length);
    //         editorElement.dispatchEvent(new KeyboardEvent('keydown', { key: 'Enter', bubbles: true }));
    //         editorElement.dispatchEvent(new KeyboardEvent('keydown', { key: 'Enter', bubbles: true }));

    //         setTimeout(() => {
    //             expect(editor.blocks.length).toBe(initialTopLevelCount + 1);
    //             const newLastBlock = editor.blocks[editor.blocks.length - 1];
    //             expect(newLastBlock.blockType).toBe(BlockType.Paragraph);

    //             const newLastBlockEl = editor.element.querySelector(`#${newLastBlock.id}`) as HTMLElement;
    //             const newLastContentEl = getBlockContentElement(newLastBlockEl);
    //             editor.blockManager.setFocusToBlock(newLastBlockEl);
    //             setCursorPosition(newLastContentEl, 0);
    //             measurePerformanceSync('Synchronous handlePaste() for 100 mixed blocks (paste into new paragraph)', () => {
    //                 editor.blockManager.clipboardAction.handlePaste(createMockClipboardEvent('paste', mockClipboard));
    //             }, 1000); 

    //             setTimeout(() => {
    //                 expect(editor.blocks.length).toBe(initialTopLevelCount + 100); 

    //                 const afterPrevLast = editor.blocks[editor.blocks.length - 100]; 
    //                 const afterPrevLastEl = editor.element.querySelector(`#${afterPrevLast.id}`);
    //                 expect(afterPrevLastEl).not.toBeNull();

    //                 const domBlocks = editorElement.querySelectorAll('.e-block');
    //                 expect(domBlocks.length).toBeGreaterThan(initialTopLevelCount);
    //                 done();
    //             }, 250);
    //         }, 50);
    //     }, 3000); 
