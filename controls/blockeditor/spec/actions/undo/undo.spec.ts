...
import { BaseChildrenProp, BaseStylesProp, BlockModel, ICalloutBlockSettings, IChecklistBlockSettings, ICollapsibleBlockSettings, IHeadingBlockSettings, IImageBlockSettings, ILinkContentSettings, IQuoteBlockSettings, UserModel } from '../../../src/models/index';
...
describe('UndoRedo', () => {
    ...
    describe('ContentChanged', () => {
        ...
        it('Undo & Redo action for paragraph content changed', (done) => {
            ...
            const paragraph = domBlocks[0].querySelector('p');
            paragraph.textContent = 'Updated content';
            editor.blockManager.stateManager.updateContentOnUserTyping(domBlocks[0]);
        ...
        it('Undo action when stack length is 0', (done) => {
            ...
            const paragraph = domBlocks[0].querySelector('p');
            setTimeout(() => {
                editor.blockManager.setFocusToBlock(domBlocks[0]);
                const undoEvent: KeyboardEvent = new KeyboardEvent('keydown', { key: 'z', ctrlKey: true, code: 'KeyZ' });
                editorElement.dispatchEvent(undoEvent);
                expect(paragraph.textContent).toBe('Block 1 content');
    ...
    describe('Single-Block Formatting Action', () => {
            ...
            const blocks: BlockModel[] = [
                { id: 'block1', blockType: BlockType.Paragraph, content: [{ contentType: ContentType.Text, content: 'Block 1 content' }] },
                {
                    id: 'callout', blockType: BlockType.Callout, properties: {
                        children: [
                            {
                                id: 'callout-block-1', blockType: BlockType.Paragraph, content: [{ contentType: ContentType.Text, content: 'Callout item 1' }],
                            }
                        ]
                    }
                }
        ...
        it('Redo action for paragraph content formatted to bold', (done) => {
            setTimeout(() => {
                ...
                expect(contentElement.childElementCount).toBe(1);
                // Check DOM structure: strong -> textnode
                let nodes = contentElement.childNodes;
                expect(nodes[0].nodeName).toBe('STRONG');
                expect(nodes[1].nodeType).toBe(Node.TEXT_NODE);
                expect(nodes[1].textContent).toBe('1 content');
                expect((editor.blocks[0].content[0].properties as BaseStylesProp).styles.bold).toBe(true);
        ...
        it('Undo action for content formatted to bold in callout', (done) => {
            setTimeout(() => {
                ...
                expect(nodes[0].nodeName).toBe('STRONG');
                expect(nodes[1].nodeType).toBe(Node.TEXT_NODE);
                expect(nodes[1].textContent).toBe('item 1');
    ...
    describe('Single and Multi-Block Addition', () => {
        ...
        it('Undo action for single block addition', (done) => {
            ...
            const newBlock: BlockModel = {
                ...
                blockType: BlockType.Paragraph,
                content: [
                    { contentType: ContentType.Text, content: 'Block 3 content' }
                ]
            };
        ...
        it('Undo action for multiple block addition', (done) => {
            ...
            setTimeout(() => {
                ...
                const newBlock2: BlockModel = {
                    ...
                    blockType: BlockType.Paragraph,
                    content: [
                        { contentType: ContentType.Text, content: 'Block 4 content' }
                    ]
                };
    ...
    describe('Single & Multi-Block Clipboard actions', () => {
        ...
        it('copy & paste whole block - UNDO & REDO', (done) => {
            ...
            editor.blockManager.clipboardAction.handlePaste(createMockClipboardEvent('paste', mockClipboard));
                ...
                expect(domBlocks.length).toBe(3);
                expect(domBlocks[1].textContent).toBe('First paragraph');
                expect(editorElement.querySelector('#paragraph1').nextElementSibling.id).toBe(editor.blocks[1].id);
                done();
        });
        ...
        it('copy & paste partial content', (done) => {
            ...
            var startNode = editor.element.querySelector('em').firstChild;
            var endNode = editor.element.querySelector('u').firstChild;
    ...
    describe('Selective Deletion of blocks', () => {
        ...
        it('Entire editor deletion using backspace', (done) => {
            ...
            expect(modelBlocks[3].blockType).toBe(BlockType.Paragraph);

            expect(domBlocks.length).toBe(4);
            expect(domBlocks[0].querySelector('p')).not.toBeNull();
            expect(domBlocks[1].querySelector('h3')).not.toBeNull();
            expect(domBlocks[2].querySelector('ul')).not.toBeNull();
            expect(domBlocks[3].querySelector('p')).not.toBeNull();
    ...
    describe('Entire Deletion of blocks when first block is toggle type', () => {
            ...
            const blocks: BlockModel[] = [
                ...
                },
                {
                    ...
                    properties: {
                        children: [
                            {
                                id: 'quote-block-1', blockType: BlockType.Paragraph,
                                content: [{ id: 'q-content', contentType: ContentType.Text, content: 'Quote' }],
                            },
                            {
                                id: 'quote-block-2', blockType: BlockType.Paragraph,
                                content: [{ id: 'strike-content', contentType: ContentType.Text, content: 'content', properties: { styles: { strikethrough: true } } }],
                            }]
                    }
                }
            ];
        ...
        it('Entire editor deletion using backspace', (done) => {
            ...
            expect(domBlocks.length).toBe(7);
    ...
    describe('Undo/Redo for Character and Word Deletion', () => {
        ...
        beforeEach(() => {
            ...
            const blocks: BlockModel[] = [{
                id: 'paragraph',
                blockType: BlockType.Paragraph,
                content: [{ contentType: ContentType.Text, content: initialContent }]
            }];
            editor = createEditor({ blocks });
            editor.appendTo('#editor');
        });
    ...
    describe('Undo/Redo for Inline Style Formatting', () => {
        ...
        it('should UNDO and REDO bold on overlapping text', (done) => {
            const blocks: BlockModel[] = [{
                ...
                blockType: BlockType.Paragraph,
                content: [
                    { contentType: ContentType.Text, content: 'Part one, ' },
                ]
            }];
            editor = createEditor({ blocks });
            editor.appendTo('#editor');
    ...
    describe('Undo/Redo for Multiple Formatting on a Single Paragraph', () => {
        ...
        it('should undo & redo all styles applied sequentially to a single paragraph selection', (done) => {
            ...
            domBlocks = editor.element.querySelectorAll<HTMLElement>('.e-block');
            const contentEle = getBlockContentElement(domBlocks[0]);
            ...
            expect(contentEle.querySelector('span')).not.toBeNull();
            expect(contentEle.querySelector('span').style.color).toBe('rgb(229, 57, 53)');
            ...
            expect(styles.strikethrough).toBe(true);
            expect(styles.color).toBe('#E53935');
            const contentEleAfterRedo = getBlockContentElement(domBlocks[0]);
            expect(contentEleAfterRedo).not.toBeNull();
            expect(contentEleAfterRedo.querySelector('em')).not.toBeNull();
            expect(contentEleAfterRedo.querySelector('u')).not.toBeNull();
            expect(contentEleAfterRedo.querySelector('s')).not.toBeNull();
            expect(contentEleAfterRedo.querySelector('span')).not.toBeNull();
            expect(contentEleAfterRedo.querySelector('span').style.color).toBe('rgb(229, 57, 53)');
            done();
        });
    });
    ...
    describe('Undo/Redo for Mention and Label Insertion', () => {
        ...
        // Bug
        // it('should UNDO and REDO a mention insertion on empty block', (done) => {
        //     const blockElement = editorElement.querySelector('#paragraph') as HTMLElement;
        //     editor.blockManager.setFocusToBlock(blockElement);
        //     const contentElement = getBlockContentElement(blockElement);
        //     contentElement.textContent = '@';
        ...
        //     expect(contentElement.querySelector('.e-mention-chip').nextSibling).not.toBeNull();
        ...
        // Bug
        // it('should UNDO and REDO a label insertion on empty block', (done) => {
        //     const blockElement = editor.element.querySelector('#paragraph') as HTMLElement;
        ...
        //     expect(contentElement.querySelector('.e-label-chip').nextSibling).not.toBeNull();
        ...
        // Bug
        // it('should UNDO and REDO multiple mention insertion on empty block', function (done) {
        //     const blockElement = editorElement.querySelector('#paragraph') as HTMLElement;
        //     editor.blockManager.setFocusToBlock(blockElement);
        //     const contentElement = getBlockContentElement(blockElement);
        //     contentElement.textContent = '@';
        ...
        //     contentElement.childNodes[0].textContent = '@';
        ...
        // Bug
        // it('should place caret at end after UNDO of a mention inserted after existing text', (done) => {
        //     const blockElement = editorElement.querySelector('#paragraph') as HTMLElement;
        //     const contentElement = getBlockContentElement(blockElement) as HTMLElement;
        //     editor.blockManager.setFocusToBlock(blockElement);
        ...
        //     triggerUndo(editorElement);
        //     expect(contentElement.querySelector('.e-mention-chip')).toBeNull();
        //     expect(contentElement.textContent).toBe('Initial text ');
        //     const selection = window.getSelection();
        //     expect(selection.rangeCount).toBeGreaterThan(0);
        //     const range = selection.getRangeAt(0);
        //     expect(range.collapsed).toBe(true);
        //     expect(range.startContainer.nodeType).toBe(Node.TEXT_NODE);
        //     expect(range.startOffset).toBe((range.startContainer as Text).data.length);
        //     expect((range.startContainer as Text).data.length).toBe(contentElement.textContent.length);
        //     done();
        // });

        // Bug
        // it('should place caret at end after UNDO of a label inserted after existing text', (done) => {
        //     const blockElement = editorElement.querySelector('#paragraph') as HTMLElement;
        //     const contentElement = getBlockContentElement(blockElement) as HTMLElement;
        //     editor.blockManager.setFocusToBlock(blockElement);
        //     const initialText = contentElement.textContent;
                    ...
                    children: [
                        {
                            ...
                            blockType: BlockType.Paragraph, 
                            content: [{ contentType: ContentType.Text, content: 'Callout item 2' }]
                        }
                    ]
                }
            };
    ...
    //     it('should UNDO and REDO adding a Quote block', (done) => {
    //         const initialBlockCount = editor.blocks.length;
    //         expect(initialBlockCount).toBe(1);
    //         const newQouteBlock: BlockModel = {
    //             id: 'quote-1',
    //             blockType: BlockType.Quote,
    //             properties: {
    //                 children: [
    ...
    //                         id: 'quote-child-para', 
    //                         blockType: BlockType.Paragraph, 
    //                         content: [{ contentType: ContentType.Text, content: 'Quote item 2' }]
    //                     }
    //                 ]
    //             }
    //         };
    //         editor.blockManager.editorMethods.addBlock(newQouteBlock, 'block1', true);
    //         expect(editor.blocks.length).toBe(initialBlockCount + 1);
    //         expect(editor.blocks[1].id).toBe('quote-1');
    //         expect(editor.blocks[1].blockType).toBe(BlockType.Quote);
    //         const quoteElement = document.getElementById('quote-1');
    //         expect(quoteElement).not.toBeNull('The Quote DOM element should be created');
                   
    //         triggerUndo(editorElement);
    //         expect(editor.blocks.length).toBe(initialBlockCount);
    //         expect(document.getElementById('quote-1')).toBeNull();
        
    //         triggerRedo(editorElement);
    //         expect(editor.blocks.length).toBe(initialBlockCount + 1);
    //         expect(editor.blocks[1].id).toBe('quote-1');
    //         const restoredQuoteElement = document.getElementById('quote-1');
    //         expect(restoredQuoteElement).not.toBeNull();
    //         done();
    //     });
    ...
    // describe('Undo/Redo for Operations within a Quote Block', () => {
    //     beforeEach(() => {
    //         editorElement = createElement('div', { id: 'editor' });
    //         document.body.appendChild(editorElement);
    //         const blocks: BlockModel[] = [{
    ...
    //             properties: {
    //                 children: [
    //                     { id: 'child-para-1', blockType: BlockType.Paragraph, content: [{ contentType: ContentType.Text, content: 'Initial child text.' }] },
    //                     { id: 'child-para-2', blockType: BlockType.Paragraph, content: [{ contentType: ContentType.Text, content: 'Second child text.' }] },
    //                 ]
    //             }
    //         }];
    //         editor = createEditor({ blocks });
    ...
    //     it('should UNDO and REDO typing text in a Quote’s child Paragraph', (done) => {
    ...
    //         const quoteChildren = (editor.blocks[0].properties as IQuoteBlockSettings).children;
    //         expect(quoteChildren[0].content[0].content).toBe(newText);
    //         triggerUndo(editorElement);
    //         expect(quoteChildren[0].content[0].content).toBe(initialText);
    //         expect(contentElement.textContent).toBe(initialText);
    //         triggerRedo(editorElement);
    //         expect(quoteChildren[0].content[0].content).toBe(newText);
    ...
    //     it('should UNDO and REDO adding a child Paragraph to a Quote', (done) => {
    ...
    //         editorElement.dispatchEvent(new KeyboardEvent('keydown', { key: 'Enter', bubbles: true }));
    //         const quoteChildren = (editor.blocks[0].properties as IQuoteBlockSettings).children;
    //         expect(quoteChildren.length).toBe(3);
    //         triggerUndo(editorElement);
    ...
    //     it('should UNDO and REDO deleting a Quote’s child Paragraph', (done) => {
    //         const lastChildBlock = editorElement.querySelector('#child-para-2') as HTMLElement;
    //         editor.blockManager.setFocusToBlock(lastChildBlock);
    ...
    //         triggerUndo(editorElement);
    //         expect(quoteChildren.length).toBe(2);
    //         expect(quoteChildren[1].id).toBe('child-para-2');
    //         expect(editor.element.querySelector('#quote-block #child-para-2')).not.toBeNull();
    //         expect(editor.element.querySelector('#quote-block #child-para-2').textContent).toBe('Second child text.');
    //         triggerRedo(editorElement);
    //         expect(quoteChildren.length).toBe(1);
    //         expect(editor.element.querySelector('#quote-block #child-para-2')).toBeNull();
    //         done();
    //     });
    // });
    ...
    describe('Undo/Redo for Operations within a CollapsibleParagraph Block', () => {
        beforeEach(() => {
            ...
            const blocks: BlockModel[] = [{
                ...
                properties: {
                    isExpanded: false,
                    children: [{ id: 'cp-child-1', blockType: BlockType.Paragraph, content: [{ contentType: ContentType.Text, content: 'Child content' }] },
                { id: 'cp-child-2', blockType: BlockType.Paragraph, content: [{ contentType: ContentType.Text, content: 'Child2 content' }] }]
                }
            }];
            editor = createEditor({ blocks });
            editor.appendTo('#editor');
    ...
    describe('Undo/Redo for Operations within a CollapsibleHeading Block', () => {
        beforeEach(() => {
            ...
            const blocks: BlockModel[] = [{
                ...
                blockType: BlockType.CollapsibleHeading,
                content: [{ contentType: ContentType.Text, content: 'Heading Title' }],
                properties: {
                    level: 1,
                    isExpanded: false,
                    children: [{ id: 'ch-child-1', blockType: BlockType.Paragraph, content: [{ contentType: ContentType.Text, content: 'Child content' }] },
                    { id: 'ch-child-2', blockType: BlockType.Paragraph, content: [{ contentType: ContentType.Text, content: 'Child2 content' }] }]
    ...
    describe('Undo/Redo for Enter key on quote', () => {
        beforeEach(() => {
            editorElement = createElement('div', { id: 'editor' });
            document.body.appendChild(editorElement);
            const blocks: BlockModel[] = [{
                ...
                properties: {
                    children: [
                        { id: 'child-para-1', blockType: BlockType.Paragraph, content: [{ contentType: ContentType.Text, content: '' }] },
        ...
        it('should UNDO and REDO on Enter key action on empty child Paragraph of a Quote', (done) => {
            ...
            const quoteChildren = (editor.blocks[0].properties as IQuoteBlockSettings).children;
            expect(quoteChildren.length).toBe(2);
            const quotetBlock = editorElement.querySelector('.e-block[data-block-type="Quote"]') as HTMLElement;
            const childBlocks = quotetBlock.querySelectorAll('.e-block');
            ...
            triggerUndo(editorElement);
            expect(quoteChildren.length).toBe(1);
            const quoteBlockAfterUndo = editorElement.querySelector('.e-block[data-block-type="Quote"]') as HTMLElement;
            const childBlocksAfterUndo = quoteBlockAfterUndo.querySelectorAll('.e-block');   
            expect(childBlocksAfterUndo.length).toBe(1); // DOM should reflect removal of the new child

            triggerRedo(editorElement);
            expect(quoteChildren.length).toBe(2);
            const quoteBlockAfterRedo = editorElement.querySelector('.e-block[data-block-type="Quote"]') as HTMLElement;
            const childBlocksAfterRedo = quoteBlockAfterRedo.querySelectorAll('.e-block');
    ...
    describe('Undo/Redo for Enter key on Collapsible Paragraph', () => {
        beforeEach(() => {
            ...
            const blocks: BlockModel[] = [{
                ...
                properties: {
                    isExpanded: false,
                    children: [{ id: 'cp-child-1', blockType: BlockType.Paragraph, content: [{ contentType: ContentType.Text, content: '' }] }]
                }
            }];
            editor = createEditor({ blocks });
            editor.appendTo('#editor');
        ...
        beforeEach(() => {
            ...
            const blocks: BlockModel[] = [{
                ...
                properties: {
                    level: 1,
                    isExpanded: false,
                    children: [{ id: 'ch-child-1', blockType: BlockType.Paragraph, content: [{ contentType: ContentType.Text, content: '' }] }]
                },
            }];
            editor = createEditor({ blocks });
            editor.appendTo('#editor');
    ...
    describe('Undo/Redo for Deleting a Quote Block', () => {
        const quoteBlockId = 'quote-block-to-delete';
        beforeEach(() => {
            editorElement = createElement('div', { id: 'editor' });
            document.body.appendChild(editorElement);
            // Initialize with a complex Quote block
            const blocks: BlockModel[] = [{
                id: quoteBlockId,
                blockType: BlockType.Quote,
                properties: {
                    children: [
                        { id: 'child-h1', blockType: BlockType.Heading, properties: { level: 1 }, content: [{ contentType: ContentType.Text, content: 'Child Heading' }] },
                        { id: 'child-p1', blockType: BlockType.Paragraph, content: [{ contentType: ContentType.Text, content: 'Child paragraph.' }] }
                    ]
                }
            }];
            editor = createEditor({ blocks });
        ...
        it('should UNDO and REDO deleting an entire Quote block', (done) => {
            const initialChildrenLength = (editor.blocks[0].properties as BaseChildrenProp).children.length;
            expect(editor.blocks.length).toBe(1);
            editor.blockManager.execCommand({ command: 'DeleteBlock', state: { blockElement: editorElement.querySelector("#" + quoteBlockId) }});
            
            setTimeout(() => {
                expect(editor.blocks.length).toBe(1);
                expect(editor.blocks[0].blockType).toBe('Paragraph');
                const quoteBlockElement = editorElement.querySelector("#" + quoteBlockId);
                expect(quoteBlockElement).toBeNull();

                triggerUndo(editorElement);
                expect(editor.blocks.length).toBe(2);
                expect(editor.blocks[0].blockType).toBe('Quote');
                expect((editor.blocks[0].properties as BaseChildrenProp).children.length).toEqual(initialChildrenLength);

                triggerRedo(editorElement);
                expect(editor.blocks.length).toBe(1);
                expect(editor.blocks[0].blockType).toBe('Paragraph');
                const quoteBlockElement1 = editorElement.querySelector("#" + quoteBlockId);
                expect(quoteBlockElement1).toBeNull();
    ...
    describe('Undo/Redo for Deleting a CollapsibleParagraph Block', () => {
        ...
        beforeEach(() => {
            ...
            const blocks: BlockModel[] = [{
                ...
                blockType: BlockType.CollapsibleParagraph,
                content: [{ contentType: ContentType.Text, content: 'Main Content' }],
                properties: {
                    isExpanded: true,
                    children: [
                        { id: 'cp-child-1', blockType: BlockType.Paragraph, content: [{ contentType: ContentType.Text, content: 'Child Content' }] }
                    ]
                }
            }  ];
            editor = createEditor({ blocks });
    ...
    describe('Undo/Redo for Deleting a CollapsibleHeading Block', () => {
        ...
        beforeEach(() => {
            ...
            const blocks: BlockModel[] = [{
                ...
                blockType: BlockType.CollapsibleHeading,
                content: [{ contentType: ContentType.Text, content: 'Heading Content' }],
                properties: {
                    level: 2,
                    isExpanded: false,
                    children: [
                        { id: 'ch-child-1', blockType: BlockType.Paragraph, content: [{ contentType: ContentType.Text, content: 'Child Content' }] }
                    ]
                }
            }];
            editor = createEditor({ blocks });
    ...
    describe('Mention in a Callout child Paragraph', () => {
        ...
        // Bug
        // it('should UNDO and REDO applying a Mention on empty block', (done) => {
    ...
    describe('Label in a CollapsibleParagraph content', () => {
        beforeEach(() => {
            ...
            const blocks: BlockModel[] = [{
                id: 'collapsible-para',
                blockType: BlockType.CollapsibleParagraph,
                content: [{ contentType: ContentType.Text, content: 'Collapsible text ' }],
                properties: {
                    isExpanded: true,
                    children: []
                }
    ...
    describe('Link in a CollapsibleHeading content', () => {
        beforeEach(() => {
            ...
            const blocks: BlockModel[] = [{
                id: 'collapsible-head',
                blockType: BlockType.CollapsibleHeading,
                content: [{ contentType: ContentType.Text, content: 'Linkable heading text' }],
    ...
    describe('Undo/Redo for Multi-Style Formatting in a Quote Child', () => {
        const initialChildContent = 'This is the child content inside the quote.';
        beforeEach(() => {
            editorElement = createElement('div', { id: 'editor' });
            document.body.appendChild(editorElement);
            const blocks: BlockModel[] = [{
                ...
                properties: {
                    children: [{
                        id: 'child-paragraph',
                        blockType: BlockType.Paragraph,
                        content: [{ contentType: ContentType.Text, content: initialChildContent }]
                    }]
                }
            }];
            editor = createEditor({ blocks });
        ...
        it('should undo and redo all styles applied sequentially to a quote child', (done) => {
            ...
            let quotetChildren = (editor.blocks[0].properties as IQuoteBlockSettings).children;
            expect(quotetChildren[0].content.length).toBe(3);
            let styledPart = quotetChildren[0].content[1];
            let styles = (styledPart.properties as BaseStylesProp).styles;
            expect(styles.bold).toBe(true);
            expect(styles.italic).toBe(true);
            expect(styles.color).toBe('#007BFF');

            const contentEle = getBlockContentElement(childBlockElement);
            expect(contentEle).not.toBeNull();
            expect(contentEle.querySelector('strong')).not.toBeNull();
            expect(contentEle.querySelector('em')).not.toBeNull();
            expect(contentEle.querySelector('span')).not.toBeNull();
            expect(contentEle.querySelector('span').style.color).toBe('rgb(0, 123, 255)');

            triggerUndo(editorElement);
            triggerUndo(editorElement);
            triggerUndo(editorElement);

            const childrenAfterUndo = (editor.blocks[0].properties as IQuoteBlockSettings).children;
            ...
            expect(stylesAreEmpty).toBe(true);
            const contentEleAfterUndo = getBlockContentElement(childBlockElement);
            expect(contentEleAfterUndo).not.toBeNull();
            expect(contentEleAfterUndo.querySelector('strong')).toBeNull();
            expect(contentEleAfterUndo.querySelector('em')).toBeNull();
            expect(contentEleAfterUndo.querySelector('span')).toBeNull();

            triggerRedo(editorElement);
            triggerRedo(editorElement);
            triggerRedo(editorElement);

            const childrenAfterRedo = (editor.blocks[0].properties as IQuoteBlockSettings).children;
            ...
            const contentEleAfterRedo = getBlockContentElement(childBlockElement);
            expect(contentEleAfterRedo).not.toBeNull();
            expect(contentEleAfterRedo.querySelector('strong')).not.toBeNull();
            expect(contentEleAfterRedo.querySelector('em')).not.toBeNull();
            expect(contentEleAfterRedo.querySelector('span')).not.toBeNull();
            expect(contentEleAfterRedo.querySelector('span').style.color).toBe('rgb(0, 123, 255)');
            done();
        });
    });
    ...
    describe('Pasting into a Quote child', () => {
        ...
        it('should UNDO and REDO pasting a NumberedList', (done) => {
            ...
            setTimeout(() => {
                const quoteChildren = (editor.blocks[1].properties as IQuoteBlockSettings).children;
                // Pasting a block into a callout child adds as new block below.
                expect(quoteChildren.length).toBe(2);
                expect(quoteChildren[0].blockType).toBe(BlockType.Paragraph);
                expect(quoteChildren[1].blockType).toBe(BlockType.NumberedList);
                const quoteChildrenDOM = editorElement.querySelector('#quote-container').querySelectorAll('.e-block');
                expect(quoteChildrenDOM.length).toBe(2);
                expect(quoteChildrenDOM[0].textContent).toBe('Paste here');
                expect(quoteChildrenDOM[1].textContent).toBe('List Item 1');

                // UNDO
                triggerUndo(editorElement);
                const childrenAfterUndo = (editor.blocks[1].properties as IQuoteBlockSettings).children;
                ...
                const childrenAfterUndoDOM = editorElement.querySelector('#quote-container').querySelectorAll('.e-block');
                expect(childrenAfterUndoDOM.length).toBe(1);
                expect(childrenAfterUndoDOM[0].textContent).toBe('Paste here');

                // REDO
                triggerRedo(editorElement);
                const childrenAfterRedo = (editor.blocks[1].properties as IQuoteBlockSettings).children;
                ...
                const childrenAfterRedoDOM = editorElement.querySelector('#quote-container').querySelectorAll('.e-block');
