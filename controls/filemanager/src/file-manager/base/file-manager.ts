...
export class FileManager extends Component<HTMLElement> implements INotifyPropertyChanged {
    ...
    private onDragStart(event: MouseEvent): void {
        if (this.viewElem) {
            ...
            const contentElem: HTMLElement = this.viewElem.firstChild as HTMLElement;
            const isLargeIcons: boolean = this.viewElem.classList.contains('e-large-icons');
            if (isLargeIcons) {
                this.dragContainer = (contentElem && contentElem.tagName === 'UL') ? contentElem : this.viewElem;
            } else {
                this.dragContainer = this.viewElem.firstChild as HTMLElement;
            }
            if (!this.dragContainer) { return; }
            event.preventDefault();
            this.lastDragEvent = event;
            const ulRect: DOMRect = this.dragContainer.getBoundingClientRect() as DOMRect;
    ...
    private onDrag(event: MouseEvent): void {
        ...
        if (this.dragSelectElement && this.dragContainer) {
            const ulRect: DOMRect = this.dragContainer.getBoundingClientRect() as DOMRect;
            const currentX: number = event.clientX - ulRect.left - this.dragContainer.clientLeft + this.dragContainer.scrollLeft;
            const currentY: number = event.clientY - ulRect.top - this.dragContainer.clientTop + this.dragContainer.scrollTop;
            setStyleAttribute(this.dragSelectElement, {
    ...
    private onDragScroll(): void {
        if (!this.dragSelectElement || !this.lastDragEvent) { return; }
    ...
    public restoreFocus(): void {
        ...
        const targetItem: Element = this.largeiconsviewModule.getFocusedItem()
        || this.visitedItem || this.largeiconsviewModule.getFirstItem();
        if (targetItem && this.largeiconsviewModule) {
            this.largeiconsviewModule.addFocus(targetItem as Element);
        }
    }
}
