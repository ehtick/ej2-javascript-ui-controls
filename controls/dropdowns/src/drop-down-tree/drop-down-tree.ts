...
     */
    ...
    public render(): void {
        ...
        if (this.element.tagName === 'INPUT') {
            ...
            if (!isNOU(this.element.id)) {
                this.inputEle = this.createElement('input', { attrs: { role: 'textbox', type: 'text', id: this.element.id + '_textbox' } }) as HTMLInputElement;
            }
            else {
    ...
    private restoreFilterSelection(): void {
        ...
        const nodeIds: string[] = Array.isArray(this.value) ? this.value : [];
        if (this.showCheckBox) {
    ...
    public hidePopup(): void {
        ...
        if (this.popupObj && this.isPopupOpen) {
            ...
            if (this.isFilteredData) {
                (this.treeObj as any)['treeData'] = this.treeData;
            }
            if (this.destroyPopupOnHide) {
                this.isFirstRender = true;
                this.destroyPopup();
