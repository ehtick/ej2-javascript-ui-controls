...
export class MultiSelect extends DropDownBase implements IInput {
    ...
    private setPlaceholderSize(downIconWidth: number): void {
        if (isNullOrUndefined(this.value) || this.value.length === 0) {
            if (this.dropIcon.offsetWidth !== 0) {
                let totalIconWidth: number = downIconWidth + 10;
                if (this.showClearButton && this.overAllClear) {
                    ...
                    const clearWidth: number = this.overAllClear.offsetWidth;
    ...
    private focusInHandler(e?: FocusEvent | MouseEvent | KeyboardEvent | TouchEvent): boolean {
        if (this.enabled) {
            ...
            if (this.floatLabelType !== 'Never' && this.overAllWrapper) {
                const label: HTMLElement = this.overAllWrapper.querySelector('.e-float-text');
                if (label) {
            ...
            if (!isNullOrUndefined(this.value)) {
                for (let index: number = 0; !isNullOrUndefined(this.value[index as number]); index++) {
                    ...
                    if (((element && (element.getAttribute('aria-selected') !== 'true')) ||
                        ...
                        const indexItem: number = this.dataSource instanceof DataManager ? this.totalItemCount : this.listData.length;
                        ...
                        if (newValue && this.allowObjectBinding && this.allowCustomValue && this.value.length > 0 && typeof this.value[0] === 'object') {
                            const valueArray: any[] = this.value;
                            const filteredItems: any[] = valueArray.filter((valObj: any) => {
                                return (valObj as any)[this.fields.value] === value;
                            });
        ...
        if (state) {
            ...
            if (this.virtualSelectAllData instanceof Array) {
                ...
                if (length > 50) {
                    ...
                    setTimeout(
                        (): void => {
                            const dataArray: any[] = this.virtualSelectAllData as any[];
            ...
            if (!beforeSelectArgs.preventSelectEvent) {
                if (this.enableVirtualization) {
                    ...
                    if (length > 50) {
                        ...
                        const maxIndex: number = Math.min(length, count);
                        // Use two-frame yield to ensure spinner paints before heavy work starts
                        const CHUNK_SIZE: number = 6;
                        requestAnimationFrame(() => {
                            requestAnimationFrame(function processFrame(this: any): void {
                                if (!this.list) {
                                    return;
                                }
                                ...
                                while (processed < CHUNK_SIZE && indexLocal < maxIndex) {
                                    ...
                                    this.updateListSelection(li[indexLocal as number], event, length - indexLocal);
                                    if (this.enableGroupCheckBox) {
                                        this.findGroupStart(li[indexLocal as number]);
                                    }
                                    indexLocal++;
                                    processed++;
                                }
    ...
    private executeLocalForLargeSelection(
        ...
        const {
            ...
            allowObjectBinding,
            isPrimitiveData,
            predicateChunkSize = 100
        } = options;
        if (values.length === dataSource.length) {
            return dataSource.slice();
        }
        const field: string = isPrimitiveData ? '' : fields.value;
        const allResults: any[] = [];
        let predicate: any = null;
        let count: number = 0;
        const flushPredicate: () => void = () => {
            if (!predicate) {
                return;
            }
            const query: Query = new Query().where(predicate);
            const result: Object[] = new DataManager(dataSource).executeLocal(query);
            allResults.push(...result);
            predicate = null;
            count = 0;
        };
        for (let i: number = 0; i < values.length; i++) {
            const value: any = allowObjectBinding
                ? getValue(fields.value || '', values[i as number])
                : values[i as number];
            predicate = predicate
        ...
        for (let i: number = 0; i < allResults.length; i++) {
            const item: any = allResults[i as number];
            const key: string = isPrimitiveData
                ? item.toString()
    ...
    public destroy(): void {
        ...
        if (this.hiddenElement) {
            ...
            const children: HTMLCollection = this.hiddenElement.children;
            for (let i: number = 0; i < children.length; i++) {
                const child: Element = children[i as number];
                (child as any).onclick = null;
                (child as any).onchange = null;
                child.textContent = '';
                const attr: any = Array.prototype.slice.call(child.attributes);
                for (let n: number = 0; n < attr.length; n++) {
                    child.removeAttribute(attr[n as number].name);
                }
            }
            let selectedElement: HTMLOptionElement = this.hiddenElement.querySelector('option');
            if (selectedElement) {
            ...
            if (this.hiddenElement.attributes) {
                const attr: any = Array.prototype.slice.call(this.hiddenElement.attributes);
                for (let n: number = 0; n < attr.length; n++) {
                    this.hiddenElement.removeAttribute(attr[n as number].name);
                }
            }
