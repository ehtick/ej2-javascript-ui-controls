...
import {
    ...
    characterSpacingProperty, scalingProperty, hiddenProperty, fontFamilyFarEastProperty, fontFamilyNonFarEastProperty,
    bordersProperty, leftIndentProperty,
    ...
    characterFormatProperty, textProperty, fieldTypeProperty, hasFieldEndProperty, localeIdFarEastProperty, ligatureProperty
} from '../../index';
...
export class HelperMethods {
    ...
    /**
     * Returns a legacy-style keyCode derived from KeyboardEvent using key, code, and location.
     * @private
     * @param event - Specifies keyboard event.
     * @returns Returns a legacy-style keyCode derived from KeyboardEvent.
     */
    public static getLegacyKeyCode(event: KeyboardEvent): number {
        const code: string = event.code || '';
        const key: string = event.key || '';
        const loc: number = event.location || 0;
        switch (code) {
        ...
        if (/^F\d{1,2}$/.test(code)) {
            const n: number = parseInt(code.substring(1), 10);
            if (n >= 1 && n <= 12) {
        ...
        if (/^Digit[0-9]$/.test(code)) {
            return 48 + parseInt(code.substring(5), 10);
        }
        if (/^Numpad[0-9]$/.test(code)) {
            return 96 + parseInt(code.substring(6), 10);
        }

        // Some UAs used 'Spacebar'.
        if (key === ' ' || key === 'Spacebar') {
            return 32;
        }
        ...
        if (key && key.length === 1) {
            ...
            if (ch >= '0' && ch <= '9') {
                return (loc === 3 ? 96 : 48) + (ch.charCodeAt(0) - '0'.charCodeAt(0));
            }
            switch (ch) {
                case ';': return 186;
                case '=': return 187;
    ...
    public static writeCharacterFormat(characterFormat: any, isInline: boolean, format: WCharacterFormat, keywordIndex?: number, isWriteAllValues?: boolean): void {
        ...
        characterFormat[ligatureProperty[keywordIndex]] = isWriteAllValues ? format.ligature : isInline ? format.ligature : format.getValue('ligature');
    ...
    public static setCustomHeaders(httpRequest: XMLHttpRequest, headers: object[]): void {
