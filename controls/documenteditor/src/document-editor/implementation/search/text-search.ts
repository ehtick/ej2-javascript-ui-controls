        ...
        for (let i: number = 0; i < matches.length; i++) {
            ...
            for (let j: number = 0; j < spanKeys.length; j++) {
                ...
                if (match.index + match[0].length <= startIndex + spanLength) {
                    ...
                    if (!(!isNullOrUndefined(this.owner.searchModule.textSearchResults) && this.owner.searchModule.textSearchResults.length !== 0) && span.text.charAt(span.text.length - 1) !== ' ' && !isNullOrUndefined(span.nextElement) && (span.nextElement instanceof BookmarkElementBox)) {
                        ...
                        if (element && element instanceof TextElementBox && element.text.charAt(0) !== ' ' && element.text.charAt(0) !== '\t' && element.text.charAt(0) !== '' && element.text.charAt(0) !== '\f') {
                            isContainField = true;
                            continue;
                        }
                    }
