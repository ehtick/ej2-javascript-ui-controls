...
export class DocumentHelper {
    ...
    private isKeyPressEvent: boolean = false;
    ...
    */
    ...
    public onKeyPressInternal = (event: KeyboardEvent): void => {
        this.isKeyPressEvent = true;
        const key: number = HelperMethods.getLegacyKeyCode(event);
        ...
        if (!this.owner.isReadOnlyMode && this.owner.editorModule.canEditContentControl || (this.owner.documentHelper.protectionType === 'FormFieldsOnly' && this.owner.editorModule.canEditContentControl && !isNullOrUndefined(this.owner.documentHelper.selection) && this.owner.documentHelper.selection.checkContentControlLocked()) || (!isNullOrUndefined(this.selection) && this.selection.isInlineFormFillMode())) {
            let char: string = '';
            // Prefer the actual printable character from event.key for keypress event
            if (event.key && event.key.length === 1 && !event.ctrlKey && !event.altKey && !event.metaKey) {
                char = event.key;
            } else {
                const charCode: number = (event.charCode && event.charCode !== 0) ? event.charCode : key;
                if (charCode) {
    ...
    public updateFocus = (): void => {
        ...
        if (this.selection && !(this.isMobileDevice && this.owner.isReadOnly)) {
            ...
            if (!isNullOrUndefined(this.editableDiv) && !isNullOrUndefined(this.viewerContainer)) {
                if (this.isKeyPressEvent && !this.isScrollHandler) {
                    this.editableDiv.focus({ preventScroll: true });
                }
                else {
 ...
 */
    ...
    public onMouseDownInternal = (event: MouseEvent): void => {
        ...
        if (!isNullOrUndefined(this.selection)) {
            ...
            if (event.button === 2 && !this.owner.selectionModule.isEmpty
    ...
    public onKeyDownInternal = (event: KeyboardEvent): void => {
        ...
        if (key !== 8 && key !== 46 && !isNullOrUndefined(this.owner.editor) && this.selection.checkContentControlLocked()) {
            this.owner.editorModule.insertContentControlPlaceholder();
        }
...
export abstract class LayoutViewer {
     ...
     * @private
     */
    public zoomType: PageFitType = 'None';
    ...
    public findFocusedPage(currentPoint: Point, updateCurrentPage: boolean, updateHeaderFooterPage?: boolean, ispageUpAndDown?: boolean): Point {
        ...
        for (let i: number = 0; i < this.documentHelper.pages.length; i++) {
            ...
            if (pageTop <= point.y && pageTop + pageHeight >= point.y) {
                ...
                if (ispageUpAndDown && page.bodyWidgets.length > 0 && page.bodyWidgets[0].childWidgets.length > 0) {
                    const widget = page.bodyWidgets[0].childWidgets[0] as BodyWidget;
                    if (point.y < widget.y && widget) {
                        this.documentHelper.selection.updateYPointPosition(widget, point);
                    }
                }
                return point;
            }
