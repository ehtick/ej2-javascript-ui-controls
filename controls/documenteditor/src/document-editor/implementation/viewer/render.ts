...
export class Renderer {
    ...
    private renderHFWidgets(page: Page, widget: HeaderFooterWidget, width: number, isHeader: boolean): void {
        ...
        let headerFooterHeight: number = this.documentHelper.compatibilityMode === 'Word2013'
    ...
    private renderWidget(page: Page, widget: Widget): void {
        ...
        if (this.documentHelper.compatibilityMode === 'Word2003') {
            if (widget instanceof ParagraphWidget) {
                if ((widget as ParagraphWidget).isRender === false) {
                    return;
                }
            } else if (widget instanceof TableWidget) {
                if ((widget as TableWidget).isRender === false) {
                    return;
                }
            }
        }
...
private calculatePathBounds(data: string): Rect {
            ...
            for (let j: number = 0; j < bodyWidget.childWidgets.length; j++) {
                ...
                if (i === 0 && j === 0) {
                    ...
                    if (page.bodyWidgets.length > 1 && !isNullOrUndefined(bodyWidget.nextWidget) && !((bodyWidget.nextWidget as BodyWidget).sectionFormat.breakCode === 'NoBreak')) {
                        ...
                        let footNoteRightAlign: boolean = false;
                        if (this.documentHelper.footnotes.separator.length > 0 && this.documentHelper.footnotes.separator[0] instanceof ParagraphWidget) {
                            let separatorPara: ParagraphWidget = this.documentHelper.footnotes.separator[0] as ParagraphWidget;
                            if (separatorPara.paragraphFormat.bidi || separatorPara.paragraphFormat.textAlignment === "Right") {
                                footNoteRightAlign = true;
                            }
                        }
                        let footNotePosition: number = footNoteRightAlign ? (xPos + page.bodyWidgets[0].width - (210 * this.documentHelper.zoomFactor)) : xPos;
                        this.renderSolidLine(ctx, this.getScaledValue(footNotePosition, 1), this.getScaledValue(footnote.y + (footnote.margin.top / 2) + 1, 2), 210 * this.documentHelper.zoomFactor, '#000000');
                    }
                }
    ...
    private renderContentControlBorder(page: Page, lineWidget: LineWidget, top: number): void {
        if (!isNullOrUndefined(this.documentHelper.owner.editorModule) && page.documentHelper.selection && !isNullOrUndefined(page.documentHelper.selection.contentControleditRegionHighlighters)) {
            ...
            if (!isNullOrUndefined(contentControls)) {
                for (let i: number = 0; i < contentControls.length; i++) {
                    ...
                    if (!isNullOrUndefined(widgetInfo)) {
                        ...
                        if (lineIndex !== -1) {
                            ...
                            for (var j = 0; j < widget.length; j++) {
                                ...
                                if (lineIndex === 0 || lineWidget === contentControl.line) {
                                    //top
                                ...
                                else if (lineWidget !== contentControl.line && lineWidget !== contentControl.reference.line) {
                                    //top
                                    if (isNullOrUndefined(paragraph.previousWidget) && lineWidget === paragraph.firstChild) {
                                        this.renderSingleBorder(color, startX, startY, endX, startY, 1, 'single');
                                    }
                                    else if (contentControl.contentControlWidgetType !== 'Block') {
                                ...
                                else if (lineIndex === widgetInfo.length - 1 && lineWidget === contentControl.reference.line) {
                                    //Top
                                    let widgets: SelectionWidgetInfo[] = widgetInfo.get(widgetInfo.keys[lineIndex - 1]);
                                    if (isNullOrUndefined(paragraph.previousWidget) && lineWidget === paragraph.firstChild) {
                                        endX = this.documentHelper.getParagraphLeftPosition(paragraph) + this.getContainerWidth(paragraph, page) + HelperMethods.convertPointToPixel(paragraph.paragraphFormat.borders.right.space);
                                    ...
                                    if (!isNullOrUndefined(lastLine) && (!isNullOrUndefined(page.nextPage) || !isNullOrUndefined(paragraph.nextWidget)) && lineWidget === lastLine && lastLine.children[lastLine.children.length - 1] instanceof ContentControl  && lastLine.children[lastLine.children.length - 1] === contentControl.reference && contentControl.contentControlWidgetType === 'Block') {
                                        //bottom
                                        endX = this.documentHelper.getParagraphLeftPosition(paragraph) + this.getContainerWidth(paragraph, page) + HelperMethods.convertPointToPixel(paragraph.paragraphFormat.borders.right.space);
                                        this.renderSingleBorder(color, startX, endY, endX, endY, 1, 'single');
                                    }
    ...
    private  renderLine(lineWidget: LineWidget, page: Page, left: number, top: number): void {
        ...
        if (this.documentHelper.owner.documentEditorSettings.showHiddenMarks && !this.isPrinting) {
            ...
            if (lineWidget.isEndsWithColumnBreak) {
                ...
                characterFont = this.retriveCharacterformat(currentCharFormat, 0.7);
                text = this.getPageBreakText(lineWidget.paragraph, l10n);
            ...
            if (lineWidget.paragraph.containerWidget instanceof BodyWidget && !isNullOrUndefined(lineWidget.paragraph.nextRenderedWidget)) {
                if (lineWidget.paragraph.containerWidget.sectionIndex !== (lineWidget.paragraph.nextRenderedWidget.containerWidget as BodyWidget).sectionIndex && lineWidget.isLastLine()) {
                    ...
                    characterFont = this.retriveCharacterformat(currentCharFormat, 0.7);
                    const previousWidget: Widget = lineWidget.paragraph.previousRenderedWidget;
                    // If the previous paragraph is ends with page break and current paragraph is section break then add the width of the page break.
                    if (previousWidget instanceof ParagraphWidget && previousWidget.isEndsWithPageBreak && lineWidget.paragraph.isSectionBreak) {
                        const pageBreakText: string = this.getPageBreakText(previousWidget, l10n);
                        // Calculating the width of the Page break. And 0.7 is previously used in retrieveCharacterFormat 
                        x += (this.documentHelper.textHelper.getWidth(pageBreakText, previousWidget.characterFormat, FontScriptType.English) * 0.7);
                        // In layouting itself mark is calculated and Updated in X property.
                        const paragraphMarkSize: TextSizeInfo = this.documentHelper.textHelper.getParagraphMarkSize(previousWidget.characterFormat);
                        x -= paragraphMarkSize.Width;
                    } else if (lineWidget.isEndsWithPageBreak) {
                        if (lineWidget.paragraph.bidi) {
    ...
    private getPageBreakText(paragraph: ParagraphWidget, l10n: L10n): string {
        if (paragraph.bidi) {
            return String.fromCharCode(182) + '.....' + l10n.getConstant('Page Break') + '.....';
        } else {
    ...
    private getBackgroundColorHeirachy(element: ElementBox): string {
        ...
        if (element.paragraph.isInsideTable) {
            ...
            let foreColor: string = cell.cellFormat.shading.foregroundColor;
            if (cell.cellFormat.shading.hasValue('textureStyle') && foreColor ==='empty' && (bgColor === "empty" || bgColor === '#00000000')) {
                return this.drawTextureStyle(cell.cellFormat.shading.textureStyle, HelperMethods.getColor(foreColor), HelperMethods.getColor(bgColor), true, true);
            }
        ...
        if (currentText.length > 0) {
            ...
            if (spellInfo.isElementPresent && this.spellChecker.enableOptimizedSpellCheck) {
                ...
                if (this.spellChecker.isInUniqueWords(currentText)) {
                    ...
                    const prevCombined: TextElementBox[] = isNullOrUndefined(combinedElements) ? [] : [...combinedElements];
                    /* eslint-disable @typescript-eslint/no-explicit-any */
    ...
    private renderCellBackground(height: number, cellWidget: TableCellWidget, leftMargin: number, rightMargin: number, lineWidth: number): void {
        ...
        if (bgColor !== 'empty') {
            const cellLeftBorder: WBorder = TableCellWidget.getCellLeftBorder(cellWidget);
            const cellTopBorder: WBorder = TableCellWidget.getCellTopBorder(cellWidget);
            // If the current cell is not the last cell in the row, and the left border is either not styled (lineStyle is 'None') or has been explicitly cleared,
            // increase the width by 0.5 units to account for the visual gap.
            if (cellWidget !== cellWidget.ownerRow.lastChild && (cellLeftBorder.lineStyle === 'None' || cellLeftBorder.lineStyle === 'Cleared')) {
                width += 0.5;
            }
            // If the current row is not the last row in the table, and the top border is either not styled (lineStyle is 'None') or has been explicitly cleared,
            // increase the height by 0.5 units to account for the visual gap.
            if (cellWidget.ownerRow.ownerTable.lastChild !== cellWidget.ownerRow && (cellTopBorder.lineStyle === 'None' || cellTopBorder.lineStyle === 'Cleared')) {
                height += 0.5;
            }
        }
