...
export class Layout {
    ...
    */
    ...
    private skip2003: boolean = false
    private textWrappingBottom: number = 0;
    private istableYpositionChange: boolean = false;
    /**
     * @private
     */
     ...
     * @private
     */
    public lastScrollUpdatePageIndex: number = -1;
    /**
     * @private
     */
    ...
    public layoutItems(sections: BodyWidget[], isReLayout: boolean, isContinuousSection?: boolean, isAsync?: boolean): void | Promise<void> {
        const processLayout = async (): Promise<void> => {
            let page: Page;
            let height = 0;
            let width = 0;
            for (let i = 0; i < sections.length; i++) {
                const section = sections[i] as BodyWidget;
                if (!this.documentHelper.owner.enableLayout) {
                    this.viewer.createNewPage(section);
                    const headersFooters = this.documentHelper.headersFooters[section.index];
                    if (headersFooters) {
                        for (const key in headersFooters) {
                            if (headersFooters.hasOwnProperty(key)) {
                                const currentHeaderFooter = headersFooters[key];
                    ...
                    const nextSection: BodyWidget | undefined = sections[i + 1] as BodyWidget;
                    this.viewer.columnLayoutArea.setColumns(section.sectionFormat);
                    let lastpage = this.documentHelper.pages[this.documentHelper.pages.length - 1];
                    ...
                    if (i > 0 && !isNullOrUndefined(bodyWidget) && !isNullOrUndefined(bodyWidget.lastChild) && !(bodyWidget.lastChild instanceof TableWidget)) {
                        ...
                        if (lastChild instanceof ParagraphWidget && lastChild.isSectionBreak && previousWidget instanceof ParagraphWidget && previousWidget.isEndsWithPageBreak) {
                    ...
                    const layoutResult = this.layoutSection(section, 0, nextSection, isAsync);
    ...
    private shouldScrollUpdateIn(): boolean {
        const currentPageCount = this.documentHelper.pages.length;
        const settings = this.owner.documentEditorSettings.openAsyncSettings;
        const initialPageCount = isNullOrUndefined(settings.initialPageLoadCount) || settings.initialPageLoadCount < 0 ? 3 : settings.initialPageLoadCount;
        const incrementalPageCount = isNullOrUndefined(settings.incrementalPageLoadCount) || settings.incrementalPageLoadCount < 0 ? 5 : settings.incrementalPageLoadCount;
    ...
    private layoutSection(section: BodyWidget, index: number, nextSection: BodyWidget, isAsync?: boolean): void | Promise<void> {
        ...
        const processBlocks = async (): Promise<void> => {
            while (block) {
                ...
                if (!isNullOrUndefined(block)) {
                    ...
                    const layoutResult = this.layoutBlock(block, index, undefined, isAsync);
                    if (layoutResult instanceof Promise) {
                        nextBlock = await layoutResult;
                    } else {
                ...
                if (isAsync) {
                    const shouldScrollUpdate = this.shouldScrollUpdateIn();
                    if (shouldScrollUpdate) {
                        ...
                        await new Promise<void>(resolve => {
                            ...
                            if (this.documentHelper.currentDocumentLoadId) {
                                this.documentHelper.storeTimeoutId(this.documentHelper.currentDocumentLoadId, timeoutId as unknown as number);
                            }
                        });
                    }
                }
    ...
    private processAsyncPageUpdate(section: BodyWidget): void {
        ...
        if (section instanceof BodyWidget && section.firstChild && (section.firstChild as BlockWidget).bodyWidget && (section.firstChild as BlockWidget).bodyWidget.page) {
            page = (section.firstChild as BlockWidget).bodyWidget.page;
        }
        while (page) {
            if (page.footnoteWidget) {
                this.layoutfootNote(page.footnoteWidget);
    ...
    private reLayoutMultiColumnBlock(section: BodyWidget, nextSection: BodyWidget, blockIndex: number): void {
        ...
        do {
            ...
            if (!isNullOrUndefined(block)) {
                ...
                nextBlock = this.layoutBlock(block, 0, block.index < blockIndex ? true : false) as BlockWidget;
                this.viewer.updateClientAreaForBlock(block, false);
                block = nextBlock;
            }
   ...
   public getColumnBreak(section: BodyWidget): boolean {
        ...
        while (section && section.columnIndex !== 0) {
            ...
            for (let i: number = 0; i < section.childWidgets.length; i++) {
                if (section.childWidgets[i] instanceof BlockWidget && !isNullOrUndefined((section.childWidgets[i] as BlockWidget).previousSplitWidget)
                    ...
                    && ((section.childWidgets[i] as BlockWidget).previousSplitWidget as BlockWidget).bodyWidget.page !== ((section.childWidgets[i] as BlockWidget).previousSplitWidget.previousSplitWidget as BlockWidget).bodyWidget.page) {
                    isPreviousSplit = true;
                    if (section.childWidgets[i] instanceof TableWidget && prevSection.lastChild instanceof TableWidget) {
                        const tableWidget: TableWidget = section.childWidgets[i] as TableWidget;
                        const prevTableWidget: TableWidget = prevSection.lastChild as TableWidget;
                        const previousRowIndex: number = (prevTableWidget.childWidgets[prevTableWidget.childWidgets.length - 1] as TableRowWidget).index;
                        for (let j: number = 0; j < tableWidget.childWidgets.length; j++) {
                            const row: TableRowWidget = tableWidget.childWidgets[j] as TableRowWidget;
                            const currentRowIndex: number = row.index;
                            if (currentRowIndex !== previousRowIndex) {
                                // Different rows: push the whole row
                                prevTableWidget.childWidgets.push(row);
                                row.containerWidget = prevTableWidget;
                            } else {
                                // Same row: merge the contents
    ...
    public updateRevisionsToHeaderFooter(clone: HeaderFooterWidget, page: Page): any {
        ...
        if (clone instanceof HeaderFooterWidget && childWidge.length > 0) {
            for (let i: number = 0; i < childWidge.length; i++) {
                if (childWidge[i] instanceof ParagraphWidget) {
                    this.updateRevisionsToHeaderFooterParagraph(childWidge[i] as ParagraphWidget, page);

                } else if (childWidge[i] instanceof TableWidget) {
                    this.updateRevisionsToHeaderFooterTable(childWidge[i] as TableWidget, page);
                }
            }
        }
    }
    ...
    private updateRevisionRange(revision: Revision, page: Page): any {
        for (let i: number = 0; i < revision.getRange().length; i++) {
            const inline: ElementBox | WCharacterFormat | WRowFormat = (revision.getRange()[i]);
            if (inline instanceof ElementBox && !isNullOrUndefined(inline.line.paragraph.bodyWidget) && 
                !isNullOrUndefined(inline.line.paragraph.bodyWidget.page)) {
                inline.line.paragraph.bodyWidget.page = page;
            }
            else if (inline instanceof WCharacterFormat && !isNullOrUndefined((inline.ownerBase as ParagraphWidget).bodyWidget) && 
                !isNullOrUndefined((inline.ownerBase as ParagraphWidget).bodyWidget.page)) {
                (inline.ownerBase as ParagraphWidget).bodyWidget.page = page;
            }
            else if (inline instanceof WRowFormat && !isNullOrUndefined(inline.ownerBase.bodyWidget) && 
                !isNullOrUndefined(inline.ownerBase.bodyWidget.page)) {
                inline.ownerBase.bodyWidget.page = page;
            }
        }
    }

    private updateRevisionsToHeaderFooterParagraph(para: ParagraphWidget, page: Page): void {
        if (para.childWidgets.length > 0) {
            for (let i: number = 0; i < para.childWidgets.length; i++) {
                const lineWidget: LineWidget = para.childWidgets[i] as LineWidget;
                for (let j: number = 0; j < lineWidget.children.length; j++) {
                    ...
                    if (element.removedIds.length > 0) {
                        const removedIds = element.removedIds;
                        for (let k: number = 0; k < removedIds.length; k++) {
                            const revision: Revision = this.documentHelper.revisionsInternal.get(removedIds[k]);
                            element.insertRevisionAt(k, revision);
        ...
        if (para.characterFormat.removedIds.length > 0) {
            const removedIds = para.characterFormat.removedIds;
            for (let i: number = 0; i < removedIds.length; i++) {
                const revision: Revision = this.documentHelper.revisionsInternal.get(removedIds[i]);
                para.characterFormat.insertRevisionAt(i, revision);
    ...
    private updateRevisionsToHeaderFooterTable(table: TableWidget, page: Page): void {
        for (let i: number = 0; i < table.childWidgets.length; i++) {
            ...
            for (let j: number = 0; j < rowWidget.childWidgets.length; j++) {
                ...
                for (let k: number = 0; k < cellWidget.childWidgets.length; k++) {
                    if (cellWidget.childWidgets[k] instanceof ParagraphWidget) {
                        this.updateRevisionsToHeaderFooterParagraph(cellWidget.childWidgets[k] as ParagraphWidget, page);
                    }
                    else if (cellWidget.childWidgets[k] instanceof TableWidget) {
                        this.updateRevisionsToHeaderFooterTable(cellWidget.childWidgets[k] as TableWidget, page);
                    }
                }
            }
            if (rowWidget.rowFormat.removedIds.length > 0) {
                const removedIds = rowWidget.rowFormat.removedIds;
                for (let l: number = 0; l < removedIds.length; l++) {
                    const revision: Revision = this.documentHelper.revisionsInternal.get(removedIds[l]);
                    rowWidget.rowFormat.insertRevisionAt(l, revision);
                    removedIds.splice(l, 1);
                    l--;
                    this.updateRevisionRange(revision, page);
                }
            }
    ...
    private shiftChildLocation(shiftTop: number, bodyWidget: HeaderFooterWidget | FootNoteWidget): number {
        ...
        if (this.documentHelper.compatibilityMode === 'Word2003') {
            footerMaxHeight = bodyWidget.page.boundingRectangle.height - (bodyWidget.page.boundingRectangle.height / 100) * 50;
        }
                            ...
                            if (element.verticalOrigin === "Paragraph" || element.verticalOrigin === "Line") {
                                ...
                                if (element.verticalOrigin === "Paragraph" && element.textWrappingStyle == "TopAndBottom") {
                                    ...
                                    if (shapeRect.isIntersecting(paraRect)) {
                                        element.y = childWidget.y - element.height;
                                    }
                                }
                            }
                            else {
    ...
    private shiftChildLocationForTableCellWidget(cellWidget: TableCellWidget, shiftTop: number): void {
        ...
        for (let i: number = 0; i < cellWidget.childWidgets.length; i++) {
            if (cellWidget.childWidgets[i] instanceof ParagraphWidget) {
                ...
                let floatInlineShapes: ShapeBase[] = (cellWidget.childWidgets[i] as ParagraphWidget).floatingElements;
                if (!isNullOrUndefined(floatInlineShapes) && floatInlineShapes.length > 0) {
                    for (let j: number = 0; j < floatInlineShapes.length; j++) {
                        if (floatInlineShapes[j].textWrappingStyle === "Inline" && floatInlineShapes[j] instanceof ShapeElementBox) {
                            floatInlineShapes[j].y += shiftTop;
                            this.updateChildLocationForCellOrShape(floatInlineShapes[j].y, (floatInlineShapes[j] as ShapeElementBox));
                        }
                    }
                }
            } else {
    ...
    private layoutBlock(block: BlockWidget, index: number, isUpdatedList?: boolean, isAsync?: boolean): BlockWidget | Promise<BlockWidget> {
        ...
        if (block instanceof ParagraphWidget) {
            ...
            const layoutResult = this.layoutParagraph(block, index, isUpdatedList, isAsync);
            if (layoutResult instanceof Promise) {
                // Handle Promise case with .then()
                return layoutResult.then((result: BlockWidget) => {
                    nextBlock = result;
            ...
            const layoutResult = this.layoutTable(block as TableWidget, index, isAsync);
            if (layoutResult instanceof Promise) {
                // Handle Promise case with .then()
                return layoutResult.then((result: BlockWidget) => {
                    nextBlock = result;
     ...
     * @private
     */
    public addLineWidget(paragraphWidget: ParagraphWidget): LineWidget {
    ...
    private layoutParagraph(paragraph: ParagraphWidget, lineIndex: number, isUpdatedList?: boolean, isAsync?: boolean): BlockWidget | Promise<BlockWidget> {
        if (!this.viewer.owner.enableLayout) {
            return null;
        }
        ...
        const finalizeParagraph = (): BlockWidget => {
        ...
        const processSingleLine = (): boolean => {
            if (isNullOrUndefined(line)) {
                return false;
            }
        ...
        if (isAsync) {
            return new Promise<BlockWidget>((resolve) => {
                const processLineAsync = (): void => {
                    if (!processSingleLine()) {
                        resolve(finalizeParagraph());
                        return;
                    }
                    const shouldScrollUpdate = this.shouldScrollUpdateIn();
                    if (shouldScrollUpdate) {
                        ...
                        if (paragraph.bodyWidget instanceof BodyWidget) {
                            section = paragraph.bodyWidget.getSplitWidgets()[0] as BodyWidget;
                        }
                        this.processAsyncPageUpdate(section);
                        if (!this.documentHelper.isUpdatedWhileLoadAsync) {
                            this.documentHelper.performUpdateAfterLoad(true);
                        ...
                        const timeoutId = setTimeout(processLineAsync, 0);
                        if (this.documentHelper.currentDocumentLoadId) {
                            this.documentHelper.storeTimeoutId(this.documentHelper.currentDocumentLoadId, timeoutId as unknown as number);
                        }
                    } else {
    ...
    private layoutElement(element: ElementBox, paragraph: ParagraphWidget, isEmptyField?: boolean): void {
        ...
        if (this.viewer.owner.editorModule && this.viewer.owner.editorHistoryModule && this.viewer.owner.editorHistoryModule.isRedoing && !isNullOrUndefined((element.paragraph.containerWidget as BodyWidget).footNoteReference) && this.viewer.owner.enableTrackChanges && element.removedIds.length > 0) {
            this.viewer.owner.editorModule.constructRevisionFromID(element);
        }
        ...
        let bodyWidget = paragraph.bodyWidget as BodyWidget;
        ...
        if (text === '\f' && !isNullOrUndefined(element.nextNode) && element.nextNode instanceof ContentControl) {
    ...
    */
    ...
    private isNeedToWrapForSquareTightAndThrough(bodyWidget: BlockContainer, elementBox: ElementBox, wrapOwnerIndex: number, wrapItemIndex: number, textWrappingStyle: TextWrappingStyle, textWrappingBounds: Rect, allowOverlap: boolean, wrapCollectionIndex: number, floatingEntity: ShapeBase | TableWidget, isTextRangeInTextBox: boolean, rect: Rect, width: number, height: number): boolean {
        ...
        return (floatingElements.length > 0
    ...
    private isNeedToWrapForSquareTightAndThroughForTable(container: BlockContainer, table: TableWidget, wrapIndex: number, wrapItemIndex: number, wrappingStyle: TextWrappingStyle, textWrappingBounds: Rect, allowOverlap: boolean, wrapCollectionIndex: number, floatingElemnt: ShapeBase | TableWidget, isInTextBox: boolean, rect: Rect, width: number, height: number): boolean {
        ...
        if (container.floatingElements.length > 0) {
            floatingElements = container.floatingElements;
        } else if (!isNullOrUndefined(container.page) && !isNullOrUndefined(container.page.headerWidget) && container.page.headerWidget.floatingElements.length > 0) {
            floatingElements = container.page.headerWidget.floatingElements;
        }
        return (floatingElements.length > 0 && wrapIndex !== wrapCollectionIndex
    ...
    private adjustClientAreaBasedOnTextWrap(elementBox: ElementBox, rect: Rect, isEmptyPara: boolean): Rect {
        ...
        if (this.isNeedToWrapLeafWidget(ownerPara, elementBox)) {
            ...
            let floatingElements: (ShapeBase | TableWidget)[] = [];
            if (bodyWidget.floatingElements.length > 0) {
                const page: Page = bodyWidget.page;
                for (let i: number = 0; i < page.bodyWidgets.length; i++) {
                    // In few cases current paragraph and previous body widget floating items will get overlapped. So need to adjust the client area for previous body widget in the page. 
                    if (bodyWidget instanceof BodyWidget && page.bodyWidgets[i] !== bodyWidget) {
                        floatingElements = [...floatingElements, ...page.bodyWidgets[i].floatingElements];
                    } else {
                        ...
                        floatingElements = [...floatingElements, ...bodyWidget.floatingElements];
                        break;
                    }
                }
            }
    ...
    private adjustClientAreaBasedOnTextWrapForTable(table: TableWidget, rect: Rect): Rect {
        ...
        if (!isNullOrUndefined(floatingElements) && floatingElements.length > 0) {
            ...
            for (let i: number = 0; i < floatingElements.length; i++) {
                let floatingElement: ShapeBase | TableWidget = floatingElements[i];
                let allowOverlap: boolean = false;
                ...
                if (!(clientLayoutArea.x > (wrappingBounds.right + minimumWidthRequired) || clientLayoutArea.right < wrappingBounds.x - minimumWidthRequired)) {
                    if (this.isNeedToWrapForSquareTightAndThroughForTable(bodyWidget, table, -1, -1, textWrappingStyle, wrappingBounds, allowOverlap, 1, floatingElement, false, rect, tableWidth, tableHeight) && !(this.isRelayout && floatingElement instanceof TableWidget && floatingElement.positioning.verticalOrigin === 'Paragraph'&& table.index < floatingElement.index)) {
                        ...
                        if (rect.x >= wrappingBounds.x && rect.x < wrappingBounds.right && textWrappingType !== 'Left') // Skip to update when the wrap type as left
                        {
                        ...
                        } else if (wrappingBounds.x > rect.x && rect.right > wrappingBounds.x && wrappingBounds.bottom > rect.y && !table.wrapTextAround) {
                            let width: number = this.getTableWidth(table);
                            if (table.x + width < wrappingBounds.x) {
                                continue;
                            }
                            ...
                            var remainingHeightOfFloatingItem = (wrappingBounds.bottom > rect.y) ? wrappingBounds.bottom - rect.y : 0;
                            rect.y = wrappingBounds.bottom;
    ...
    private checkLineWidgetWithClientArea(line: LineWidget, element: ElementBox): void {
        let moveToNextPage: boolean = false;
        if (line !== element.line || element.line === line && isNullOrUndefined(element.nextElement)
            && !element.line.isLastLine()) {
            moveToNextPage = this.is2013Justification && this.viewer.clientActiveArea.width > 0;
            this.moveToNextLine(line);
        ...
        if (element.line !== line && this.viewer instanceof PageLayoutViewer
            ...
            this.moveToNextPage(this.viewer, element.line.nextLine);
    ...
    private layoutEmptyLineWidget(paragraph: ParagraphWidget, isEmptyLine: boolean, line?: LineWidget, isShiftEnter?: boolean): void {
        ...
        const afterSpacingValue: number = HelperMethods.convertPointToPixel(this.getAfterSpacing(paragraph));
        if (afterSpacingValue > 0 && isShiftEnter && !isNullOrUndefined(line) && isEmptyLine && lineWidget.isLastLine()) {
            line.margin.bottom = Math.max(0, line.margin.bottom - afterSpacingValue);
            line.height = Math.max(0, line.height - afterSpacingValue);
            this.viewer.clientActiveArea.y -= afterSpacingValue;
            this.viewer.clientActiveArea.height += afterSpacingValue;
        }
     ...
     * @private
     */
    public updateMargin(line: LineWidget): void {
    ...
    private moveToNextLine(line: LineWidget, isMultiColumnSplit?: boolean, index?: number): void {
        ...
        for (let i: number = 0; i < children.length; i++) {
            ...
            if (textAlignment === "Justify" && elementBox instanceof ShapeBase && elementBox.textWrappingStyle === 'Inline' && subWidth !== 0) {
                elementBox.x = elementLeft;
                if (line.paragraph.firstChild === line && line.paragraph.paragraphFormat.firstLineIndent < 0) {
                    elementBox.x += HelperMethods.convertPointToPixel(line.paragraph.paragraphFormat.firstLineIndent);
                }
                if (elementBox instanceof ShapeElementBox) {
                    for (let i = 0; i < elementBox.textFrame.childWidgets.length; i++) {
                        const widget: BlockWidget = elementBox.textFrame.childWidgets[i] as BlockWidget;
                        let indent: number = widget.bidi ? widget.rightIndent : widget.leftIndent;
                        widget.x = elementBox.x + HelperMethods.convertPointToPixel(indent + elementBox.textFrame.marginLeft);
                    }
                }
            }
            elementLeft += elementBox.width;
        ...
        if (paragraphWidget.bodyWidget instanceof HeaderFooterWidget) {
            if (!paragraphWidget.isInsideTable) {
                if ((!this.skipRelayoutOverlap || (this.endOverlapWidget instanceof TableWidget && this.endOverlapWidget.wrapTextAround))) {
                    if (paragraphWidget.containerWidget instanceof HeaderFooterWidget && this.documentHelper && this.documentHelper.compatibilityMode === 'Word2003') {
                        const container: any = paragraphWidget.containerWidget;
                        const pageHeight = (container && container.page) ? container.page.boundingRectangle.height : container.height;
                        const maxAllowed = pageHeight * 0.5;
                        if ((container.height + paragraphWidget.height <= maxAllowed && !this.skip2003) || paragraphWidget.containerWidget.firstChild === paragraphWidget) {
                            container.height += paragraphWidget.height;
                            paragraphWidget.isRender = true;
                        } else {
                            this.viewer.clientActiveArea.y -= paragraphWidget.height;
                            this.skip2003 = true;
                            paragraphWidget.isRender = false;
                        }
                    } else {
                        paragraphWidget.containerWidget.height += paragraphWidget.height;
                    }
    ...
    public getListNumber(paragraph: ParagraphWidget, isAutoList?: boolean): string {
        let listFormat: WListFormat = paragraph.paragraphFormat.listFormat;
    ...
    public getListTextListLevel(listLevel: WListLevel, listValue: number, localeId: number, customPattern?: ListLevelPattern): string {
        switch (!isNullOrUndefined(customPattern) ? customPattern : listLevel.listLevelPattern) {
            ...
            case 'Number':
                return (listValue).toString();
            case 'OrdinalText':
                if (listLevel.characterFormat.localeIdAscii === 3082 || localeId === 3082) {
                ...
                if (cell.childWidgets.length > 0) {
                    for (let k: number = 0; k < cell.childWidgets.length; k++) {
                        ...
                        if (block instanceof ParagraphWidget) {
                            ...
                            if (!isNullOrUndefined(existingPara) && existingPara.index === block.index) {
    ...
    private getSplittedWidgetForSpannedRow(bottom: number, tableRowWidget: TableRowWidget, tableCollection: TableWidget[], rowCollection: TableRowWidget[], footNoteCollection: FootnoteElementBox[]): TableRowWidget {
        ...
        for (let i: number = 0; i < this.documentHelper.splittedCellWidgets.length; i++) {
            ...
            while (previousColumnIndex > 0 && !issplit) {
                ...
                splittedCell = this.getSplittedWidget(bottom, true, tableCollection, rowCollection, cellWidget, footNoteCollection);
                splittedWidget.childWidgets.splice(0, 0, splittedCell);
                if (splittedCell.index === (splittedCell.containerWidget as TableRowWidget).childWidgets.length - 1) {
                    splittedCell.width += splittedCell.rightBorderWidth;
                }
   ...
   public updateWidgetsToTable(tableWidgets: TableWidget[], rowWidgets: TableRowWidget[], row: TableRowWidget, rearrangeRow: boolean, lineIndexInCell?: number, cellIndex?: number, isMultiColumnSplit?: boolean): void {
        ...
        while (count < rowWidgets.length) {
            ...
            if (!isMultiColumnSplit && (row.ownerTable.isInsideTable || row.bodyWidget instanceof HeaderFooterWidget  || (this.documentHelper.splittedCellWidgets.length === 0 && tableRowWidget.y + tableRowWidget.height + cellSpacing + this.footHeight <= viewer.clientArea.bottom))) {
                row.ownerTable.isHeaderNotRepeatForAllPages = false;
                ...
                let skipToInsert: boolean = this.documentHelper.splittedCellWidgets.length > 0 && row.ownerTable.isInsideTable && !row.ownerTable.equals(this.documentHelper.splittedCellWidgets[0].ownerTable) ? true : false;
                if (!isNullOrUndefined(row.bodyWidget) && row.bodyWidget instanceof BodyWidget && this.documentHelper.splittedCellWidgets.length > 0 && !skipToInsert && isNullOrUndefined(rowWidgets[rowWidgets.length - 1].nextRow)) {
                    count--;
                    isLastRow = true;
                ...
                } else if (!isNullOrUndefined(row.bodyWidget) && row.bodyWidget instanceof BodyWidget && this.documentHelper.splittedCellWidgets.length > 0 && !skipToInsert && !isNullOrUndefined(rowWidgets[rowWidgets.length - 1].nextRow) && !this.isRowSpanEnd(rowWidgets[rowWidgets.length - 1].nextRow, viewer)) {
                    count--;
                }
                ...
                if (!isNullOrUndefined(splittedWidget) && (isNullOrUndefined(this.documentHelper.owner.editorModule) || this.documentHelper.owner.editorModule && !this.documentHelper.owner.editorModule.isTableInsert) && !(splittedWidget.bodyWidget.containerWidget instanceof FootNoteWidget)) {
                    tableWidget.isHeaderNotRepeatForAllPages = false;
                    ...
                    if (moveRowToNextTable && rowToMove.index > 0 || rowWidgets.length > 1) {
                        ...
                        if (HelperMethods.convertPointToPixel(currentRow.rowFormat.height) + tableWidget.headerHeight > this.viewer.clientArea.height) {
                            tableWidget.isHeaderNotRepeatForAllPages = true;
                        } else {
                            tableWidget.isHeaderNotRepeatForAllPages = false;
                        }
                    ...
                    if (insertHeaderRow && rowToMove.ownerTable.header && !keepNext && !(this.documentHelper.compatibilityMode === "Word2013" && tableWidget.isHeaderNotRepeatForAllPages)) {
                        if (viewer instanceof PageLayoutViewer) {
    ...
    public updateCellVerticalPosition(cellWidget: TableCellWidget, isUpdateToTop: boolean, isInsideTable: boolean): void {
        ...
        let parentDisplacement: number = 0;
        if (containerWidget instanceof BlockContainer || containerWidget instanceof TextFrame || isInsideTable) {
            ...
            if (containerWidget instanceof TableCellWidget) {
                parentDisplacement = this.getDisplacement(containerWidget, isUpdateToTop);
            }
            //Update Y position alone for the child widget of cell
            this.updateCellContentVerticalPosition(cellWidget, displacement, parentDisplacement, isUpdateToTop);
        }
    }

    private updateCellContentVerticalPosition(cellWidget: TableCellWidget, displacement: number, parentDisplacement: number, isUpdateToTop: boolean): void {
        if (parentDisplacement === 0 && displacement === 0) {
            return;
        }
    ...
    private getCellContentHeight(cellWidget: TableCellWidget, isDisplacement: boolean, paraIndex?: number): number {
        ...
        for (let i: number = 0; i < cellWidget.childWidgets.length; i++) {
            if (cellWidget.childWidgets[i] instanceof ParagraphWidget) {
                const para: ParagraphWidget = cellWidget.childWidgets[i] as ParagraphWidget;
                const currentParaBottom: number = para.y + para.height;
                ...
                if (currentParaBottom > withShapeBottom || withShapeBottom < lastParaBottom || para.isInHeaderFooter) {
                ...
                for (let k: number = 0; k < para.floatingElements.length; k++) {
                    ...
                    const rowHeight: number = cellWidget.height > cellWidget.containerWidget.height && cellWidget.cellFormat.rowSpan > 1 ? cellWidget.height : cellWidget.containerWidget.height;
                    if ((cellY + rowHeight) > shapeBottom && shapeBottom > withShapeBottom) {
    ...
    private considerPositionTableHeight(cellWidget: TableCellWidget, nestedWrapTable: TableWidget): boolean {
        ...
        if (nestedWrapTable.isLayouted && nestedWrapTable.wrapTextAround) {
            ...
            if (nestedWrapTable.nextWidget && (nestedWrapTable.nextWidget.y + nestedWrapTable.nextWidget.height) >
            (nestedWrapTable.y + nestedWrapTable.height)) {
                isConsider = true;
            }
        }
        return isConsider;
            ...
            if (paragraphWidget.isInsideTable) {
                ...
                if (!this.isReplacingAll && skipWholeTableLayout && container instanceof BodyWidget && isNullOrUndefined(container.containerWidget)) {
                    ...
                    if (parentTable.tableFormat.allowAutoFit) {
                        paragraphWidget = paragraphWidget.getSplitWidgets()[0] as ParagraphWidget;
    ...
    public reLayoutTable(block: BlockWidget, isFootnoteReLayout?: boolean): void {
        if ((this.viewer.owner.editorModule && this.viewer.owner.editorModule.restrictLayout) || !this.viewer.owner.enableLayout) {
            return;
        }
        //Get Top level owner of block
    ...
    public layoutBodyWidgetCollection(blockIndex: number, bodyWidget: Widget, block: BlockWidget, shiftNextWidget: boolean, isSkipShifting?: boolean, isSelectionInsideTable?: boolean): void {
        ...
        if (!isNullOrUndefined(this.documentHelper.owner)
            ...
            if ((this.viewer.owner.editorModule && this.viewer.owner.editor.restrictLayout) || !this.viewer.owner.enableLayout) {
                return;
            }
            if (bodyWidget instanceof BlockContainer || bodyWidget instanceof TextFrame) {
                ...
                if (blockIndex > 0 || (curretBlock.bodyWidget.sectionFormat.breakCode === 'NoBreak' && curretBlock.bodyWidget.index !== 0 && curretBlock === bodyWidget.firstChild)) {
                    const nextSplit: BlockWidget = (curretBlock as BlockWidget).nextSplitWidget as BlockWidget;
                    if (!isNullOrUndefined(nextSplit)) {
                        const nextSplitWidgetContainer: BodyWidget = nextSplit.containerWidget as BodyWidget;
                        const splitParaFootnotes: BodyWidget[] = this.getFootNoteWidgetsOf(nextSplit);
                        if (splitParaFootnotes && splitParaFootnotes.length > 0) {
                            this.moveFootNotesToPage(splitParaFootnotes, nextSplitWidgetContainer, curretBlock.containerWidget as BodyWidget);
                        }
                    }
    ...
    public layoutTable(table: TableWidget, startIndex: number, isAsync?: boolean): BlockWidget | Promise<BlockWidget> {
        ...
        const finalizeTable = (): BlockWidget => {
            this.updateFootnoteHeight(table, false);
            if (this.documentHelper.viewer instanceof PageLayoutViewer && table.wrapTextAround && (table.positioning.verticalAlignment === 'Bottom' ||
                table.positioning.verticalAlignment === 'Center' ||
                table.positioning.verticalAlignment === 'Outside')) {
                this.updateTableFloatPoints(table);
                this.updateChildLocationForTable(table.y, table);
            }
            this.updateWidgetsToPage(tableView, [], table, true);
            if (wrapDiff > 0) {
                this.viewer.clientArea.x -= wrapDiff;
            }
            if (table.wrapTextAround) {
            ...
            if (this.documentHelper.compatibilityMode !== 'Word2013' && !table.isInsideTable && !table.wrapTextAround &&
                !isNullOrUndefined(((table.firstChild as TableRowWidget).firstChild as TableCellWidget).leftMargin)) {
                this.viewer.clientArea.x += HelperMethods.convertPointToPixel(((table.firstChild as TableRowWidget).firstChild as TableCellWidget).leftMargin);
            }
        ...
        if (isAsync) {
            return new Promise<BlockWidget>((resolve) => {
                const processRowAsync = (): void => {
                    if (isNullOrUndefined(row)) {
                        resolve(finalizeTable());
                        return;
                    }
                    processRow();
                    const shouldScrollUpdate = this.shouldScrollUpdateIn();
                    if (shouldScrollUpdate) {
                        let section: BodyWidget;
                        if (table.bodyWidget instanceof BodyWidget) {
                            section = table.bodyWidget.getSplitWidgets()[0] as BodyWidget;
                        }
                        this.processAsyncPageUpdate(section);
                        if (!this.documentHelper.isUpdatedWhileLoadAsync) {
                            this.documentHelper.performUpdateAfterLoad(true);
                        ...
                        const timeoutId = setTimeout(processRowAsync, 0);
                        if (this.documentHelper.currentDocumentLoadId) {
                            this.documentHelper.storeTimeoutId(this.documentHelper.currentDocumentLoadId, timeoutId as unknown as number);
                        }
                    } else {
    ...
    private updateClientAreaForWrapTable(tables: TableWidget[], table: TableWidget, beforeLayout: boolean, clientActiveAreaForTableWrap: Rect, clientAreaForTableWrap: Rect): void {
        if (beforeLayout) {
            ...
            if (table.wrapTextAround && table.bodyWidget) {
                if (tables.length == 1) {
                    if ((!isNullOrUndefined(table.previousRenderedWidget) && ((table.previousRenderedWidget) as BlockWidget).bodyWidget.page === table.bodyWidget.page) || table.isInHeaderFooter || table.isInsideTable) {
                        ...
                        if (!table.isLayouted && clientActiveArea.height < table.height && table.width >= clientActiveArea.width) {
                            let bodyWidget: BodyWidget = this.moveBlocksToNextPage(table as BlockWidget, false);
                            let curretTable: TableWidget = table;
                            //Move table to next page
                            if (curretTable.containerWidget && curretTable.containerWidget.childWidgets.indexOf(curretTable) !== -1) {
                                curretTable.containerWidget.childWidgets.splice(curretTable.containerWidget.childWidgets.indexOf(curretTable), 1);
                            }
                            if (bodyWidget.childWidgets.indexOf(curretTable) !== -1) {
                                bodyWidget.childWidgets.splice(bodyWidget.childWidgets.indexOf(curretTable), 1);
    ...
    public updateWidgetsToPage(tables: TableWidget[], rows: TableRowWidget[], table: TableWidget, rearrangeRow:boolean, endRowWidget?: TableRowWidget): void {
        ...
        if (table.bodyWidget instanceof HeaderFooterWidget && !table.wrapTextAround) {
            var container = table.containerWidget;
            var isHeaderFooter = container && container instanceof HeaderFooterWidget;
            if (isHeaderFooter && this.documentHelper && this.documentHelper.compatibilityMode === 'Word2003') {
                const container: any = table.containerWidget;
                const pageHeight = (container && container.page) ? container.page.boundingRectangle.height : container.height;
                const maxAllowed = pageHeight * 0.5;
                if ((container.height + table.height <= maxAllowed && !this.skip2003) || table.containerWidget.firstChild === table) {
                    container.height += table.height;
                    table.isRender = true;
                } else {
                    this.viewer.clientActiveArea.y -= table.height;
                    this.skip2003 = true;
                    table.isRender = false;
                    if(this.istableYpositionChange){
                            this.viewer.clientActiveArea.y = this.textWrappingBottom
                            this.istableYpositionChange = false;
                    }
                }
            } else {
                table.containerWidget.height += table.height;
    ...
    private splitWidget(paragraphWidget: ParagraphWidget, viewer: LayoutViewer, previousBodyWidget: BodyWidget, index: number, isPageBreak: boolean, footWidget?: BodyWidget[], isColumnBreak?: boolean, footHeight?: number): boolean {
        ...
        if (viewer.clientActiveArea.height >= maxElementHeight + height && !isPageBreak && !isColumnBreak) {
            ...
            if (previousBodyWidget !== paragraphWidget.containerWidget && !this.skipUpdateContainerWidget) {
                this.updateContainerWidget(paragraphWidget, previousBodyWidget, index, true, footWidget);
            }
            ...
            if (isNullOrUndefined(splittedWidget.containerWidget) && splittedWidget.childWidgets.length > 0) {
                ...
                if (prevPage !== nextBodyWidget.page) {
                    ...
                    const footWidgets: BodyWidget[] = this.getFootNoteWidgetsOf(splittedWidget);
                    if (nextBodyWidget != previousBodyWidget) {
                        this.moveFootNotesToPage(footWidgets, previousBodyWidget, nextBodyWidget);
                    }
    ...
    public reLayoutLine(paragraph: ParagraphWidget, lineIndex: number, isBidi: boolean, isSkip?: boolean, isSkipList?: boolean): void {
        ...
        else if (lineToLayout.children.length === 0 && lineToLayout.isLastLine() && !isNullOrUndefined(lineToLayout.previousLine) && lineToLayout.previousLine.isEndsWithLineBreak) {
            this.updateClientAreaForLine(lineToLayout);
            this.layoutEmptyLineWidget(paragraph, true, lineToLayout);
            this.updateWidgetToPage(this.viewer, paragraph);
        }else {
            this.updateClientAreaForLine(lineToLayout);
            ...
            if (this.documentHelper.viewer instanceof WebLayoutViewer) {
                ...
                if (!(table.containerWidget instanceof TextFrame) && !table.isInsideTable) {
                    ...
                    if (position.horizontalAlignment === 'Center') {
                        const cellWidth: number = ownerCell.width;
                        const centerLeft: number = ownerCell.x + ((cellWidth - tableTotalWidth) / 2);
                        const minLeft: number = ownerCell.x + ownerCell.margin.left;
                        if (tableTotalWidth > cellWidth) {
                            this.viewer.clientActiveArea.x = minLeft;
                        } else {
                            this.viewer.clientActiveArea.x = Math.max(minLeft, centerLeft);
                        }
