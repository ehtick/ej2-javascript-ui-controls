...
export class CommentView {
    ...
    private preventKeyboardShortcuts(event: KeyboardEvent): void {
        // Normalize key code using project helper (handles `key`/`code` differences)
        const key: number = HelperMethods.getLegacyKeyCode(event as any);
        const ctrl: boolean = !!(event.ctrlKey || event.metaKey);
        const shift: boolean = !!event.shiftKey;
        const alt: boolean = !!event.altKey;

        // Define whether to prevent the shortcut
        let prevent = false;

        if (ctrl && key === 66) prevent = true; // Ctrl/Cmd + B (Bold)
        else if (ctrl && key === 73) prevent = true; // Ctrl/Cmd + I (Italic)
        else if (ctrl && key === 85) prevent = true; // Ctrl/Cmd + U (Underline)
        else if (alt && shift && key === 53) prevent = true; // Alt + Shift + 5 (Strikethrough)
        else if (ctrl && key === 190) prevent = true; // Ctrl/Cmd + . (Superscript)
        else if (ctrl && key === 188) prevent = true; // Ctrl/Cmd + , (Subscript)
        else if (ctrl && shift && key === 76) prevent = true; // Ctrl/Cmd + Shift + L (Unordered / Align Left)
        else if (ctrl && shift && key === 55) prevent = true; // Ctrl/Cmd + Shift + 7 (Ordered list)
        else if (ctrl && key === 220) prevent = true; // Ctrl/Cmd + \ (Remove formatting)
        else if (ctrl && shift && key === 69) prevent = true; // Ctrl/Cmd + Shift + E (Align Center)
        else if (ctrl && shift && key === 82) prevent = true; // Ctrl/Cmd + Shift + R (Align Right)
        else if (ctrl && shift && key === 74) prevent = true; // Ctrl/Cmd + Shift + J (Align Justify)
        else if (shift && key === 9) prevent = true; // Shift + Tab (Outdent)
        else if (ctrl && alt && key >= 49 && key <= 54) prevent = true; // Ctrl/Cmd + Alt + 1..6 (Headings)
        else if (ctrl && shift && key === 81) prevent = true; // Ctrl/Cmd + Shift + Q (Quote)
    ...
    private postReply(): void {
        ...
        this.owner.editorModule.replyComment(this.comment, replyText, this.itemData, undefined, replyHtmlText);
        if (this.itemData) {
            this.itemData = [];
        }
