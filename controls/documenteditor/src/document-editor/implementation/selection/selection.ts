...
export class Selection {
    ...
    */
    ...
    public select(selectionSettings: SelectionSettings | string, startOrEnd?: string): void {
        if (typeof (selectionSettings) === 'string') {
            ...
            const pageCoordinates: Point = this.viewer.findFocusedPage(point, true, false, this.isPageUpAndDown);
     ...
     * @returns {void}
     */
    public selectContentControl(contentControl: ContentControl): void {
        if (contentControl) {
    ...
    */
     ...
     * @private
     */
    public getTableCellWidget(widget: TableRowWidget, point: Point): TableCellWidget {
        for (let i: number = 0; i < widget.childWidgets.length; i++) {
            let cellSpacing: number = 0;
            ...
            if ((widget.childWidgets[i] as TableCellWidget).x -
                ...
                if ((widget.childWidgets[i] as TableCellWidget).childWidgets.length > 0) {
                    return widget.childWidgets[i] as TableCellWidget;
                } else if (this.isPageUpAndDown) {
                    for (let j: number = 0; j < widget.childWidgets.length; j++) {
                        if ((widget.childWidgets[j] as TableCellWidget).childWidgets.length > 0) {
                            return widget.childWidgets[j] as TableCellWidget;
                        }
                    }
                }
            }
    ...
    public getLineWidgetRowWidget(widget: TableRowWidget, point: Point): LineWidget {
        let cell: TableCellWidget = this.getTableCellWidget(widget, point);
        if (cell) {
            return this.getLineWidgetCellWidget(cell, point);
        }
        let lineWidget: LineWidget = undefined;
    ...
    public getLineWidgetCellWidget(widget: TableCellWidget, point: Point): LineWidget {
        ...
        if (widget.childWidgets.length > 0) {
            ...
            } else if (this.isPageUpAndDown && widget.y < point.y && (widget.y + widget.height > point.y)) {
                this.updateYPointPosition(widget.childWidgets[0] as Widget, point);
                if ((widget.childWidgets[0] as Widget) instanceof ParagraphWidget) {
                    return this.getLineWidgetParaWidget((widget.childWidgets[0] as ParagraphWidget), point);
                } else {
                    return this.getLineWidgetTableWidget((widget.childWidgets[0] as TableWidget), point);
                }
            }
        }
        return lineWidget;
     ...
     * @private
     */
    public updateYPointPosition(widget: Widget, point: Point): void {
        if (widget instanceof ParagraphWidget) {
            point.y = widget.y;
        } else if (widget instanceof TableWidget && widget.firstChild instanceof TableRowWidget) {
            let cell: TableCellWidget = this.documentHelper.selection.getTableCellWidget(widget.firstChild, point);
            if (cell) {
                let firstWidget: Widget = cell.firstChild as Widget;
                if (firstWidget instanceof ParagraphWidget) {
                    this.updateYPointPosition(firstWidget, point);
                } else if (firstWidget instanceof TableWidget) {
                    this.updateYPointPosition(firstWidget, point);
                }
            }
        }
    ...
    public setHyperlinkContentToToolTip(fieldBegin: FieldElementBox, widget: LineWidget, xPos: number, isFormField?: boolean): void {
        if (fieldBegin) {
            ...
            if (isFormField) {
                ...
                this.toolTipTextElement.innerText = '';
            } else {
    ...
    */
    ...
    public onKeyDownInternal(event: KeyboardEvent, ctrl: boolean, shift: boolean, alt: boolean): void {
        let key: number = HelperMethods.getLegacyKeyCode(event);
        this.owner.focusIn();
        if (ctrl && !shift && !alt) {
        ...
        if (this.isFormField() && !(this.documentHelper.isDocumentProtected)) {
            ...
            if (formField && (formField as FieldElementBox).formFieldData instanceof DropDownFormField) {
                ...
                formField = (key === 37 || key === 38 || key === 40) ? formField : formField.nextElement instanceof BookmarkElementBox ? formField.nextElement.reference : (formField as FieldElementBox).fieldEnd;
                let index: number = key === 39 ? 1 : 0;
    ...
    public checkContentControlLocked(checkFormat?: boolean): boolean {
        ...
        for (let i: number = 0; i < this.documentHelper.contentControlCollection.length; i++) {
            ...
            if ((start.isExistAfter(cCstart) || start.isAtSamePosition(cCstart))
                ...
                if (this.owner.editorModule && contentControlStart.contentControlProperties.xmlMapping
