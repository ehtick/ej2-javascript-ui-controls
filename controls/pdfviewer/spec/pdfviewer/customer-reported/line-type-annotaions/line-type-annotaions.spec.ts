...
describe('PDF_Viewer_Line_Type_Line_Arrow', () => {
    ...
    beforeAll((done) => {
        const element: HTMLElement = createElement('div', { id: 'pdfviewer_lineType_line_arrow' });
        ...
        pdfviewer_lineType.appendTo('#pdfviewer_lineType_line_arrow');
    });
    // Cleanup PdfViewer instance after all tests complete
    afterAll(() => {
        if (pdfviewer_lineType) {
            pdfviewer_lineType.destroy();
            const el = document.getElementById('pdfviewer_lineType_line_arrow');
    ...
    it('1014478- VertexPoints and bounds are updated properly after resizing Line annotation and persist through export/import', async () => {
        try {
            const target = getTarget('#pdfviewer_lineType_line_arrow_textLayer_0');
...
describe('PDF_Viewer_Line_Type_Polygon_Distance', () => {
    let pdfviewer_lineType: PdfViewer = null;
    // Inject required PdfViewer modules
    PdfViewer.Inject(
        Toolbar, Magnification, Navigation, LinkAnnotation, ThumbnailView, BookmarkView,
    ...
    // Setup PdfViewer instance before running tests
    beforeAll((done) => {
        const element: HTMLElement = createElement('div', { id: 'pdfviewer_lineType_polygon_distance' });
        document.body.appendChild(element);
        pdfviewer_lineType = new PdfViewer({
            resourceUrl: window.location.origin + '/base/src/pdfviewer/ej2-pdfviewer-lib',
            documentPath: "data:application/pdf;base64," + EMPTY_PDF_B64
        ...
        pdfviewer_lineType.documentLoad = () => done();
        pdfviewer_lineType.appendTo('#pdfviewer_lineType_polygon_distance');
    });
    // Cleanup PdfViewer instance after all tests complete
    afterAll(() => {
        if (pdfviewer_lineType) {
            pdfviewer_lineType.destroy();
            const el = document.getElementById('pdfviewer_lineType_polygon_distance');
            if (el && el.parentNode) { el.parentNode.removeChild(el); }
            pdfviewer_lineType = null;
        }
    });
    ...
    it('1014478- VertexPoints and bounds are updated properly after resizing Polygon annotation and persist through export/import', async () => {
        try {
            const target = getTarget('#pdfviewer_lineType_polygon_distance_textLayer_0');
            const rect = target.getBoundingClientRect();
...
describe('PDF_Viewer_Line_Type_Perimeter_Area', () => {
    let pdfviewer_lineType: PdfViewer = null;
    // Inject required PdfViewer modules
    PdfViewer.Inject(
        Toolbar, Magnification, Navigation, LinkAnnotation, ThumbnailView, BookmarkView,
    ...
    // Setup PdfViewer instance before running tests
    beforeAll((done) => {
        const element: HTMLElement = createElement('div', { id: 'pdfviewer_lineType_perimeter_area' });
        document.body.appendChild(element);
        pdfviewer_lineType = new PdfViewer({
            resourceUrl: window.location.origin + '/base/src/pdfviewer/ej2-pdfviewer-lib',
            documentPath: "data:application/pdf;base64," + EMPTY_PDF_B64
        ...
        pdfviewer_lineType.documentLoad = () => done();
        pdfviewer_lineType.appendTo('#pdfviewer_lineType_perimeter_area');
    });
    // Cleanup PdfViewer instance after all tests complete
    afterAll(() => {
        if (pdfviewer_lineType) {
            pdfviewer_lineType.destroy();
            const el = document.getElementById('pdfviewer_lineType_perimeter_area');
            if (el && el.parentNode) { el.parentNode.removeChild(el); }
            pdfviewer_lineType = null;
        }
    });
    ...
    it('1014478- VertexPoints and bounds are updated properly after resizing Perimeter annotation and persist through export/import', async () => {
        try {
            const target = getTarget('#pdfviewer_lineType_perimeter_area_textLayer_0');
...
describe('PDF_Viewer_Line_Type_Volume', () => {
    let pdfviewer_lineType: PdfViewer = null;
    // Inject required PdfViewer modules
    PdfViewer.Inject(
        Toolbar, Magnification, Navigation, LinkAnnotation, ThumbnailView, BookmarkView,
    ...
    // Setup PdfViewer instance before running tests
    beforeAll((done) => {
        const element: HTMLElement = createElement('div', { id: 'pdfviewer_lineType_volume' });
        document.body.appendChild(element);
        pdfviewer_lineType = new PdfViewer({
            resourceUrl: window.location.origin + '/base/src/pdfviewer/ej2-pdfviewer-lib',
            documentPath: "data:application/pdf;base64," + EMPTY_PDF_B64
        ...
        pdfviewer_lineType.documentLoad = () => done();
        pdfviewer_lineType.appendTo('#pdfviewer_lineType_volume');
    });
    // Cleanup PdfViewer instance after all tests complete
    afterAll(() => {
        if (pdfviewer_lineType) {
            pdfviewer_lineType.destroy();
            const el = document.getElementById('pdfviewer_lineType_volume');
            if (el && el.parentNode) { el.parentNode.removeChild(el); }
            pdfviewer_lineType = null;
        }
    });
    it('1014478- VertexPoints and bounds are updated properly after resizing Volume annotation and persist through export/import', async () => {
        try {
            const target = getTarget('#pdfviewer_lineType_volume_textLayer_0');
            const rect = target.getBoundingClientRect();
