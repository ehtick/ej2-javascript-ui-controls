...
export class StickyNotesAnnotation {
    ...
    private modifyStatusProperty(text: string, statusElement: any): void {
        let currentAnnotation: any;
        let commentsDiv: HTMLElement = null;
        ...
        if (currentAnnotation && currentAnnotation.annotName !== '') {
            commentsDiv = document.getElementById(currentAnnotation.annotName);
            pageNumber = parseInt(commentsDiv.accessKey.split('_')[0], 10);
            pageIndex = pageNumber - 1;
            author = commentsDiv.getAttribute('author');
        } else {
            let id: any = statusElement.id;
            const childNodes: any = statusElement.parentElement.childNodes;
            const replyCount: number = Array.from(childNodes)
                .filter((node: any) => node.classList.contains('e-pv-reply-div')).length;
            if (replyCount > 0 && statusElement.className.includes('e-pv-reply-div')) {
                id = statusElement.parentElement.firstChild.id;
            }
            let lastValue: any = id.split('_').pop();
            if (this.pdfViewer.annotationModule && this.pdfViewer.annotationModule.actionCollection.length > 0) {
                const deleteCount: number = this.pdfViewer.annotationModule.actionCollection.filter((item: any) => item.action === 'Delete').length;
                lastValue = parseFloat(lastValue) - deleteCount;
            }
            currentAnnotation = this.pdfViewer.annotationCollection[lastValue as number];
            commentsDiv = document.getElementById(currentAnnotation.annotationId);
            pageNumber = parseInt(commentsDiv.accessKey.split('_')[0], 10);
            pageIndex = pageNumber - 1;
            author = commentsDiv.getAttribute('author');
        }
        if (currentAnnotation) {
            if (currentAnnotation.measureType && currentAnnotation.measureType !== '' || (!isNullOrUndefined(currentAnnotation.calibrate))) {
                pageAnnotations = this.getAnnotations(pageIndex, null, 'shape_measure');
                isMeasure = true;
            } else {
                pageAnnotations = this.getAnnotations(pageIndex, null, currentAnnotation.shapeAnnotationType);
