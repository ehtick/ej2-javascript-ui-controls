    ...
    public showToolbarItem(items: (CustomToolbarItemModel | ToolbarItem)[], isVisible: boolean): void {
        for (let i: number = 0; i < items.length; i++) {
            ...
            case 'OrganizePagesTool':
                ...
                break;
            default: {
                const item: any = (items[parseInt(i.toString(), 10)]);
                let element: HTMLElement;
                let toolbarItemElement: HTMLElement;
                if (item.template && typeof item.template === 'string') {
                    ...
                    if (element) {
                        toolbarItemElement = element.parentElement.parentElement;
                        if (toolbarItemElement && !isNullOrUndefined(this.toolbar)) {
                            this.toolbar.hideItem(toolbarItemElement, !isVisible);
                        }
                    }
                }
                else if (item.id || typeof item.template === 'object') {
                    let id: string;
                    if (typeof item.template === 'object' && !isNullOrUndefined(item.template.id)) {
                        id = item.template.id;
                    }
                    else {
                        id = item.id;
                    ...
                    if (element) {
                        toolbarItemElement = element.parentElement;
                        const hasIcon: any = document.querySelector(`#${id}`).closest('.e-control-wrapper');
                        if (!isNullOrUndefined(hasIcon)) {
                            toolbarItemElement = toolbarItemElement.parentElement;
                        }
                        if (toolbarItemElement && !isNullOrUndefined(this.toolbar)) {
                            this.toolbar.hideItem(toolbarItemElement, !isVisible);
                        }
                ...
                else if (!isNullOrUndefined(item.prefixIcon)) {
                    const className: string = item.prefixIcon.replace(/\s+/g, '.');
                    element = document.querySelector(`.${className}`);
                    if (element && element.parentElement && element.parentElement) {
                        toolbarItemElement = element.parentElement.parentElement;
                    }
                    if (toolbarItemElement && !isNullOrUndefined(this.toolbar)) {
                        this.toolbar.hideItem(toolbarItemElement, !isVisible);
                ...
                if (toolbarItemElement) {
                    const hasSeparator: boolean =
                        toolbarItemElement.nextElementSibling && toolbarItemElement.nextElementSibling.classList.length > 0 &&
                        toolbarItemElement.nextElementSibling.classList.contains('e-separator');
                    if (hasSeparator) {
                        this.toolbar.hideItem(toolbarItemElement.nextElementSibling, !isVisible);
                    }
                }
                break;
            }
            ...
            default: {
                ...
                if (item.template && typeof item.template === 'string') {
                    ...
                    if (element) {
                        toolbarItemElement = element.parentElement.parentElement;
                        if (toolbarItemElement && !isNullOrUndefined(this.toolbar)) {
                            this.enableItems(toolbarItemElement, isEnable);
                        }
                    }
                }
                else if (item.id || typeof item.template === 'object') {
