...
export function processPathData(data: string): Object[] {
    ...
    if (arrayCollection.length > 0) {
        for (let i: number = 0; i < arrayCollection.length; i++) {
            const ob: any = arrayCollection[parseInt(i.toString(), 10)];
            let char: string = '';
            char = ob[0];
            ...
            case 'c':
                for (j = 1; j < (ob as Object[]).length; j++) {
                    collection.push({
                        command: char, x1: ob[parseInt(j.toString(), 10)], y1: ob[j + 1],
                        x2: ob[j + 2], y2: ob[j + 3], x: ob[j + 4], y: ob[j + 5]
                    });
                    j = j + 5;
                }
                break;
            case 's':
                for (j = 1; j < (ob as Object[]).length; j++) {
                    collection.push({ command: char, x2: ob[parseInt(j.toString(), 10)],
                        y2: ob[j + 1], x: ob[j + 2], y: ob[j + 3] });
                    j = j + 3;
                }
                break;
            case 'q':
                for (j = 1; j < (ob as Object[]).length; j++) {
                    collection.push({ command: char, x1: ob[parseInt(j.toString(), 10)], y1: ob[j + 1],
                        x: ob[j + 2], y: ob[j + 3] });
                    j = j + 3;
                }
                break;
...
/**
 * @param {number} cornerRadius - cornerRadius
...
export function pathSegmentCollection(collection: Object[]): Object[] {
    ...
    for (x = 0, y = 0, i = 0, length = collection.length; i < length; ++i) {
        const obj: Object = collection[parseInt(i.toString(), 10)];
...
export function transformPath(arr: Object[], sX: number, sY: number, s: boolean, bX: number, bY: number, iX: number, iY: number): string {
    ...
    for (x = 0, y = 0, i = 0, length = arr.length; i < length; ++i) {
        const obj: Object = arr[parseInt(i.toString(), 10)];
        const seg: PathSegment = obj;
...
/**
 * @param {PathSegment} segment - segment
 * @param {string} char - char
 * @param {PathSegment} obj - obj
 * @param {boolean} isScale - isScale
 * @param {number} sX - sX
 * @param {number} sY - sY
 * @private
 * @returns {Object} - Object
 ...
 * @param {number} scaleFactor - scaleFactor
 * @param {number} oldOffset - oldOffset
 * @param {number} newOffset - newOffset
 * @private
...
/**
 * @param {Object[]} arrayCollection - arrayCollection
 * @private
 * @returns {Object[]} - Object[]
 */
