...
import { PdfAnnotationFlag, PdfGraphics, PdfInkAnnotation, PdfPage, PdfAnnotation, PdfPen, PdfPath, Point, PdfColor } from '@syncfusion/ej2-pdf';
import { AnnotationStatus, PdfViewer, PdfViewerBase } from '../index';
...
export class SignatureBase {
            ...
            for (let i: number = 0; i < signatureDetails.length; i++) {
                ...
                if (pageData.length > 0) {
                    for (let p: number = 0; p < pageData.length; p++) {
                        if (jsonObject['action'] === 'ExportAnnotations' || pageData[parseInt(p.toString(), 10)].status === AnnotationStatus.NewlyAdded) {
                                    ...
                                    for (let j: number = 0; j < stampObjects.length; j++) {
                                        ...
                                        if (path === 'M') {
                                            ...
                                            if (graphics != null) {
                                                ...
                                                dataPath.addLine({ x: newPoint1[0], y: newPoint1[1] },
                                                                 { x: newPoint2[0], y: newPoint2[1] });
                                            }
                                            newPoint1 = newPoint2;
                                        }
     ...
     * @returns {void}
     */
    public updateSignatureData(jsonObject: { [key: string]: string }, loadedDocument: any): void {
        const formfields: FormFieldsBase = new FormFieldsBase(this.pdfViewer, this.pdfViewerBase);
        const signatureDetails: any = JSON.parse(jsonObject.signatureData);
        if (!isNullOrUndefined(signatureDetails)) {
            for (let i: number = 0; i < signatureDetails.length; i++) {
                ...
                if (pageData.length > 0) {
                    for (let p: number = 0; p < pageData.length; p++) {
                        if (pageData[parseInt(p.toString(), 10)].status === AnnotationStatus.ExistingModified) {
                            const data: any = pageData[parseInt(p.toString(), 10)];
                            const signatureType: string = Object.prototype.hasOwnProperty.call(data, 'shapeAnnotationType') && data['shapeAnnotationType'] !== null
                                ? data['shapeAnnotationType'].toString()
                                : null;
                            if (signatureType !== null && signatureType === 'SignatureText') {
                                formfields.updateFreeTextAnnotations(data, loadedDocument, true);
                            }
                            else if (signatureType !== null && signatureType === 'SignatureImage') {
                                formfields.updateImage(data, loadedDocument, true);
                            }
                            else {
                                const pageNumber: number = data.pageIndex;
                                const page: PdfPage = loadedDocument.getPage(pageNumber);
                                ...
                                const opacity: number = data.opacity;
                                const thickness: number = data.thickness;
                                const strokeColor: { [key: string]: number } = JSON.parse(data.strokeColor);
                                const color: PdfColor = {r: strokeColor.r, g: strokeColor.g, b: strokeColor.b};
                                let minimumX: number = -1;
                                let minimumY: number = -1;
                                let maximumX: number = -1;
                    ...
                    for (let p: number = 0; p < pageData.length; p++) {
                        if (jsonObject['action'] === 'ExportAnnotations' || pageData[parseInt(p.toString(), 10)].status === AnnotationStatus.NewlyAdded) {
                            ...
                            if (signatureType !== null && signatureType === 'SignatureText') {
                                ...
                                const inkAnnotation: PdfInkAnnotation =
                                new PdfInkAnnotation({ x: left, y: top, width: width, height: height }, linePoints);
                                let bound: Rect = new Rect();
                                bound = new Rect(inkAnnotation.bounds.x, (page.size.height -
                                    (inkAnnotation.bounds.y + inkAnnotation.bounds.height)), inkAnnotation.bounds.width,
                                                ...
                                                for (let z: number = 0; z < rotatedPoints._points.length; z += 2) {
                                                    graphicsPoints.push({
                                                        x: (rotatedPoints._points[parseInt(z.toString(), 10)].x - minimumX) /
                                                            newDifferenceX + left,
                                                        y: page.size.height -
                                                        (rotatedPoints._points[parseInt(z.toString(), 10)].y - minimumY) /
                                                            newDifferenceY - top
                                                    });
                                                }
     ...
     * @returns {void}
     */
    public updateSignatureAsAnnotatation(jsonObject: { [key: string]: string }, loadedDocument: any): void {
        const annotationRenderer: AnnotationRenderer = new AnnotationRenderer(this.pdfViewer, this.pdfViewerBase);
        const signatureDetails: any = JSON.parse(jsonObject.signatureData);
        if (!isNullOrUndefined(signatureDetails)) {
            for (let i: number = 0; i < signatureDetails.length; i++) {
                    ...
                    for (let p: number = 0; p < pageData.length; p++) {
                        if (pageData[parseInt(p.toString(), 10)].status === AnnotationStatus.ExistingModified) {
                            ...
                            const signatureType: string = Object.prototype.hasOwnProperty.call(signatureAnnotation, 'shapeAnnotationType') && signatureAnnotation['shapeAnnotationType'] !== null
                                ...
                                : null;
                            if (signatureType !== null && signatureType === 'SignatureText') {
                                formfields.updateFreeTextAnnotations(signatureAnnotation, loadedDocument, false);
                            }
                            else if (signatureType !== null && signatureType === 'SignatureImage') {
                                formfields.updateImage(signatureAnnotation, loadedDocument, false);
                            }
                            else {
                                const bounds: Rect = JSON.parse(signatureAnnotation.bounds);
                                const stampObjects: any = JSON.parse(signatureAnnotation.data);
                                const annotationIndex: number = signatureAnnotation.annotationIndex;
                                const pageNumber: number = signatureAnnotation.pageIndex;
                                const page: PdfPage = loadedDocument.getPage(pageNumber);
                                const annotation: PdfAnnotation = page.annotations.at(annotationIndex);
                                const cropValues: PointBase = annotationRenderer.getCropBoxValue(page, false);
                                const left: number = cropValues.x + this.convertPixelToPoint(bounds.left);
                                let top: number = this.convertPixelToPoint(bounds.top);
                                if (!(cropValues.x === 0 && (page.cropBox[2] === page.size.width && cropValues.y === page.size.height))) {
                                ...
                                const opacity: number = signatureAnnotation.opacity;
                                const thickness: number = signatureAnnotation.thickness;
                                const strokeColor: { [key: string]: number } = JSON.parse(signatureAnnotation.strokeColor);
                                const color: PdfColor = {r: strokeColor.r, g: strokeColor.g, b: strokeColor.b};
                                let minimumX: number = -1;
                                let minimumY: number = -1;
                                let maximumX: number = -1;
                                ...
                                if (annotation instanceof PdfInkAnnotation) {
                                    ...
                                    inkAnnotation.bounds = { x: left, y: top, width: width, height: height };
                                    inkAnnotation._points = linePoints;
                                    const x: number = inkAnnotation.bounds.x;
                                    const y: number = (page.size.height - (inkAnnotation.bounds.y + inkAnnotation.bounds.height));
                                    const inkWidth: number = inkAnnotation.bounds.width;
                                    const inkHeight: number = inkAnnotation.bounds.height;
                                    inkAnnotation.bounds = { x: x, y: y, width: inkWidth, height: inkHeight };
                                    inkAnnotation.color = color;
                                    linePoints = [];
                                    if (isNewValues > 0) {
                                        if (rotationAngle !== 0) {
                                                ...
                                                if (pointsCollections.length > 0) {
                                                    ...
                                                    for (let z: number = 0; z < rotatedPoints._points.length; z += 2) {
                                                        graphicsPoints.push({
                                                            x: (rotatedPoints._points[parseInt(z.toString(), 10)].x - minimumX) /
                                                                newDifferenceX + left,
                                                            y: page.size.height - (rotatedPoints._points[parseInt(z.toString(), 10)].y
