import { createElement, Browser, isNullOrUndefined, isBlazor, SanitizeHtmlHelper, ChildProperty, Property, initializeCSPTemplate } from '@syncfusion/ej2-base';
...
export class TextSearch {
    ...
    public createTextSearchBox(): void {
        ...
        const headerTemplate: any = initializeCSPTemplate(function (data: any): any {
            const total: number = (data && data.total != null) ? data.total : '';
            const template: any = `<div class="e-pv-automplete-header">
                <span class="e-pv-search-exact-matches">${this.pdfViewer.localeObj.getConstant('Exact Matches')}</span>
                <span class="e-pv-total-exact-matches">${total}</span>
                </div>`;
            return template;
        }, this);
        const itemTemplate: any = initializeCSPTemplate(function (data: any): any {
            const template: any = `<div class="e-pv-autocomplete-item">
                <span class="e-pv-autocomplete-word">${data.SearchString}</span>
                <span class="e-pv-autocomplete-count">${data.Count}</span>
                </div>`;
            return template;
        }, this);
    ...
    private calculateSearchCount(inputString: string, documentTextCollection: any): void {
        ...
        for (let i: number = 0; i < documentTextCollection.length; i++) {
            ...
            while (matchIndex !== 0 || (matchIndex === 0 && matches.length > 0 && matches[0] === 0)) {
                ...
                if (inputString.indexOf(' ') !== -1 && matchIndex !== -1) {
