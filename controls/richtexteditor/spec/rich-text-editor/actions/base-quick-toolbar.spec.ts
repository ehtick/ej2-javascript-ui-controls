...
describe('Base Quick Toolbar', ()=> {
    ...
    describe('Bug 984457: Inline toolbar doesn"t show properly for the texts with Images in RichTextEditor', () => {
        ...
        beforeAll(() => {
            rteObj = renderRTE({
                value: `<p><img alt="Editor Features Overview" src="https://cdn.syncfusion.com/ej2/richtexteditor-resources/RTE-Overview.png" width="400" height="200" class= "e-img-left e-rte-image e-img-inline"/>image</p>`,
                inlineMode: {
                    enable: true,
                    onSelection: true
                },
    ...
    describe('Task 1014637: Test and fix quick toolbar enableAppendToBody positioning and visibility issues', () => {
        ...
        beforeEach(() => {
            rteObj = renderRTE({
                value: `<p><img alt="Editor Features Overview" src="https://cdn.syncfusion.com/ej2/richtexteditor-resources/RTE-Overview.png" width="200" height="100" class="e-img-left e-rte-image e-img-inline"/> image</p><p><strong>Syncfusion editor</strong></p>`,
                inlineMode: {
                    enable: true,
                    onSelection: true
                },
                quickToolbarSettings: {
                    ...
                    text: ['Formats', '|', 'Bold', 'Italic', 'Fontcolor', 'BackgroundColor', '|', 'CreateLink', 'Image', 'CreateTable', '|', 'Unorderedlist', 'Orderedlist']
                }
            });
        });
        ...
        it('Should close inline toolbar after selecting image option from inline toolbar', (done: DoneFn) => {
            rteObj.focusIn();
            const mouseUpEvent: MouseEvent = new MouseEvent('mouseup', BASIC_MOUSE_EVENT_INIT);
            const mouseDownEvent: MouseEvent = new MouseEvent('mousedown', BASIC_MOUSE_EVENT_INIT);
            rteObj.inputElement.dispatchEvent(mouseDownEvent);
            const target: HTMLElement = rteObj.inputElement.querySelector('strong');
            setSelection(target.firstChild, 0, 4);
            target.dispatchEvent(mouseUpEvent);
            setTimeout(() => {
                const inlinePopup: HTMLElement = document.querySelector('.e-rte-inline-popup.e-popup-open') as HTMLElement;
                expect(inlinePopup).not.toBeNull();
                expect(inlinePopup.parentElement).toBe(rteObj.contentModule.getPanel().ownerDocument.body);
                const imageButton: HTMLElement = inlinePopup.querySelector('button[id*="Image"]') as HTMLElement;
                expect(imageButton).not.toBeNull();
                imageButton.click();
                setTimeout(() => {
                    expect(document.querySelector('.e-rte-inline-popup.e-popup-open')).toBeNull();
                    done();
                }, 100);
            }, 100);
        });

        it('Should keep all inline toolbar icons enabled after opening image quick toolbar', (done: DoneFn) => {
            rteObj.quickToolbarSettings.showOnRightClick = false;
            rteObj.focusIn();
            rteObj.inputElement.dispatchEvent(INIT_MOUSEDOWN_EVENT);
            const imageTarget: HTMLElement = rteObj.inputElement.querySelector('img');
            setCursorPoint(imageTarget, 0);
            imageTarget.dispatchEvent(MOUSEUP_EVENT);
            setTimeout(() => {
                const imagePopup: HTMLElement = document.querySelector('.e-rte-quick-popup[id*="Image_Quick_Popup"]') as HTMLElement;
                expect(imagePopup).not.toBeNull();
                expect(imagePopup.parentElement).toBe(rteObj.contentModule.getPanel().ownerDocument.body);
                const textTarget: HTMLElement = rteObj.inputElement.querySelector('strong');
                rteObj.inputElement.dispatchEvent(INIT_MOUSEDOWN_EVENT);
                setSelection(textTarget.firstChild, 0, 4);
                textTarget.dispatchEvent(MOUSEUP_EVENT);
                setTimeout(() => {
                    const inlinePopup: HTMLElement = document.querySelector('.e-rte-inline-popup.e-popup-open') as HTMLElement;
                    expect(inlinePopup).not.toBeNull();
                    const toolbarItems: NodeListOf<HTMLElement> = inlinePopup.querySelectorAll('.e-toolbar-item:not(.e-separator)');
                    expect(toolbarItems.length).toBeGreaterThan(0);
                    for (let i: number = 0; i < 10; i++) {
                        expect(toolbarItems[i].classList.contains('e-overlay')).toBe(false);
                        const button: HTMLButtonElement = toolbarItems[i].querySelector('button') as HTMLButtonElement;
                        if (button) {
