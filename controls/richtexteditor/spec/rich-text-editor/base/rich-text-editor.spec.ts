...
describe('RTE Base module ', () => {
    ...
    describe('Bug 975481: Font Color Fails to Retain on New Line and Script Error Occurs After Multiple Enter Key Presses ', () => {
        let rteObj: RichTextEditor;
        let keyboardEventArg: any;
        beforeAll((done: Function) => {
                    rteObj = renderRTE({
                        toolbarSettings: {
                            items: ['FontColor']
                        },
                        value: `<h1>Testing 1</h1>`,
                    });
            keyboardEventArg = {
                preventDefault: () => { },
                stopPropagation: () => { },
        ...
        it(' pressing enter twice after selecting font color', (done: Function) => {
            const node: any = (rteObj as any).inputElement.childNodes[0].firstChild;
            setCursorPoint(document, node, 0);
    ...
    describe('EJ2-44314: Improvement with backSpaceKey action in the Rich Text Editor', () => {
        ...
        it('Checking the LI element with blocknodes', (done: Function) => {
            rteObj = renderRTE({
                value: `<p><b>Functional Specifications/Requirements:</b></p><ol><li><p>Provide the tool bar support, it\u2019s also customizable.</p></li><li><p>Options to get the HTML elements with styles.</p></li><li><p>Support to insert image from a defined path.</p></li></ol></p>`,
            });
            let node: any = (rteObj as any).inputElement.childNodes[1].childNodes[1];
            rteObj.formatter.editorManager.nodeSelection.setSelectionText(document, node, node, 0, 1);
            keyBoardEvent.keyCode = 8;
            keyBoardEvent.code = 'Backspace';
            (rteObj as any).keyDown(keyBoardEvent);
    ...
    describe('EJ2-44314: Improvement with backSpaceKey action in the Rich Text Editor', () => {
        ...
        it('Checking the LI element with text nodes', (done: Function) => {
            ...
            Specifications/Requirements:</b></p><ol><li>Provide
            the tool bar support, it’s also customizable.</li><li>Options
            to get the HTML elements with styles.</li><li>Support
            to insert image from a defined path.</li><li>Footer
            elements and styles(tag / Element information , Action button (Upload, Cancel))</li></ol><p></p>`,
            });
    ...
    describe('EJ2-50975: Improvement with deleteKey action in the Rich Text Editor', () => {
        ...
        it('Checking the deletekey between two nodes with child elements', (done: Function) => {
            ...
            let node: any = rteObj.inputElement.childNodes[1].childNodes[0].firstChild;
            setCursorPoint(document, node, rteObj.inputElement.childNodes[1].childNodes[0].textContent.length);
            (rteObj as any).mouseUp({ target: rteObj.inputElement, isTrusted: true });
            keyBoardEvent.keyCode = 46;
            keyBoardEvent.code = 'Delete';
            (rteObj as any).keyDown(keyBoardEvent);
            setTimeout(() => {
                expect(rteObj.inputElement.childNodes[1].childNodes[0].textContent === "Provide the tool bar support, it’s also customizable.Options to get the HTML elements with styles.").toBe(true);
                done();
            }, 100);
        });
        afterAll((done) => {
    ...
    describe('EJ2-50975: Improvement with deleteKey action in the Rich Text Editor', () => {
        ...
        it('Checking the deletekey between two nodes one with child elements and other with no child', (done: Function) => {
            ...
            (rteObj as any).keyDown(keyBoardEvent);
            setTimeout(() => {
                expect(rteObj.inputElement.childNodes[1].childNodes[1].textContent === "Options to get the HTML elements with styles.Support to insert image from a defined path.").toBe(true);
                done();
            }, 100);
        });
        afterAll((done) => {
    ...
    describe('EJ2-57112: Delete Key not working when image is focused and deleted', () => {
        ...
        it('Delete Key not working when image is focused and deleted', (done: Function) => {
            rteObj = renderRTE({
                value: `<ol><li><p>image</p></li></ol><p class="focusNode"><img alt="Logo" src="https://ej2.syncfusion.com/angular/demos/assets/rich-text-editor/images/RTEImage-Feather.png" class="e-rte-image e-img-inline" style="width: 300px;"></p><p>Content</p>`,
            });
            let node: any = rteObj.inputElement.querySelector('.focusNode');
            ...
            (rteObj as any).keyDown(keyBoardEvent);
            setTimeout(() => {
                expect((rteObj as any).inputElement.innerHTML === `<ol><li>image</li></ol><p class="focusNode"><img alt="Logo" src="https://ej2.syncfusion.com/angular/demos/assets/rich-text-editor/images/RTEImage-Feather.png" class="e-rte-image e-img-inline" style="width: 300px;"></p><p>Content</p>`).toBe(true);
                done();
            }, 100);
        });
        afterAll((done) => {
    ...
    describe('RTE base module', () => {
        ...
        describe('Bug 978166: To give shortcut support for print in RTE', () => {
            let rteObj: RichTextEditor;
            ...
            it(' checking shortcut key triggers the print action in RTE', function () {
                rteObj.focusIn();
                keyboardEventArgs.action = 'print';
                keyboardEventArgs.ctrlKey = true;
                keyboardEventArgs.keyCode = 80;
                (<any>rteObj).keyDown(keyboardEventArgs);
                expect(requestType === 'print').toBe(true);
            });
        ...
        describe('EJ2-49452- Disable toolbar when quicktoolbar is opened', () => {
            ...
            beforeAll(() => {
                rteObj = renderRTE({
                    ...
                    value: '<p><img src="https://media.sproutsocial.com/uploads/2017/02/10x-featured-social-media-image-size.png" class="e-resize e-rte-image e-img-inline"></p>'
                });
                ...
                QTBarModule.imageQTBar.showPopup(trg, null);
            });
        ...
        describe('Toolbar - Print Module', () => {
            describe('Print rendering testing', () => {
                ...
                it('with cancel is false', () => {
                    expect(rteEle.querySelectorAll(".e-toolbar-item")[3].getAttribute("title")).toBe("Print (Ctrl+P)");
        ...
        describe('RTE Properties', () => {
            ...
            it('Ensure placeholder property', () => {
                expect(((rteObj as any).placeHolderWrapper as HTMLElement).classList.contains('e-placeholder-enabled')).toBe(true);
            ...
            it('ensure through onproperty change - placeholder', () => {
                ...
                rteObj.dataBind();
                expect(((rteObj as any).placeHolderWrapper as HTMLElement).classList.contains('e-placeholder-enabled')).toBe(false);
        ...
        describe("987796- Apply fontFamily for the empty text inbetween the inline elements.", () => {
            let rteObj: RichTextEditor;
            beforeAll(() => {
                rteObj = renderRTE({
                    value: `<h2><span style="color: rgb(255, 0, 0); text-decoration: inherit;"><span style="background-color: rgb(255, 255, 0);"><strong>Welcome to the </strong></span></span><span style="color: rgb(47, 84, 150); text-decoration: inherit;"><span style="background-color: rgb(255, 255, 0);"><strong>Syncfusion</strong></span></span><span style="color: rgb(255, 0, 0); text-decoration: inherit;"><span style="background-color: rgb(255, 255, 0);"><sup><strong>®</strong></sup></span></span><span style="color: rgb(255, 0, 0); text-decoration: inherit;"><span style="background-color: rgb(255, 255, 0);"><strong> </strong></span></span><span style="color: rgb(83, 129, 53); text-decoration: inherit;"><span style="background-color: rgb(255, 255, 0);"><strong>Rich Text Editor</strong></span></span></h2>`,
                    toolbarSettings: {
                        items: ['FontName', 'FontSize']
                    }
                });
            ...
            it('Apply fontFamily for the empty text inbetween the inline elements.', (done: Function) => {
                rteObj.focusIn();
                let startText: HTMLElement = rteObj.inputElement.querySelector('h2').firstChild.firstChild.firstChild.firstChild as HTMLElement;
                let endText: HTMLElement = rteObj.inputElement.querySelector('h2').lastChild.firstChild.firstChild.firstChild as HTMLElement;
                let sel = new NodeSelection().setSelectionText(document, startText, endText, 0, endText.textContent.length);
                ((rteObj.element.querySelectorAll(".e-toolbar-item")[0] as HTMLElement).querySelector('button') as HTMLButtonElement).click();
                ((document.querySelector('.e-font-name-tbar-btn ul') as HTMLElement).childNodes[1] as HTMLElement).click();
                setTimeout(() => {
                    expect((rteObj.inputElement.querySelector('h2').childNodes[3].childNodes[0] as HTMLElement).style.fontFamily === '"Segoe UI"').toBe(true);
                    expect((rteObj.inputElement.querySelector('h2').childNodes[3].childNodes[0] as HTMLElement).textContent === ' ').toBe(true);
                    done();
                }, 10);
            });
        });
...
<ul class='ul-third-node'><li>one-node</li><li>two-node</li><li>three-node</li></ul>`;
    ...
    describe('Value property when xhtml is enabled', function () {
        ...
        it("value property checking when xhtml is enabled", function () {
            expect(rteObj.value).toBe('<div><p>ad<br/></p><hr/><p>asd</p><p><br/></p></div>');
    ...
    describe('XHTML validation', function () {
        ...
        it("ImageTags", function () {
            ...
            rteObj.enableXhtml = true;
            rteObj.dataBind();
            expect(rteObj.inputElement.innerHTML).toBe('<p><img src="image.jpg" class="e-rte-image e-img-inline" alt=""></p><p>dfg<img src="image.jpg" class="e-rte-image e-img-inline" alt=""> ds</p>');
        });
...
<p>The Editor can integrate with the Syncfusion<sup>®</sup> Image Editor to crop, rotate, annotate, and apply filters to images. Check out the demos <a class="e-rte-anchor" title="Image Editor Demo" href="https://ej2.syncfusion.com/javascript/demos/#/material/rich-text-editor/image-editor-integration.html" aria-label="Open in new window">here</a>.</p>`
    ...
    describe('983776: MAC Editor Toolbar items tooltip Keyboard shortcut value is not having a space character.', function () {
        let rteEle: HTMLElement;
        let rteObj: any;
        ...
        it(' To check correct shortcut key is displayed in tooltip for mac', (done: Function) => {
            const title = document.querySelector('.e-toolbar-item.e-tbtn-align').getAttribute('title');
            expect(title === 'Bold (Ctrl+B)' || title === 'Bold (⌘ B)').toBe(true);
            done();
        });
    });
    ...
    describe('969416: Verify saving the contents in editor only after content changes', () => {
        let rteObj: RichTextEditor;
        beforeAll(() => {
            rteObj = renderRTE({
        ...
        it('should not call updateValueOnIdle on initial focus', (done: Function) => {
            const updateValueOnIdleSpy = spyOn<any>(rteObj, 'updateValueOnIdle');
            rteObj.focusIn();
            setTimeout(() => {
                expect(updateValueOnIdleSpy).not.toHaveBeenCalled();
                updateValueOnIdleSpy.calls.reset();
                rteObj.focusIn();
                setTimeout(() => {
                    expect(updateValueOnIdleSpy).not.toHaveBeenCalled();
                    done();
    ...
    describe('969416: Verify saving the contents in editor only after content changes', () => {
        ...
        it('should call updateValueOnIdle after applying bold formatting', (done: Function) => {
            const updateValueOnIdleSpy = spyOn<any>(rteObj, 'updateValueOnIdle');
            let node = rteObj.inputElement.querySelector('p').childNodes[0];
            let sel = new NodeSelection().setSelectionText(document, node, node, 0, 4);
            rteObj.executeCommand('bold');
            setTimeout(() => {
                expect(updateValueOnIdleSpy).toHaveBeenCalled();
                done();
            }, 400);
        });
        afterAll((done) => {
    ...
    describe("Null or undefined value testing", () => {
        ...
        it("quickToolbarSettings", () => {
            ...
            rteObj.appendTo('#rteTarget');
            expect(rteObj.quickToolbarSettings.actionOnScroll).toBe('none');
            rteObj.destroy();
            ...
            rteObj.appendTo('#rteTarget');
            expect(rteObj.quickToolbarSettings.image.length === 15).toBe(true);
            rteObj.destroy();
            ...
            rteObj.appendTo('#rteTarget');
            expect(rteObj.quickToolbarSettings.table.length === 10).toBe(true);
            rteObj.destroy();
            ...
            rteObj.appendTo('#rteTarget');
            expect(rteObj.quickToolbarSettings.video.length === 6).toBe(true);
            rteObj.destroy();
        });
        it("readonly", () => {
    ...
    describe('916913: After inserting a video into the table, the main toolbar is still enabled.', () => {
        ...
        beforeEach((done: Function) => {
            ...
            trg.dispatchEvent(clickEvent);
            QTBarModule = getQTBarModule(rteObj);
            QTBarModule.audioQTBar.showPopup(trg, null);
            done();
        });
    ...
    describe('916913: After inserting a video into the table, the main toolbar is still enabled.', () => {
        ...
        beforeEach((done: Function) => {
            ...
            trg.dispatchEvent(clickEvent);
            QTBarModule = getQTBarModule(rteObj);
            QTBarModule.videoQTBar.showPopup(trg, null);
            done();
        });
        afterEach((done: Function) => {
            destroy(rteObj);
    ...
    describe('981097: Dynamic media/table resize enable/disable behavior', () => {
        ...
        it('Image: resizer visible initially, hides after disabling insertImageSettings.resize dynamically', (done: DoneFn) => {
            rteObj = renderRTE({
                value: '<p><img alt="Sky with sun" src="https://cdn.syncfusion.com/ej2/richtexteditor-resources/RTE-Overview.png" style="width: 300px;" class="e-rte-image e-img-inline"></p>'
            });
            rteObj.focusIn();
    ...
    describe('975644: Script Error on Opening Markdown Editor Overview Sample in React Platform.', () => {
        let rteObj: RichTextEditor;
        let changetriggered: boolean = false;
        beforeAll(() => {
            rteObj = renderRTE({
                toolbarSettings: {
                    items: ['FontName', 'FontSize', 'FontColor', 'BackgroundColor']
        ...
        it('Check the change event is not triggered for the initial render', (done: Function) => {
            setTimeout(() => {
                setTimeout(() => {
    ...
    describe('975644: change event is not triggerd for markdown', () => {
        let rteObj: RichTextEditor;
        let changetriggered: boolean = false;
        beforeAll(() => {
            rteObj = renderRTE({
...
Rich Text Editor 3`
    ...
    describe('950768: Backward Slash Removal in an Editor', () => {
        let rteObj: RichTextEditor;
        let style: HTMLStyleElement;
        beforeAll(() => {
            let css: string = ".e-richtexteditor { white-space: pre-wrap; }";
            style = document.createElement('style');
            style.type = "text/css";
            style.id = "rteStyle";
            style.appendChild(document.createTextNode(css));
        ...
        it('Should not remove the backward slash from pre-wrap style', (done: Function) => {
            const result = rteObj.inputElement.innerHTML;
            const expected = `<p>\n     <strong>hello</strong>\n     <span>world</span>\n</p>`;
            expect(result === expected).toBe(true);
            done();
    ...
    describe('950768: Backward Slash Removal in an Editor', () => {
        ...
        let style: HTMLStyleElement;
        beforeAll(() => {
            let css: string = ".e-richtexteditor { white-space: pre-line; }";
            style = document.createElement('style');
            style.type = "text/css";
            style.id = "rteStyle";
            style.appendChild(document.createTextNode(css));
    ...
    describe('950768: Backward Slash Removal in an Editor', () => {
        ...
        let style: HTMLStyleElement;
        beforeAll(() => {
            let css: string = ".e-richtexteditor { white-space: pre; }";
            style = document.createElement('style');
            style.type = "text/css";
            style.id = "rteStyle";
            style.appendChild(document.createTextNode(css));
        ...
        it('Should not remove the backward slash from pre inline style', (done: Function) => {
            const result = rteObj.inputElement.innerHTML;
            const expected = `<p>\n     <strong>hello</strong>\n     <span>world</span>\n</p>`;
            expect(result === expected).toBe(true);
            done();
    ...
    describe('957549: Improve the re-structuring of the editor input value for editor', () => {
        ...
        beforeAll(() => {
            rteObj = renderRTE({
                value: `<div>Hello <span>World</span></div>`,
            });
        });
        it('Formatting while initial rendering, should not wrap the text with block element ', (done: Function) => {
            const result = rteObj.inputElement.innerHTML;
   ...
   <li>three</li>
    ...
    describe("Bug 980378: MaxLength restriction is not enforced in Code View mode", () => {
        let rteObj : RichTextEditor;
        beforeAll( () =>{
            rteObj = renderRTE({
                toolbarSettings: {
        ...
        it('click codeview and type the content', (done) => {
            rteObj.focusIn();
            ...
            setTimeout(() => {
                ...
                setTimeout(() => {
                    expect(rteObj.inputElement.textContent === `Hello`).toBe(true);
                    done();
                }, 150);
            }, 100);
        });
    ...
    describe("Bug 980378: MaxLength restriction is not enforced in Code View mode", () => {
        ...
        it('click codeview and paste the content', (done) => {
            rteObj.focusIn();
            let codeView: HTMLElement = <HTMLElement>document.body.querySelectorAll(".e-toolbar-items")[0].childNodes[0];
            codeView.click();
            setTimeout(() => {
                const textarea = rteObj.element.querySelector('.e-rte-srctextarea') as HTMLTextAreaElement;
                const clipBoardData: string = 'pasted content';
                const dataTransfer: DataTransfer = new DataTransfer();
    ...
    describe("Bug 980378: MaxLength restriction is not enforced in Code View mode", () => {
        ...
        it('click codeview and paste the content for the empty space case', (done) => {
            rteObj.focusIn();
            let codeView: HTMLElement = <HTMLElement>document.body.querySelectorAll(".e-toolbar-items")[0].childNodes[0];
            codeView.click();
            setTimeout(() => {
...
<p><em><br/></em></p>
    ...
    describe('960989 - Active formatting styles not reset after sending message in Bottom Toolbar sample', () => {
        ...
        it('Bold button should lose active state after value is null and focusIn', () => {
            rteObj.focusIn();
            const boldButton = rteObj.element.querySelector('.e-toolbar-item');
    ...
    describe('Bug 1014333: RTE beforeSanitizeHtml event triggered multiple times.', () => {
        let rteObj: RichTextEditor;
        let beforeSanitizeCount: number = 0;
        beforeAll(() => {
            rteObj = renderRTE({
                value: '<p>RTE Content</p><script>alert("xss")</script>',
                beforeSanitizeHtml: () => {
                    beforeSanitizeCount++;
                }
            });
