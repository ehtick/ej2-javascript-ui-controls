...
describe('AI Assistant Module', ()=> {
    ...
    describe('Bug 1014431: AI Assistant “Insert” behaves unpredictably after programmatic selectAll; whole-content replacement and subsequent partial replacements affect list items', () => {
        let editor: RichTextEditor;
        let requestCount: number;

        const executeFirstAICommand = (done: Function): void => {
            const button: HTMLButtonElement = editor.getToolbarElement().querySelector('.e-ai-commands-tbar-btn');
            button.click();
            setTimeout(() => {
                const menu: HTMLElement = document.querySelector('.e-dropdown-popup.e-ai-commands-tbar-btn .e-menu-parent') as HTMLElement;
                menu.querySelector('li').classList.add('e-focused');
                const mouseOverEvent: MouseEvent = new MouseEvent('mouseover', BASIC_MOUSE_EVENT_INIT);
                menu.querySelector('li').dispatchEvent(mouseOverEvent);
                setTimeout(() => {
                    const subMenu: HTMLElement = document.querySelectorAll('.e-menu-popup')[0] as HTMLElement;
                    subMenu.querySelector('li').click();
                    setTimeout(() => {
                        ...
                        setTimeout(() => {
                            ...
                            const listItems: NodeListOf<HTMLElement> = editor.inputElement.querySelectorAll('li') as NodeListOf<HTMLElement>;
                            expect(listItems[0].textContent.trim()).toBe('test1');
                            expect(listItems[1].textContent.trim()).toBe('test2');
                            expect(listItems[2].textContent.trim()).toBe('test3');
                            done();
                        }, 100);
                    }, 100);
                }, 100);
        ...
        beforeAll(() => {
            requestCount = 0;
            editor = renderRTE({
                value: `<p>Initial content to process.</p>`,
                toolbarSettings: {
                    items: ['aiquery', 'aicommands', '|', 'undo', 'redo']
                },
                aiAssistantSettings: {
                ...
                aiAssistantPromptRequest: () => {
                    ...
                    const fullText: string = requestCount === 1 ? '- test1 \n- test2 \n- test3' : 'updated test1';
                ...
                beforePopupOpen: (args: any) => {
                    if (args.type === 'AIAssistant') {
                        const selection: string = editor.getSelection().toString();
        ...
        it('should replace only the selected list item on second AI command insertion', (done: DoneFn) => {
            editor.focusIn();
            executeFirstAICommand(() => {
                const textNode: ChildNode = editor.inputElement.querySelectorAll('li')[0].firstChild as ChildNode;
                    ...
                    setTimeout(() => {
                        ...
                        setTimeout(() => {
                            ...
                            setTimeout(() => {
                                ...
                                expect(listItems[0].textContent.trim()).toBe('updated test1');
                                expect(listItems[1].textContent.trim()).toBe('test2');
                                expect(listItems[2].textContent.trim()).toBe('test3');
                                done();
                            }, 100);
