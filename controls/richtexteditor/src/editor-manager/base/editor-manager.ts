...
export class EditorManager {
    ...
    private onPropertyChanged(props: { [key: string]: Object }): void {
        if (props.module === 'enterAction') {
            this.enterAction = props.enterAction as EnterKey;
        } else {
    ...
    private isCaretAtStartOrAfterBrOrBlock(range: Range): boolean {
        // Find node immediately before caret in DOM order relative to the startContainer/offset
        let currentRange: Node = range.startContainer;
        let previousElem: Node = currentRange.previousSibling;
        ...
        if (previousElem && (previousElem.nodeName === 'BR' || isBlockNode(previousElem as Element))) {
            return true;
    ...
    public isAtStart(): boolean {
        ...
        if (this.enterAction !== 'BR') {
            const blockNode: Node = this.domNode.getImmediateBlockNode(range.startContainer);
            if (blockNode) {
                const domMethods: DOMMethods = new DOMMethods(this.editableElement as HTMLDivElement);
                const firstTextNode: Node = domMethods.getFirstTextNode(blockNode);
