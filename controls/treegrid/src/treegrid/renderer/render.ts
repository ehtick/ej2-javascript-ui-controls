...
export class Render {
    ...
    public RowModifier(args: RowDataBoundEventArgs): void {
        ...
        if (summaryRow) {
            ...
            const isDragandDropCell: Element = args.row.querySelector('.e-rowdragdrop') || args.row.querySelector('.e-rowdragdrop.e-dragging');
            if (isDragandDropCell) {
                (args.row as any).cells[0].className = 'e-rowcell e-summarycell';
                (args.row as any).cells[0].innerHTML = '';
            }
        }
