...
describe('Check duplicate data in Datasource', () => {
    let gridObj: TreeGrid;
    let actionComplete: () => void;
    beforeAll((done: Function) => {
        gridObj = createGrid(
    ...
    it('Click Add - no selection', (done: Function) => {
        actionComplete = (args?: any): void => {
    ...
    it('Click Add - with selection', (done: Function) => {
        actionComplete = (args?: any): void => {
            if (args.requestType === 'add') {
                expect(args.row.rowIndex).toBe(1);
                done();
            }
        };
        gridObj.actionComplete = actionComplete;
    ...
    it('Add another row - check datasource', (done: Function) => {
        let formEle: HTMLFormElement = gridObj.grid.editModule.formObj.element;
        ...
        actionComplete = (args?: any): void => {
            ...
            expect(gridObj.dataSource[1].taskID === 122).toBeTruthy();
            expect(gridObj.dataSource[1].taskName).toBe('second');
            done();
        };
        gridObj.actionComplete = actionComplete;
        (<any>gridObj.grid.toolbarModule).toolbarClickHandler({ item: { id: gridObj.grid.element.id + '_update' } });
    });
    it('Add row - with clear selection', (done: Function) => {
        actionComplete = (args?: any): void => {
            if (args.requestType === 'add') {
                expect(args.row.rowIndex).toBe(0);
                done();
    ...
    it('Check Duplicate record added', (done: Function) => {
        let formEle: HTMLFormElement = gridObj.grid.editModule.formObj.element;
        ...
        actionComplete = (args?: any): void => {
            ...
            expect(gridObj.dataSource[0].taskID === 123).toBeTruthy();
            expect(gridObj.dataSource[0].taskName).toBe('third');
            done();
        };
        gridObj.actionComplete = actionComplete;
        (<any>gridObj.grid.toolbarModule).toolbarClickHandler({ item: { id: gridObj.grid.element.id + '_update' } });
