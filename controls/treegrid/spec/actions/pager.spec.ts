    ...
    gridObj.collapsed = (args?: RowCollapsedEventArgs) => {
      expect(
        gridObj
          .getRows()[0]
          .querySelector("span")
          .classList.contains("e-treegridcollapse")
      ).toBe(true);
      done();
    };
    gridObj.collapseRow(
      gridObj.getRows()[0],
      gridObj.getCurrentViewRecords()[0]
    );
  });
    ...
    gridObj.expanded = (args?: RowExpandedEventArgs) => {
      expect(
        gridObj
          .getRows()[0]
          .querySelector("span")
          .classList.contains("e-treegridexpand")
      ).toBe(true);
      done();
    };
...
describe("Pager with filter, sort, summary", () => {
  ...
  it("Check successful collapsing", (done: Function) => {
    gridObj.collapsed = (args?: RowCollapsedEventArgs) => {
      expect(
        gridObj.getRows()[1].querySelector(".e-treecell").innerHTML === "Design"
      ).toBe(true);
      done();
    };
...
describe("EJ2-22983: DataSource is not proper whose parentIDMapping record is not defined", () => {
  ...
  it("Rendering of unordered list", () => {
    expect(
      gridObj.getRows().length === gridObj.getCurrentViewRecords().length
    ).toBe(true);
  });
...
describe("updateExternalMessage method testing", () => {
  ...
  it("updateExternalMessage method testing", () => {
    ...
    expect(
      gridObj.grid.getPager().getElementsByClassName("e-pagerexternalmsg")[0]
        .innerHTML == "Testing"
    ).toBe(true);
  });
  afterAll(() => {
