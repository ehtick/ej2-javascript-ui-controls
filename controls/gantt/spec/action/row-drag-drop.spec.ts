...
describe('Dragging record in collapsed state', () => {
    let ganttObj_self: Gantt;
    let editingData = [
        {
        ...
        },
        {
            ...
            resources: [{ resourceId: 1 }], Predecessor: 2, Progress: 30, work: 20
        },
        {
            TaskID: 4, TaskName: 'Soil test approval', StartDate: new Date('04/09/2024'), Duration: 4,
            resources: [{ resourceId: 1}], Predecessor: 3, Progress: 30, work: 10,
        },
        {
            TaskID: 5, TaskName: 'Soil test approval', StartDate: new Date('04/09/2024'), Duration: 4,
            resources: [{ resourceId: 2}], Progress: 30, work: 10,
        },
    ];
    let resourceData = [
        { resourceId: 1, resourceName: 'Martin Tamer', resourceGroup: 'Planning Team', isExpand: false },
    ...
    it('Disabeling overlap', function () {
        ganttObj_self.allowTaskbarOverlap = false;
        
    });
    it('Reordering record', function () {
        ganttObj_self.reorderRows([1], 4, 'child');
    });
    it('Check for dates', function () {
        let firstDate = ganttObj_self.flatData[2].ganttProperties.startDate.getDate();
        let dragElement: HTMLElement = ganttObj_self.chartPane.querySelector('.e-content').querySelector('tr:last-of-type').querySelector('.e-gantt-child-taskbar') as HTMLElement
        if (dragElement) {
            triggerMouseEvent(dragElement, 'mousedown', dragElement.offsetLeft, dragElement.offsetTop);
            triggerMouseEvent(dragElement, 'mousemove', dragElement.offsetLeft + 200, 0);
            triggerMouseEvent(dragElement, 'mouseup');
            let lastDate = ganttObj_self.flatData[1].ganttProperties.startDate.getDate();
...
describe('indent/outdent actions for unscheduled task', () => {
    let ganttObj_self: Gantt;
    let data = [
        {
            TaskID: 1,
        ...
        ganttObj_self = createGantt(
            {
                ...
                toolbar: ['Add', 'Edit', 'Update', 'Delete', 'Cancel', 'ExpandAll', 'CollapseAll', 'Search', 'ZoomIn', 'ZoomOut', 'ZoomToFit',
                    'PrevTimeSpan', 'NextTimeSpan', 'Indent', 'Outdent', 'Undo', 'Redo'],
    ...
    it('indent/outdent for unscheduled task', function () {
        ganttObj_self.indent();
        ganttObj_self.undo();
        ganttObj_self.actionComplete = (args: any): void => {
            if (args.requestType == 'oudented') {
                expect(ganttObj_self.currentViewData[2].ganttProperties.predecessorsName).toBe('2FS');
            }
        };
    });
    afterAll(() => {
...
describe('indent/outdent actions for unscheduled task without predecessor', () => {
    let ganttObj_self: Gantt;
    let unscheduledData1 = [
        {
    ...
    it('indent/outdent for unscheduled task', function () {
        ganttObj_self.actionComplete = (args: any): void => {
            if (args.requestType == 'indented') {
                expect(ganttObj_self.getFormatedDate(ganttObj_self.flatData[1].ganttProperties.startDate, 'M/d/yyyy')).toBe('4/3/2019');
            }
        };
        ganttObj_self.indent();
    });
...
describe('set cancel property as true in action begin', () => {
    let splitTasksData: object[] = [
        {
            TaskID: 1,
            TaskName: "Project Schedule",
    ...
    it('Drag and drop', () => {
        ganttObj_resource.reorderRows([2], 4, 'child');
        expect(ganttObj_resource.flatData.length).toBe(6);
    });
    afterAll(() => {
        if (ganttObj_resource) {
            destroyGantt(ganttObj_resource);
