...
import { customZoomingLevels, customZoomingLevels1, defaultGanttData, editingData18, editingResources, editingResources1, manualData, projectNewData23, resourceDataUndo, resourceResourcesUndo, tempData1, tempData2, tempData3, tempData4, tempData5, tempData6, zoomData, zoomData1, zoomInData, zoomingData1, timelineData,projectNewData, MT905728, projectNewData1 } from './data-source.spec';
...
describe('Fit to project issue', () => {
    ...
    it('check fit to project works properly', () => {
        ...
        ganttObj.fitToProject();
        ganttObj.dataBind();
        expect(ganttObj.getFormatedDate(ganttObj.cloneProjectStartDate, 'MM/dd/yyyy')).toEqual('04/02/2019');
    });
    afterAll(() => {
        if (ganttObj) {
            destroyGantt(ganttObj);
...
describe('Fit to project display wrong timeline', () => {
    ...
    //     ganttObj.fitToProject();
    //     ganttObj.dataBind();
    //     expect(ganttObj.getFormatedDate(ganttObj.cloneProjectEndDate, 'MM/dd/yyyy')).toEqual('04/26/2024');
    // });
    afterAll(() => {
        if (ganttObj) {
            destroyGantt(ganttObj);
...
describe('timeline with showeekend as false', () => {
    let ganttObj: Gantt;
    const data: Object[] =  [
        {
            TaskID: 1,
    ...
    it('calculate enddate without weekend without weekends', () => {
        expect(ganttObj.getFormatedDate(ganttObj.flatData[1].ganttProperties.endDate, 'MM/dd/yyyy')).toBe('04/08/2019');
    });
    it('left resizing without weekend without weekends', () => {
        ganttObj.editModule.taskbarEditModule.getDateByLeft(461)
        expect(ganttObj.getFormatedDate(ganttObj.flatData[1].ganttProperties.endDate, 'MM/dd/yyyy')).toBe('04/08/2019');
    });
...
describe('timeline with showeekend as false in split task', () => {
    let ganttObj: Gantt;
    const splitTasksData: Object[] = [
        {
            TaskID: 1,
                    ...
                    subtasks: [
                        {
                            ...
                            Duration: 60, Progress: '60',
                            Segments: [
                                { StartDate: new Date('02/04/2019'), Duration: 12 },
                                { StartDate: new Date('02/05/2019'), Duration: 9 },
                                { StartDate: new Date('02/08/2019'), Duration: 13 }
                              ]
                        }
                    ]
                }
    ...
    beforeAll((done: Function) => {
        ganttObj = createGantt(
        {
            ...
            projectEndDate: new Date('06/04/2019')
    ...
    it('split task - date validation', () => {
            ...
            expect(ganttObj.flatData[2].ganttProperties.segments.length).toBe(4);
        });
    afterAll(() => {
        if (ganttObj) {
            destroyGantt(ganttObj);
...
describe('timeline with showeekend as false in split task', () => {
    ...
    it('merging task with action begin args as false', (done:Function) => {
...
describe('processing zoom to fit ', () => {
    let ganttObj: Gantt;
    const data: Object[] = [
            {
                TaskID: 1,
                TaskName: 'Project Initiation',
                StartDate: new Date('2019-04-02T09:00:00'),
                EndDate: new Date('2019-04-02T10:00:00') 
    ...
    beforeAll((done: Function) => {
        ganttObj = createGantt(
        {
            ...
            height: '550px',
            allowUnscheduledTasks: true,
            projectStartDate: new Date('2019-04-02T09:00:00'),
            projectEndDate: new Date('2019-04-02T10:00:00') 
           
        }, done);
    });
    it('processing zoom to fit with timeline setting as minutes', () => {
        ganttObj.timelineModule.processZoomToFit();
        expect(ganttObj.currentZoomingLevel.level).toBe(23);
    });
    afterAll(() => {
        if (ganttObj) {
            destroyGantt(ganttObj);
...
describe('zoomtfit in undo redo actions', () => {
    let ganttObj: Gantt;
    const data: Object[] = [
    {
        'TaskID': 1,
    ...
    it('zoomtfit in undo redo actions', () => {
        ganttObj.timelineModule.processZoomToFit();
        ganttObj.undo();
        ganttObj.timelineModule.processZoomToFit();
        ganttObj.undo();
...
describe('testcase to increase code coverage', () => {
    let ganttObj: Gantt;
    const data: Object[] = [
        {
            TaskID: 1,
    ...
    it('testcase to increase code coverage', () => {
        ganttObj.timelineModule['dateByLeftValue'](230,false);
        ganttObj.timelineModule['dateByLeftValue'](280,true,ganttObj.currentViewData[3].ganttProperties)
        ganttObj.timelineModule['bottomTierCellWidthCalc']('Minutes',{
        ...
        }, new Date(2023, 0, 1, 12, 0, 0, 0))
        ganttObj.timelineModule['bottomTierCellWidthCalc']('Year',{
...
describe('960606 - zoom out action with single tier', () => {
    let ganttObj: Gantt;
    const data: Object[] = [
        {
            TaskID: 1,
    ...
    it('zoom out action with single tier', () => {
        expect(ganttObj.timelineModule.customTimelineSettings.bottomTier.unit).toBe('None')
    });
    afterAll(() => {
        if (ganttObj) {
...
describe('Gantt with viewStart and ViewEndDate', () => {
    let ganttObj: Gantt;
    beforeAll((done: Function) => {
        ganttObj = createGantt(
        {
            ...
            timelineSettings: {
                ...
                viewStartDate: new Date('04/01/2019'),
                viewEndDate: 'auto',
            },
            labelSettings: {
                leftLabel: 'TaskID',
    ...
    it('check view start date', () => {
        expect(ganttObj.getFormatedDate(ganttObj.timelineModule.timelineStartDate, 'M/dd/yyyy')).toBe("4/01/2019");
    });
    afterAll(() => {
        if (ganttObj) {
            destroyGantt(ganttObj);
...
describe('DEVELOPMENT-End date is not proper in the timeline', () => {
    let ganttObj: Gantt;
    beforeAll((done: Function) => {
        ganttObj = createGantt(
        {
            ...
            timelineSettings: {
                viewStartDate: 'auto',
                viewEndDate: new Date('05/01/2019'),
                showTooltip: true,
                topTier: {
                    unit: 'Week',
                    format: 'dd/MM/yyyy'
    ...
    it('check view End Date', () => {
        expect(ganttObj.getFormatedDate(ganttObj.timelineModule.timelineEndDate, 'M/dd/yyyy')).toBe("5/01/2019");
    });
    afterAll(() => {
        if (ganttObj) {
            destroyGantt(ganttObj);
...
describe('Checking for code coverage', () => {
    let ganttObj: Gantt;
    let unscheduledData: Object[] = [
        {
            TaskId: 1, TaskName: 'Task 1', StartDate: new Date('04/02/2019'), Predecessor: '2FS',
            EndDate: new Date('04/21/2019'), Duration: '5', TaskType: '', BaselineStartDate: new Date('04/02/2019'), BaselineEndDate: new Date('04/02/2019')
        },
        {
            TaskId: 2, TaskName: 'Task 2', Duration: '5', TaskType: 'Task with duration only'
        },
        {
            TaskId: 3, TaskName: 'Task 3', StartDate: new Date('04/03/2019'), TaskType: 'Task with start date only', BaselineStartDate: new Date('04/03/2019'), BaselineEndDate: new Date('04/06/2019')
        },
        {
    ...
    beforeAll((done: Function) => {
        ganttObj = createGantt(
        {
            ...
            allowSorting:true,
            toolbar: ['Add', 'Edit', 'Update', 'Delete', 'Cancel','Undo','Redo', 'ExpandAll', 'CollapseAll', 'Search', 'ZoomIn', 'ZoomOut', 'ZoomToFit', 'CriticalPath', 
...
describe('Gantt ZoomIn action with viewStart and ViewEndDate', () => {
    let ganttObj: Gantt;
    beforeAll((done: Function) => {
        ganttObj = createGantt(
        {
            ...
            timelineSettings: {
                ...
                viewStartDate: new Date('04/01/2019'),
                viewEndDate: new Date('04/30/2019'),
            },
            labelSettings: {
                leftLabel: 'TaskID',
                rightLabel: 'Task Name: ${taskData.TaskName}',
    ...
    it("should render timeline with viewStart and viewEndDate", () => {
...
describe('Gantt ZoomIn action with viewStart and ViewEndDate', () => {
    ...
    beforeAll((done: Function) => {
        ganttObj = createGantt(
        {
            ...
            projectStartDate: new Date('5/01/2019'),
            projectEndDate: new Date('4/30/2019'),
            labelSettings: {
                leftLabel: 'TaskID',
                rightLabel: 'Task Name: ${taskData.TaskName}',
                taskLabel: '${Progress}%'
...
describe('Gantt ZoomToFit action with viewStart and ViewEndDate', () => {
    let ganttObj: Gantt;
    beforeAll((done: Function) => {
        ganttObj = createGantt(
        {
            ...
            projectStartDate: new Date('3/01/2019'),
            projectEndDate: new Date('4/30/2019'),
            labelSettings: {
                leftLabel: 'TaskID',
                rightLabel: 'Task Name: ${taskData.TaskName}',
...
describe('Gantt with viewStart and ViewEndDate is set to auto - project end date is less than UI width' , () => {
    let ganttObj: Gantt;
    beforeAll((done: Function) => {
        ganttObj = createGantt(
        {
            ...
            projectStartDate: new Date('3/30/2019'),
            projectEndDate: new Date('4/15/2019'),
            labelSettings: {
                leftLabel: 'TaskID',
                rightLabel: 'Task Name: ${taskData.TaskName}',
                taskLabel: '${Progress}%'
...
describe('Gantt with viewStart is set auto and project start date is defined' , () => {
    let ganttObj: Gantt;
    beforeAll((done: Function) => {
        ganttObj = createGantt(
        {
    ...
    it('refresh timeline coverage', ()=>{
        ganttObj.projectEndDate = undefined;
        ganttObj.timelineSettings.viewStartDate = new Date('04/01/2019');
    })
    afterAll(() => {
        if (ganttObj) {
            destroyGantt(ganttObj);
...
describe('Gantt with viewEndDate is set auto and project start date is defined' , () => {
    let ganttObj: Gantt;
    beforeAll((done: Function) => {
        ganttObj = createGantt(
        {
            ...
            timelineSettings: {
                ...
                viewStartDate: new Date('3/30/2019'),
                viewEndDate: 'auto',
            },
            projectEndDate: new Date('4/30/2019'),
            labelSettings: {
    ...
    it('timeline Start and timeline End date', () => {
        ...
        ganttObj.timelineSettings= {
                viewStartDate: new Date('3/29/2019')
            }
    });
    it('calculateIteration method', () => {
        ganttObj.timelineModule['increaseIteration'] = true;
        (ganttObj as any).timelineModule['calculateIteration'](new Date('3/30/2019') ,new Date('3/30/2019'), 'Hour', '' )
    });
    it('calculateIteration method', () => {
        ganttObj.timelineModule['isFirstLoop'] = true;
        ganttObj.timelineModule['increaseIteration'] = true;
        (ganttObj as any).timelineModule['calculateIteration'](new Date('3/30/2019') ,new Date('3/30/2019'), 'Minutes', '' )
    });
...
describe('DST Coverage - Timeline creation with Hour mode', () => {
    let ganttObj: Gantt;
    const dstData: Object[] = [
        {
            TaskID: 1,
            TaskName: 'DST Task',
    ...
    beforeAll((done: Function) => {
        ganttObj = createGantt({
            ...
            projectStartDate: new Date(2024, 2, 10),
            projectEndDate: new Date(2024, 2, 12),
            height: '450px'
        }, done);
    });

    it('DST Hour mode timeline creation', () => {
        ganttObj.dataOperation['cache'] = {2024: true};
        (ganttObj as any).pdfExportModule = {isPdfExport: true }
        ganttObj.timelineModule['createTimelineTemplate']('topTier');       
    });
...
describe('DST Coverage - Timeline creation with minute mode', () => {
    let ganttObj: Gantt;
    const dstData: Object[] = [
        {
            TaskID: 1,
    ...
    it('DST Hour mode timeline creation', () => {
        ganttObj.dataOperation['cache'] = {2024: true};
        (ganttObj as any).pdfExportModule = {isPdfExport: true };
        ganttObj.timelineModule['dstIncreaseHour'] = true;
        ganttObj.timelineModule['inconsistenceDstApplied'] = true;
        ganttObj.timelineModule['createTimelineTemplate']('bottomTier')
...
describe('CR-1028185: Dependency line and milestone not visible when taskbar renders near splitter', () => {
    ...
    it('Comparing timeline Start date and clone timeline start date- Unscheduled case', () => {
        expect(ganttObj.getFormatedDate(ganttObj.timelineModule.timelineStartDate, 'MM/dd/yyyy')).not.toBe(ganttObj.getFormatedDate(ganttObj.cloneProjectStartDate, 'MM/dd/yyyy'));
    });
    afterAll(() => {
        if (ganttObj) {
            destroyGantt(ganttObj);
