...
import { cellValidation, clearCFRule, ConditionalFormatModel, getColumn, getRow, updateCell, ExtendedImageModel , ExtendedSheet, ImageModel } from '../../workbook/index';
...
export function getCellPosition(
    ...
    return { top: top, dprTop: dprTop, left: left };
}
...
export function updateAction(
    ...
    case 'merge':
        ...
        if (spreadsheet.calculationMode === 'Manual' && isRedo &&
            ...
            for (let i: number = 0; i < eventArgs.model.length; i++) {
                const cells: string = eventArgs.model[i as number].cells;
                for (let j: number = 0; j < cells.length; j++) {
                    const value: string | number = eventArgs.model[i as number].cells[j as number].value;
                    model[i as number].cells[j as number].value =
                        (!isNullOrUndefined(value) && value !== '' ? value : model[i as number].cells[j as number].value) as string;
                }
            }
        }
    ...
    case 'deleteImage':
        if (isRedo === false) {
            spreadsheet.notify(
                createImageElement, { options: { src: options.eventArgs.imageData, height: options.eventArgs.imageHeight, width:
                options.eventArgs.imageWidth, top: options.eventArgs.imageTop, left: options.eventArgs.imageLeft,
                preservePos: options.eventArgs.preservePos, id: options.eventArgs.id }, range: options.eventArgs.address, isPublic: false,
                isUndoRedo: true });
        } else {
            spreadsheet.notify(
    ...
    case 'hyperlink':
        ...
        spreadsheet.serviceLocator.getService<ICellRenderer>('cell').refreshRange(
            getIndexesFromAddress(eventArgs.address), false, false, false, true, isImported(spreadsheet));
        break;
...
export function setRowEleHeight(
    ...
    adjustImageTopPosition(parent, sheet, rowIdx, dprHgt - prevHgt);
 ...
 * @param {SheetModel} sheet - specify the column width
 * @param {number} rowIdx - specify the rowIdx
 * @param {number} threshold - Specifies the threshold value to be added to the image's top position.
 * @returns {void} - Allign image top position based on the row height.
 */
export function adjustImageTopPosition(parent: Spreadsheet, sheet: SheetModel, rowIdx: number, threshold: number): void {
    if (threshold != null && threshold !== 0) {
        const extendedSheet: ExtendedSheet = <ExtendedSheet>sheet;
        if (extendedSheet.imageColl && !(sheet.frozenRows || sheet.frozenColumns)) {
            extendedSheet.imageColl.forEach((image: ExtendedImageModel) => {
                if (image.address && rowIdx <= image.address[0]) {
                    const cell: CellModel = getCell(image.address[0], image.address[1], sheet, false, true);
                    if (cell.image) {
                        cell.image.forEach((img: ImageModel) => {
                            if (img.top) {
                                img.top += threshold;
                                const imgEle: HTMLElement = document.getElementById(img.id);
                                if (imgEle) {
                                    imgEle.style.top = `${parseFloat(imgEle.style.top) + addDPRValue(threshold)}px`;
                                }
                            }
                        });
                    }
