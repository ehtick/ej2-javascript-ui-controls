...
import { getRangeIndexes, SheetModel, setImage, ExtendedImageModel, ExtendedSheet } from '../../workbook/index';
...
export class SpreadsheetImage {
    ...
    private createImageElement(args: {
        ...
        let overlayProps:
        { element: HTMLElement, top: number, dprTop: number, left: number } = overlayObj.insertOverlayElement(id, range, sheetIndex);
        ...
        const setTopLeftPosition: Function = (): void => {
            if (!isNullOrUndefined(args.options.top)) {
                overlayProps.element.style.top = Number(((!isFreezePane && args.options.preservePos && overlayProps.top) ?
                    overlayProps.dprTop + addDPRValue(Math.abs(args.options.top - overlayProps.top)) :
                    addDPRValue(args.options.top)).toFixed(2)) + 'px';
            }
        ...
        const imgData: ExtendedImageModel = {
            ...
            preservePos: args.options.preservePos
        };
    ...
    private refreshImgCellObj(args: BeforeImageRefreshData): void {
        ...
        if (prevCellObj && prevCellObj.image && prevCellImg.length > 0) {
            ...
            const extendedSheet: ExtendedSheet = sheet as ExtendedSheet;
            if (extendedSheet.imageColl) {
                let img: ExtendedImageModel;
                for (img of extendedSheet.imageColl as ExtendedImageModel[]) {
                    if (img.id === prevImgObj.id) {
                        img.address = [args.currentRowIdx, args.currentColIdx];
                        break;
                    }
                }
            }
