        ...
        if (scrollLeftIdx <= actIdxes[1] && hCont) {
            if (actIdxes[1] >= frozenCol) {
                if (scrollToCell) {
                    ...
                    if (scrollLeftIdx !== actIdxes[1]) {
                        const contWidth: number = hCont.clientWidth - this.parent.sheetModule.getScrollSize();
                        if (getColumnWidth(sheet, scrollLeftIdx) > contWidth) {
                            threshold++;
                        }
                    }
