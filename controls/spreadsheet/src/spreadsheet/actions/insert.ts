...
export class Insert {
    ...
    private insert(actionArgs: ActionEventArgs): void {
        ...
        const shouldDeferSheetDom: boolean = isSuspended && args.modelType === 'Sheet';
        const shouldDeferDom: boolean = shouldDeferRowColDom || shouldDeferSheetDom;
        if (shouldDeferDom && this.parent.pendingPaintRefresh !== 'fullSheet') {
        ...
        case 'Sheet':
            if (!shouldDeferSheetDom) {
                ...
                this.parent.queuePaintAction('insertSheet', (): void => {
                    this.parent.notify(
                        insertSheetTab, {
                            startIdx: args.index, endIdx: args.index + (args.model.length - 1),
