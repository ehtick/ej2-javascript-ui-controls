...
export class WorkbookFormula {
    ...
    private init(): void {
        ...
        this.parent.customFormulaCollection.forEach((value: IFormulaColl, key: string) => {
            this.addCustomFunction(value.handler, key, value.description, value.category);
        });
    }
    ...
    private performFormulaOperation(args: { [key: string]: Object }): void {
            ...
            for (let i: number = 0; i < Array.from(formulas.values()).length; i++) {
                const category: string = (formulaInfo[i as number].category ?
                    (l10n.getConstant(formulaInfo[i as number].category.split(' ').join('').replace('&', '')) ||
                        formulaInfo[i as number].category) : '');
        ...
        case 'addCustomFunction':
            this.addCustomFunction(<string | Function>args.functionHandler, <string>args.functionName,
                <string>args.formulaDescription, <string>args.category);
            break;
    ...
    private addCustomFunction(functionHandler: string | Function, functionName: string, formulaDescription: string,
                              category: string): void {
        this.calculateInstance.defineFunction(functionName, functionHandler, formulaDescription, category);
    }
