...
import { CellModel ,ExtendedImageModel, ExtendedSheet, ImageModel, setImage, SheetModel } from '../../../src/index';
...
describe('Image ->', () => {
    ...
    describe('EJ2-967471, Migrate Image properties from the cell model to the sheet model during import ->', () => {
        ...
        it('Checking sheet images update to cells during import action', (done: Function) => {
            ...
            expect(JSON.stringify(sheet.rows[0].cells[0].image[0]).includes('{"src":"https://www.w3schools.com/images/w3schools_green.jpg"')).toBeTruthy();
            expect(JSON.stringify(sheet.rows[0].cells[1].image[0]).includes('{"src":"https://www.w3schools.com/images/w3schools_green.jpg"')).toBeTruthy();
            expect(JSON.stringify(sheet.rows[0].cells[3].image[0]).includes('{"src":"https://www.w3schools.com/images/w3schools_green.jpg"')).toBeTruthy();
            expect(JSON.stringify(sheet.rows[0].cells[3].image[1]).includes('{"src":"https://www.w3schools.com/images/w3schools_green.jpg"')).toBeTruthy();
            expect(sheet.imageColl).toBeDefined();
            done();
        });
    });
    ...
    describe('EJ2-1008732: Image positions become misaligned when scrolling after importing a file in Spreadsheet ->', () => {
        beforeAll((done: Function) => {
            helper.initializeSpreadsheet({ sheets: [{ ranges: [{ dataSource: defaultData }] }] }, done);
        });
        afterAll(() => {
        ...
        it('should import multiple images with correct position mapping', (done: Function) => {
            ...
            setTimeout(() => {
                ...
                let cell: CellModel = sheet.rows[0].cells[0];
                expect(cell.image[0].top).toBe(4.6);
                expect(cell.image[0].left).toBe(9.6);
                expect(cell.image[0].height).toBe(41);
                expect(cell.image[0].width).toBe(42);
                cell = sheet.rows[34].cells[0];
                expect(cell.image[0].top).toBe(631.22222221);
                expect(cell.image[0].left).toBe(9.6);
                expect(cell.image[0].height).toBe(41);
                expect(cell.image[0].width).toBe(42);
                cell = sheet.rows[44].cells[0];
                expect(cell.image[0].top).toBe(818.22222221);
                expect(cell.image[0].left).toBe(9.6);
                expect(cell.image[0].height).toBe(41);
                expect(cell.image[0].width).toBe(42);
                done();
        ...
        it('Image drag and drop action should be refreshed properly', (done: Function) => {
            const sheet: SheetModel = helper.getInstance().sheets[0];
            const image: HTMLElement = helper.getElement().querySelector('.e-ss-overlay');
            helper.triggerMouseAction('mousedown', { x: image.getBoundingClientRect().left + 1, y: image.getBoundingClientRect().top + 1 }, image, image);
            const targetCell = helper.invoke('getCell', [0, 0]);
            const targetRect = targetCell.getBoundingClientRect();
            helper.triggerMouseAction('mousemove', { x: targetRect.left + 40, y: targetRect.top + 60 }, image, image);
            helper.triggerMouseAction('mouseup', { x: targetRect.left + 40, y: targetRect.top + 60 }, document, image);
            setTimeout(() => {
                let cell: CellModel = sheet.rows[3].cells[1];
                expect(cell.image.length).toBe(1);
                const imageArr: ImageModel = cell.image[0];
                expect(imageArr.height).toBe(41);
                expect(imageArr.width).toBe(42);
                expect(imageArr.top).toBe(59);
                expect(imageArr.left).toBe(39);
                cell = sheet.rows[0].cells[0];
                expect(cell.image[0]).toBeUndefined();
                done();
            });
        });
        it('Image previous position should be maintained properly on performing undo / redo action', (done: Function) => {
            const spreadsheet: Spreadsheet = helper.getInstance();
            const overlay: HTMLElement = helper.getElementFromSpreadsheet('.e-ss-overlay-active');
            helper.click('#spreadsheet_undo');
            setTimeout(() => {
                let image: ImageModel = spreadsheet.sheets[0].rows[0].cells[0].image[0];
                expect(image.height).toBe(41);
                expect(image.width).toBe(42);
                expect(image.top).toBe(4.6);
                expect(image.left).toBe(9.6);
                expect(overlay.style.height).toBe('41px');
                expect(overlay.style.width).toBe('42px');
                helper.click('#spreadsheet_redo');
                setTimeout(() => {
                    image = spreadsheet.sheets[0].rows[3].cells[1].image[0];
                    expect(image.height).toBe(41);
                    expect(image.width).toBe(42);
                    expect(image.top).toBe(59);
                    expect(image.left).toBe(39);
                    expect(overlay.style.height).toBe('41px');
                    expect(overlay.style.width).toBe('42px');
                    done();
                });
        ...
        it('Image updated position should be maintained properly on performing undo after delete action', (done: Function) => {
            const spreadsheet: Spreadsheet = helper.getInstance();
            let image: ImageModel[] = spreadsheet.sheets[0].rows[3].cells[1].image;
            const overlay: HTMLElement = helper.getElementFromSpreadsheet('.e-ss-overlay-active');
            expect(image.length).toBe(1);
            const imageId: string = image[0].id;
            helper.invoke('deleteImage', [imageId]);
            ...
            helper.click('#spreadsheet_undo');
            setTimeout(() => {
                image = spreadsheet.sheets[0].rows[3].cells[1].image;
                expect(image[0].height).toBe(41);
                expect(image[0].width).toBe(42);
                expect(image[0].top).toBe(59);
                expect(image[0].left).toBe(39);
                expect(overlay.style.height).toBe('41px');
                expect(overlay.style.width).toBe('42px');
                done();
            });
        });
        it('Perform scroll action check ->', (done: Function) => {
            helper.invoke('goTo', ['A68']);
            setTimeout(() => {
                expect(helper.getInstance().sheets[0].rows[68].cells[0].image).toBeDefined();
                done();
            });
        });
    });
