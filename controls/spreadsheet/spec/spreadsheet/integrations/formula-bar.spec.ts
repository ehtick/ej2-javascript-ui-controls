...
describe('Spreadsheet formula bar module ->', () => {
    ...
    describe('EJ2-1005178 ->', () => {
        ...
        it('Custom function appears under existing category when added with that category', (done: Function) => {
            const isPrime = (): string => { return 'IS PRIME TEST'; };
            const spreadsheet: Spreadsheet = helper.getInstance();
            spreadsheet.addCustomFunction(isPrime, 'ISPRIME', 'Check Prime', 'Math & Trig');
            helper.click('.e-formula-bar-panel .e-insert-function');
            helper.setAnimationToNone('.e-spreadsheet-function-dlg.e-dialog');
            const ddlObj2: any = getComponent(helper.getElement('.e-spreadsheet-function-dlg .e-dlg-content .e-input-group')
                .querySelector('.e-dropdownlist'), 'dropdownlist');
            ddlObj2.showPopup();
            // select the 'Math & Trig' category by finding the matching list item
            const popup: HTMLElement = helper.getElement('.e-ddl.e-popup');
            const items: NodeListOf<HTMLElement> = popup.querySelectorAll('li');
            for (let i = 0; i < items.length; i++) {
                if ((items[i].textContent || '').trim() === 'Math & Trig') { items[i].click(); break; }
            }
            setTimeout(() => {
                const formulaList: HTMLElement = helper.getElement('.e-spreadsheet-function-dlg .e-dlg-content .e-formula-list');
                const funcItems: NodeListOf<HTMLElement> = formulaList.querySelectorAll('li, .e-list-item');
                let found: boolean = false;
                for (let j = 0; j < funcItems.length; j++) {
                    if (((funcItems[j].textContent || '') as string).trim().indexOf('ISPRIME') > -1) { found = true; break; }
                }
        ...
        it('Custom function appears under new custom category and is listed when that category is selected', (done: Function) => {
            const medianCustom = (): string => { return 'MEDIAN CUSTOM TEST'; };
            const spreadsheet: Spreadsheet = helper.getInstance();
            spreadsheet.addCustomFunction(medianCustom, 'MEDIAN_CUSTOM', 'Median Custom Desc', 'Custom Statistics');
            helper.click('.e-formula-bar-panel .e-insert-function');
            helper.setAnimationToNone('.e-spreadsheet-function-dlg.e-dialog');
            const ddlObj3: any = getComponent(helper.getElement('.e-spreadsheet-function-dlg .e-dlg-content .e-input-group')
                .querySelector('.e-dropdownlist'), 'dropdownlist');
            ddlObj3.showPopup();
            const popup3: HTMLElement = helper.getElement('.e-ddl.e-popup');
            const items3: NodeListOf<HTMLElement> = popup3.querySelectorAll('li');
            for (let i = 0; i < items3.length; i++) {
                if ((items3[i].textContent || '').trim() === 'Custom Statistics') { items3[i].click(); break; }
            }
            setTimeout(() => {
                const formulaList: HTMLElement = helper.getElement('.e-spreadsheet-function-dlg .e-dlg-content .e-formula-list');
                const funcItems: NodeListOf<HTMLElement> = formulaList.querySelectorAll('li, .e-list-item');
                let found: boolean = false;
                for (let j = 0; j < funcItems.length; j++) {
                    if (((funcItems[j].textContent || '') as string).trim().indexOf('MEDIAN_CUSTOM') > -1) { found = true; break; }
                }
                expect(found).toBeTruthy();
                helper.click('.e-spreadsheet-function-dlg .e-footer-content button:nth-child(2)');
                done();
