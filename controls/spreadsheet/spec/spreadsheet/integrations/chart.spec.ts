...
describe('Chart ->', () => {
        ...
        describe('UI Interaction - Discontinuous Range Selection ->', () => {
            ...
            it('Insert Column chart with discontinuous range via UI->', (done: Function) => {
                ...
                setTimeout(() => {
                    ...
                    helper.invoke('numberFormat', ['$#,##0', 'D2:D3']);
                    helper.invoke('selectRange', ['B2:B2 B3:B3 D2:D2 D3:D3']);
                    helper.switchRibbonTab(2);
                    helper.getElement('#' + helper.id + '_chart-btn').click();
                    const target: HTMLElement = helper.getElement('#' + helper.id + '_chart-btn-popup .e-menu-item[aria-label="Column"]');
                    helper.triggerMouseAction('mouseover', { x: target.getBoundingClientRect().left + 5, y: target.getBoundingClientRect().top + 5 }, document, target);
    ...
    describe('EJ2-1016236: Chart rendering ignores chart.series collection and generates manually discontinuous Excel ranges on import ->', () => {
        beforeAll((done: Function) => {
            helper.initializeSpreadsheet({ sheets: [{ ranges: [{ dataSource: defaultData }] }] }, done);
        });
        afterAll(() => {
        ...
        it('importing discountinuous chart with date format in x range ->', (done: Function) => {
            const json: object = {
                Workbook: { sheets: [{ chartColl: [{ address: [5, 6], height: 284, id: "e_spreadsheet_chart_8", isSeriesInRows: false, left: 396, theme: "Material",
                        ...
                        { cells: [{ index: 0, value: '41801', format: 'm/d/yyyy' }, { index: 2, value: 15 }, { index: 4, value: 7 }] },
                        { cells: [{ index: 0, value: '41847', format: 'm/d/yyyy' }, { index: 2, value: 20 }, { index: 4, value: 11 }] },
                        { cells: [{ index: 0, value: '41964', format: 'm/d/yyyy' }, { index: 2, value: 10 }, { index: 4, value: 10 }] },
                        { cells: [{ index: 0, value: '41813', format: 'm/d/yyyy' }, { index: 2, value: 20 }, { index: 4, value: 13 }] },
