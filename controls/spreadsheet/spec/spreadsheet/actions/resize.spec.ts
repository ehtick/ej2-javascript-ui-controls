...
describe('Resize ->', () => {
    ...
    describe('EJ2-70281 ->', () => {
        ...
        it('Ensure the spreadsheet misbehaves when increasing the font size with apply conditional formatting through to entire column', (done: Function) => {
            ...
            expect(helper.invoke('getCell', [2, 5]).getElementsByClassName('e-databar')[1].style.height).toBe('22px');
            expect(helper.invoke('getCell', [2, 6]).getElementsByClassName('e-databar')[1].style.height).toBe('22px');
            expect(helper.getInstance().sheets[0].rows[2].height).toBe(25);
    ...
    describe('EJ2-859445 ->', () => {
        ...
        it('EJ2-923139 - The width of the data bar changes when the row height is adjusted', (done: Function) => {
            helper.invoke('conditionalFormat', [{ type: 'BlueDataBar', range: 'F2:F11' }]);
            ...
            expect(helper.invoke('getRow', [6]).style.height).toBe('20px');
            helper.invoke('setRowHeight', [40, 6]);
            expect(helper.invoke('getCell', [6, 5]).getElementsByClassName('e-databar')[1].style.width).toBe('67%');
