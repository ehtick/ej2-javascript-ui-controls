...
describe('Clipboard ->', () => {
            ...
            setTimeout(() => {
                ...
                setTimeout(() => {
                    helper.invoke('copy', ['A2:C5']).then(() => {
                        ...
                        helper.invoke('paste');
                        setTimeout(() => {
                            expect(JSON.stringify(spreadsheet.sheets[0].rows[12].cells[1].validation)).toBe('{"type":"List","operator":"Between","value1":"1,2,3,4","value2":"","ignoreBlank":true,"inCellDropDown":true}');
                            expect(JSON.stringify(spreadsheet.sheets[0].rows[13].cells[1].validation)).toBe('{"type":"List","operator":"Between","value1":"1,2,3,4","value2":"","ignoreBlank":true,"inCellDropDown":true}');
                            expect(JSON.stringify(spreadsheet.sheets[0].rows[14].cells[1].validation)).toBe('{"type":"List","operator":"Between","value1":"1,2,3,4","value2":"","ignoreBlank":true,"inCellDropDown":true}');
                            expect(JSON.stringify(spreadsheet.sheets[0].rows[15].cells[1].validation)).toBe('{"type":"List","operator":"Between","value1":"1,2,3,4","value2":"","ignoreBlank":true,"inCellDropDown":true}');
    ...
    describe('EJ2-914385 -> Providing support for custom data validation', () => {
        ...
        it('Copy, Cut and Paste same sheet - Cell validation', (done: Function) => {
            const spreadsheet: Spreadsheet = helper.getInstance();
            ...
            helper.invoke('copy').then(function () {
                helper.invoke('selectRange', ['I1']);
                helper.invoke('paste');
                expect(spreadsheet.sheets[spreadsheet.activeSheetIndex].rows[1].cells[8].validation.value1).toBe('=G3');
                expect(spreadsheet.sheets[spreadsheet.activeSheetIndex].rows[1].cells[8].value.toString()).toBe('20');
                helper.invoke('cut', ['E3:E4']).then(function () {
                    helper.invoke('selectRange', ['J1']);
                    helper.invoke('paste');
                    expect(spreadsheet.sheets[spreadsheet.activeSheetIndex].rows[0].cells[9].validation.value1).toBe('=C4');
                    expect(spreadsheet.sheets[spreadsheet.activeSheetIndex].rows[0].cells[9].value.toString()).toBe('30');
                    done();
                });
            });
        });
        it("Copy, Cut and Paste different sheet - cell validation", (done: Function) => {
            const spreadsheet: Spreadsheet = helper.getInstance();
            helper.invoke('selectRange', ['E1:E2']);
            helper.invoke('copy').then(function () {
                const args = { action: 'gotoSheet', eventArgs: { currentSheetIndex: 1, previousSheetIndex: 0 } };
                ...
                setTimeout((): void => {
                    ...
                    expect(spreadsheet.sheets[spreadsheet.activeSheetIndex].rows[1].cells[8].validation.value1).toBe('=G3');
                    expect(spreadsheet.sheets[spreadsheet.activeSheetIndex].rows[1].cells[8].value.toString()).toBe('20');
                    helper.invoke('cut', ['I1:I2']).then(function () {
                        const args = { action: 'gotoSheet', eventArgs: { currentSheetIndex: 0, previousSheetIndex: 1 } };
                        helper.getInstance().updateAction(args);
                        setTimeout((): void => {
                            helper.invoke('selectRange', ['J3']);
                            helper.invoke('paste');
                            expect(spreadsheet.sheets[spreadsheet.activeSheetIndex].rows[2].cells[9].validation.value1).toBe('=G2');
                            expect(spreadsheet.sheets[spreadsheet.activeSheetIndex].rows[2].cells[9].value.toString()).toBe('Price');
                            done();
                        });
                    });
                });
    ...
    describe('EJ2-987921 -> Formula Reference Incorrectly Updated Causing #REF Errors When Copy-Pasting in Spreadsheet0', () => {
        ...
        it('EJ2: 1002792 Formula paste into multi-column range produces incorrect references - I', (done: Function) => {
            helper.edit('H12', '=H6 + 100');
            helper.invoke('selectRange', ['H12']);
            helper.invoke('copy').then(() => {
                helper.invoke('selectRange', ['A13:H13']);
                helper.invoke('paste');
                const row: RowModel = helper.getInstance().sheets[0].rows[12];
                expect(row.cells[0].value).toBe('#VALUE!');
                expect(row.cells[0].formula).toBe('=A7 + 100');
                expect(row.cells[1].value).toBe('41942');
                expect(row.cells[1].formula).toBe('=B7 + 100');
                expect(row.cells[1].format).toBe('m/d/yyyy');
                expect(row.cells[1].formattedText).toBe('10/30/2014');
                expect(row.cells[2].value).toBe('100.45547453703705');
                expect(row.cells[2].formula).toBe('=C7 + 100');
                expect(row.cells[2].format).toBe('h:mm:ss AM/PM');
                expect(row.cells[2].formattedText).toBe('10:55:53 AM');
                expect(row.cells[3].value).toBe('140');
                expect(row.cells[3].formula).toBe('=D7 + 100');
                expect(row.cells[3].format).toBeFalsy();
                expect(row.cells[3].formattedText).toBeFalsy();
                expect(row.cells[4].value).toBe('120');
                expect(row.cells[4].formula).toBe('=E7 + 100');
                expect(row.cells[4].format).toBeFalsy();
                expect(row.cells[4].formattedText).toBeFalsy();
                expect(row.cells[5].value).toBe('900');
                expect(row.cells[5].formula).toBe('=F7 + 100');
                expect(row.cells[5].format).toBeFalsy();
                expect(row.cells[5].formattedText).toBeFalsy();
                expect(row.cells[6].value).toBe('113');
                expect(row.cells[6].formula).toBe('=G7 + 100');
                expect(row.cells[6].format).toBeFalsy();
                expect(row.cells[6].formattedText).toBeFalsy();
                expect(row.cells[7].value).toBe('166');
                expect(row.cells[7].formula).toBe('=H7 + 100');
                expect(row.cells[7].format).toBeFalsy();
                expect(row.cells[7].formattedText).toBeFalsy();
                done();
            });
        });
        it('EJ2: 1002792 Formula paste into multi-column range produces incorrect references - II', (done: Function) => {
            helper.edit('H14', '=H6 + 100');
            helper.invoke('numberFormat', ['$#,##0_);($#,##0)', 'H14']);
            helper.invoke('selectRange', ['H14']);
            helper.invoke('copy').then(() => {
                helper.invoke('selectRange', ['A15:H15']);
                helper.invoke('paste');
                const row: RowModel = helper.getInstance().sheets[0].rows[14];
                expect(row.cells[0].value).toBe('#VALUE!');
                expect(row.cells[0].formula).toBe('=A7 + 100');
                expect(row.cells[1].value).toBe('41942');
                expect(row.cells[1].formula).toBe('=B7 + 100');
                expect(row.cells[1].format).toBe('$#,##0_);($#,##0)');
                expect(row.cells[1].formattedText).toBe('$41,942 ');
                expect(row.cells[2].value).toBe('100.45547453703705');
                expect(row.cells[2].formula).toBe('=C7 + 100');
                expect(row.cells[2].format).toBe('$#,##0_);($#,##0)');
                expect(row.cells[2].formattedText).toBe('$100 ');
                expect(row.cells[3].value).toBe('140');
                expect(row.cells[3].formula).toBe('=D7 + 100');
                expect(row.cells[3].format).toBe('$#,##0_);($#,##0)');
                expect(row.cells[3].formattedText).toBe('$140 ');
                expect(row.cells[4].value).toBe('120');
                expect(row.cells[4].formula).toBe('=E7 + 100');
                expect(row.cells[4].format).toBe('$#,##0_);($#,##0)');
                expect(row.cells[4].formattedText).toBe('$120 ');
                expect(row.cells[5].value).toBe('900');
                expect(row.cells[5].formula).toBe('=F7 + 100');
                expect(row.cells[5].format).toBe('$#,##0_);($#,##0)');
                expect(row.cells[5].formattedText).toBe('$900 ');
                expect(row.cells[6].value).toBe('113');
                expect(row.cells[6].formula).toBe('=G7 + 100');
                expect(row.cells[6].format).toBe('$#,##0_);($#,##0)');
                expect(row.cells[6].formattedText).toBe('$113 ');
                expect(row.cells[7].value).toBe('166');
                expect(row.cells[7].formula).toBe('=H7 + 100');
                expect(row.cells[7].format).toBe('$#,##0_);($#,##0)');
                expect(row.cells[7].formattedText).toBe('$166 ');
                done();
            });
        });
        it('EJ2: 1002792 Copy pasting formula reference cell to a format applied cell', (done: Function) => {
            helper.edit('B16', '123');
            helper.invoke('numberFormat', ['$#,##0.00', 'B16']);
            helper.invoke('selectRange', ['H14']);
            helper.invoke('copy').then(function () {
                helper.invoke('selectRange', ['B16:B16']);
                helper.invoke('paste');
                var row = helper.getInstance().sheets[0].rows[15];
                expect(row.cells[1].value).toBe('41774');
                expect(row.cells[1].formula).toBe('=B8 + 100');
                expect(row.cells[1].format).toBe('$#,##0_);($#,##0)');
                expect(row.cells[1].formattedText).toBe('$41,774 ');
                done();
            });
        });
    });
