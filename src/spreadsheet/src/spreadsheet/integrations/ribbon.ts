...
export class Ribbon {
    ...
    private calcTypeOptions: DropDownButton;
        ...
        },
        {
            header: { text: l10n.getConstant('Formulas') }, content: [
                ...
                { type: 'Separator', id: id + '_separator_15' },
                {
                    ...
                    id: id + '_calc_current_sheet', click: (): void => {
                ...
                },
                {
                    ...
                    id: id + '_calc_entire_sheets', click: (): void => {
                        this.parent.notify(workbookFormulaOperation, { action: 'ClearDependentCellCollection' });
    ...
    private createCalcOptions(id: string, l10n: L10n): Element {
        const typeBtn: HTMLElement = createElement(
            'button', { id: id + '_calc_types', attrs: { 'type': 'button' }, className: 'e-calc-types' });
        const typeBtnText: HTMLElement = createElement('span', { className: 'e-calc-types-text' });
    ...
    private selectCalcOptions(args: MenuEventArgs): void {
    ...
    private updateFormulaButtons(): void {
        const l10n: L10n = this.parent.serviceLocator.getService(locale);
    ...
    private cfSelected(args: MenuEventArgs): void {
        ...
        if (args.item.id.includes('_dlg')) {
            ...
            const isCFIcons: boolean = args.item.id.includes('icons');
            if (isCFIcons || args.item.id === 'cf_cr_cells' || args.item.id === 'cf_cr_sheet') {
                if (isReadOnlyCells(this.parent, getSwapRange(getRangeIndexes(sheet.selectedRange)))) {
                    ...
                    this.parent.notify(
                        setCFRule, <CFArgs>{ cfModel: { type: trgt.id || trgt.parentElement.id, range: sheet.selectedRange },
                            isAction: true });
                ...
                if (indexes[1] === indexes[3] && indexes[0] === indexes[2]) {
                    ...
                    if (cell.rowSpan > 1 || cell.colSpan > 1) {
                        this.toggleActiveState(
                            (!(cell.rowSpan > 1) || indexes[0] + cell.rowSpan - 1 === indexes[2]) && (!(cell.colSpan > 1) ||
