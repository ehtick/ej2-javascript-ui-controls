        ...
        const json: string = JSON.stringify(model, (key: string, value: { [key: string]: object }) => {
            if (skipProp.indexOf(key) > -1) {
                ...
                if (key === 'validation' && value && typeof value.type === 'string' && value.type === 'Time') {
                    ['value1', 'value2'].forEach((valKey: string) => {
                        const val: string = value[valKey as string] && value[valKey as string].toString();
                        if (val && !isNumber(Number(val))) {
                            value[valKey as string] = this.getDateAsNumber({ range: [], cell: { value: val } }, val) as Object;
                        }
                    });
                }
